...
from app.logic.events.volunteer_allocation.add_volunteer_to_event_form_contents import AVAILABILITY, \
    MAKE_CADET_CONNECTION, PREFERRED_DUTIES, SAME_OR_DIFFERENT,  NOTES
...
def get_volunteer_at_event_from_form_contents(interface: abstractInterface):
    ...
    event =get_event_from_state(interface)
    availability_in_form = get_availablity_from_form(interface=interface, event=event, input_name=AVAILABILITY)
    ...
    any_other_information = get_any_other_information(interface=interface, volunteer_id=volunteer_id, event=event)
...
def get_any_other_information(interface: abstractInterface, event: Event, volunteer_id: str) -> str:
