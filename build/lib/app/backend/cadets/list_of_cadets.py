...
from app.data_access.store.object_definitions import object_definition_for_list_of_cadets


def get_matching_cadet(object_store:ObjectStore, cadet: Cadet, exact_match_required: bool = True) -> Cadet:
    list_of_cadets = get_list_of_cadets(object_store)

    return list_of_cadets.matching_cadet(cadet=cadet, exact_match_required=exact_match_required)
...
def get_list_of_cadets_sorted_by_surname(
    object_store: ObjectStore) -> ListOfCadets:
    return get_sorted_list_of_cadets(object_store=object_store, sort_by=SORT_BY_SURNAME)

def get_sorted_list_of_cadets(object_store: ObjectStore, sort_by: str = arg_not_passed) -> ListOfCadets:
...
def update_list_of_cadets(object_store: ObjectStore, updated_list_of_cadets:ListOfCadets):
    object_store.update(new_object=updated_list_of_cadets, object_definition=object_definition_for_list_of_cadets)
