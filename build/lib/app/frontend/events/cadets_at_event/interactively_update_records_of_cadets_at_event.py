...
from app.backend.registration_data.cadet_registration_data import is_cadet_with_id_already_at_event, \
    add_new_cadet_to_event_from_row_in_registration_data, get_cadet_at_event
...
from app.frontend.events.cadets_at_event.update_existing_cadet_at_event_forms import (
    display_form_for_update_to_existing_cadet_at_event, USE_ORIGINAL_DATA_BUTTON_LABEL, USE_NEW_DATA_BUTTON_LABEL,
...
def process_next_cadet_at_event(interface: abstractInterface) -> Union[Form, NewForm]:
    ...
    try:
        cadet_id = get_and_save_next_cadet_in_event_data(interface)
    except NoMoreData:
...
def process_update_to_existing_cadet_at_event(
    ...
    new_cadet_at_event = get_cadet_at_event_from_row_in_event_raw_registration_data(
