...
class SqlDataListOfTickSubStages(GenericSqlData):
    def modify_substage_name(
        ...
        existing_item = self.get_substage_with_id(
            substage_id=existing_substage_id, default=missing_data
        )
        if existing_item is missing_data:
            raise Exception(
                "Can't modify non existent item with id %s" % existing_substage_id
            )

        if existing_item.name == new_name:
            return

        if self.does_substage_with_name_exist(
            new_name, qualification_id=existing_item.stage_id
        ):
            raise Exception(
        ...
        self._modify_substage_name_without_checks(
            existing_substage_id=existing_substage_id, new_name=new_name
        )
    ...
    def add_new_substage_to_qualification(
        ...
        if self.does_substage_with_name_exist(
            substage_name=new_substage_name, qualification_id=qualification_id
        ):
            raise Exception(
                "Can't add %s as already exists for this qualification"
                % new_substage_name
            )

        new_substage = TickSubStage(
    ...
    def read(self) -> ListOfTickSubStages:
        ...
        new_list = [
            TickSubStage(
                name=raw_item[0], id=str(raw_item[1]), stage_id=str(raw_item[2])
            )
    ...
    def _add_substage_without_checks_or_commit(self, substage: TickSubStage):
        ...
        self.cursor.execute(insertion, (name, id, stage_id))
