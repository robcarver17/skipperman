from app.objects_OLD.utils import in_x_not_in_y
...
from app.objects_OLD.primtive_with_id.patrol_boats import ListOfPatrolBoats
...
def get_list_of_volunteers_allocated_to_patrol_boat_at_event_on_any_data(
    ...
    list_of_voluteers_at_event_with_patrol_boats = get_list_of_voluteers_at_event_with_patrol_boats_from_cache(
        cache=cache, event=event
    )
    return list_of_voluteers_at_event_with_patrol_boats.assigned_to_any_boat_on_any_day()
...
def  get_list_of_voluteers_at_event_with_patrol_boats(data_layer: DataLayer, event: Event) -> ListOfVolunteersAtEventWithSkillsAndRolesAndPatrolBoats:
    patrol_boat_data = PatrolBoatData(data_layer)
    list_of_voluteers_at_event_with_patrol_boats = \
        patrol_boat_data.get_list_of_volunteers_at_event_with_patrol_boat_and_role(event)
...
def load_list_of_patrol_boats_at_event_from_cache(
    ...
    return cache.get_from_cache(get_list_of_unique_boats_at_event_including_unallocated, event=event)
...
def get_sorted_list_of_boats_excluding_boats_already_at_event(
    ...
    list_of_patrol_boats_at_event = load_list_of_patrol_boats_at_event_from_cache(cache=cache, event=event)
