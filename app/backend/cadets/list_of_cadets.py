...
from app.objects.utilities.cadet_matching_and_sorting import (
    ...
    sort_list_of_cadets_by_similarity,
)
...
def get_matching_cadet(object_store: ObjectStore, cadet: Cadet) -> Cadet:
    return object_store.data_api.data_list_of_cadets.get_matching_cadet(
        cadet=cadet
    )  ## used for iterations, safer not to cache or used cache
...
def get_sorted_list_of_cadets(
    ...
    list_of_cadets = object_store.get(
        object_store.data_api.data_list_of_cadets.read,
        sort_by=sort_by,
    ...
    if sort_by in [
        ...
        return sort_list_of_cadets_by_similarity(
            list_of_cadets, other_cadet=similar_cadet
        )
...
def bulk_update_of_list_of_cadets(
    ...
    interface.update(
        interface.object_store.data_api.data_list_of_cadets.write,
        list_of_cadets=updated_list_of_cadets,
    )
