...
from app.backend.food.modify_food_data import is_cadet_with_already_at_event_with_food, \
    ...
    add_new_volunteer_with_food_to_event, remove_food_requirements_for_cadet_at_event
...
def update_food_for_cadets_from_registration_data(interface: abstractInterface):
    ...
    for cadet, registration_data in dict_of_cadets_at_event_with_registration_data.items():
        process_update_to_cadet_food_data(
            interface=interface, event=event, cadet=cadet, registration_data=registration_data
        )


def process_update_to_cadet_food_data(
    interface: abstractInterface, event: Event, cadet: Cadet,  registration_data: CadetRegistrationData
):
...
def process_update_to_cadet_food_data_if_already_at_event(
    ...
    remove_food_requirements_for_cadet_at_event(object_store=interface.object_store,
                                                event=event,
                                                cadet=cadet)
...
def process_update_to_cadet_food_data_if_new_to_event(
    ...
    food_from_registration = registration_data.data_in_row.get_item(CADET_FOOD_PREFERENCE, "")
...
def update_food_for_volunteers_from_registration_data(
    ...
    event = get_event_from_state(interface)
    dict_volunteers_registered_to_event = get_dict_of_registration_data_for_volunteers_at_event(object_store=interface.object_store,
                                                                                                event=event)
