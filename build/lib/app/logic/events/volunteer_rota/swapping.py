...
def swap_button_value_for_volunteer_id_and_day(volunteer_id: str, day: Day) -> str:
    return generic_button_value_for_volunteer_id_and_day(button_type="SWAP", volunteer_id=volunteer_id, day=day)


def get_list_of_swap_buttons(event: Event):
...
def get_swap_button(volunteer_in_role_at_event_on_day: VolunteerInRoleAtEvent,
        interface: abstractInterface)-> Button:

    if is_ready_to_swap(interface):
        return swap_button_if_ready_to_swap(volunteer_in_role_at_event_on_day=volunteer_in_role_at_event_on_day,
                                            interface=interface)
    else:
...
def swap_button_if_ready_to_swap(volunteer_in_role_at_event_on_day: VolunteerInRoleAtEvent, interface:abstractInterface)-> Union[Button,str]:
...
def swap_button_value_for_volunteer_in_role_on_day(
    ...
    return swap_button_value_for_volunteer_id_and_day(volunteer_id=volunteer_in_role_at_event_on_day.volunteer_id,
...
def get_day_volunteer_id_from_swap_state(interface: abstractInterface) -> Tuple[ Day, str]:
...
def get_and_store_swap_state_from_button_pressed(interface:abstractInterface, swap_button: str):
    ...
    swap_state = SwapButtonState(ready_to_swap=True, dict_of_thing_to_swap=dict(day_str=day.name, volunteer_id=volunteer_id))
    store_swap_state(interface=interface, swap_state=swap_state)


def update_if_swap_button_pressed_and_ready_to_swap(interface: abstractInterface, swap_button: str):
    original_volunteer_id, original_day = from_known_button_to_volunteer_id_and_day(swap_button)
    day_to_swap_with, volunteer_id_to_swap_with = get_day_volunteer_id_from_swap_state(interface)
