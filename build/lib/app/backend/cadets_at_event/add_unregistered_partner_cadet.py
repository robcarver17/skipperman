...
from app.backend.groups.cadets_with_groups_at_event import get_dict_of_cadets_with_groups_at_event, update_dict_of_cadets_with_groups_at_event
from app.backend.boat_classes.cadets_with_boat_classes_at_event import get_dict_of_cadets_and_boat_classes_and_partners_at_events, update_dict_of_cadets_and_boat_classes_and_partners_at_events
from app.backend.registration_data.cadet_registration_data import add_new_cadet_to_event_from_row_in_registration_data
...
from app.data_access.configuration.field_list import CADET_FIRST_NAME, CADET_SURNAME, CADET_DOUBLE_HANDED_PARTNER
...
def from_partner_name_to_cadet(partner_name: str) -> Cadet:
    ...
    return Cadet.from_name_only(
        first_name=first_name, surname=second_name
    )
...
def get_registered_two_handed_partner_name_for_cadet_at_event(
    ...
    cadets_at_event_data = get_dict_of_cadets_with_registration_data(object_store=object_store, event=event)
...
def add_unregistered_partner_cadet(
    ...
    add_identified_cadet_and_row(
        object_store=object_store,
        event=event, row_id=new_row.row_id,
        cadet = new_cadet
    )
...
def add_new_row_to_raw_registration_data_and_return_row(
    ...
    update_raw_mapped_registration_data(object_store=object_store,
                                        event=event,
...
def get_list_of_days_given_original_cadet(
    ...
    if day_or_none_if_all_days is None:
        ...
        list_of_days = registration_data.registration_data_for_cadet(cadet=original_cadet).availability.days_available()
    else:
        ## single day
...
def add_two_handed_partnership_on_day_for_new_cadet_when_have_dinghy_for_existing_cadet(
    ...
    dinghys_data = get_dict_of_cadets_and_boat_classes_and_partners_at_events(object_store=object_store, event=event)
...
def add_new_cadet_to_group_on_day(
    ...
    cadets_at_event_data = get_dict_of_cadets_with_groups_at_event(object_store=object_store, event=event)
    group = cadets_at_event_data.get_days_and_groups_for_cadet(cadet=original_cadet).group_on_day(day)

    cadets_at_event_data.add_or_upate_group_for_cadet_on_day(cadet=new_cadet,
                                                             day=day,
                                                             group=group)

    update_dict_of_cadets_with_groups_at_event(object_store=object_store, event=event, dict_of_cadets_with_groups_at_event=cadets_at_event_data)
