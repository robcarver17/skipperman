...
from app.backend.events.event_warnings import get_list_of_all_warning_ids_at_event, \
    mark_event_warning_with_id_as_ignore, mark_all_active_event_warnings_with_priority_and_category_as_ignored, \
...
def display_warnings_tables(list_of_warnings: ListOfEventWarnings) -> ListOfLines:
    active_table = display_active_warnings_table(list_of_warnings.active_only(), detail_name="Active warnings", active=True)
    ignored_table = display_active_warnings_table(list_of_warnings.ignored_only(), detail_name="Ignored warnings",active=False)
...
def display_active_warnings_table(list_of_warnings: ListOfEventWarnings, detail_name: str, active: bool) -> Union[
...
def get_inside_table(list_of_warnings: ListOfEventWarnings, active:bool):
    ...
    for warning in list_of_warnings:
        if current_warning is None:
            ...
            list_of_rows.append(
                RowInTable(['', warning.category, get_button_for_multiple_warning(priority=warning.priority,
            ...
            if warning.category!=current_warning.category:
                list_of_rows.append(RowInTable(['', warning.category, get_button_for_multiple_warning(priority=warning.priority,
                                                                                                      category=warning.category,
                                                                                                      ignore_on=ignore_on), '']))
...
def get_button_for_multiple_warning(priority: str, category: str, ignore_on: bool):
    if ignore_on:
        label = "Set all active warnings of priority '%s', category '%s' to ignored" % (priority, category)
    else:
        label = "Set all currently ignored warnings of priority '%s', category '%s' to un-ignored" % (priority, category)

    return Button(label=label, value=get_button_value_for_multiple_warning(
        priority=priority,
        category=category,
        ignore_on=ignore_on
    ))
...
def get_button_value_for_multiple_warning(priority: str, category: str, ignore_on: bool):
    return get_button_value_given_type_and_attributes(WARNINGS_BUTTON_TYPE,
                                                      priority,
                                                      category,
                                                      from_bool_to_str(ignore_on))


def get_attributes_from_button_pressed(last_button: str):
    priority,category,ignore_on =get_attributes_from_button_pressed_of_known_type(last_button, type_to_check=WARNINGS_BUTTON_TYPE,collapse_singleton=False)
...
save_warnings_button = Button("Save changes to warnings",
                              value=get_button_value_for_multiple_warning(
                                  priority=PSEUDO_PRIORITY_FOR_SAVE_ALL_WARNINGS,
                                  category="",
                                  ignore_on=False
                              ))
...
def is_save_warnings_button_pressed(last_button:str):
    return is_button_of_type(last_button,
                             type_to_check=WARNINGS_BUTTON_TYPE)
...
def save_warnings_from_table(interface: abstractInterface):
    ...
    if was_specific_button_to_flag_warnings_pressed(last_button):
        save_multiple_warnings_given_specific_button_pressed(interface=interface, last_button=last_button)

def save_warnings_from_table_checkboxes(interface: abstractInterface, event:Event):
    list_of_ids = get_list_of_all_warning_ids_at_event(object_store=interface.object_store, event=event)
...
def save_multiple_warnings_given_specific_button_pressed(interface: abstractInterface, last_button:str):
    priority, category, ignore_on = get_attributes_from_button_pressed(last_button)
    event =get_event_from_state(interface)
    if ignore_on:
        mark_all_active_event_warnings_with_priority_and_category_as_ignored(object_store=interface.object_store,
                                                                             event=event,
                                                                             priority=priority,
                                                                             category=category)
    else:
        mark_all_ignored_event_warnings_with_priority_and_category_as_unignored(object_store=interface.object_store,
                                                                             event=event,
                                                                             priority=priority,
                                                                             category=category)