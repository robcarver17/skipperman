...
def interactively_modify_report_options_in_place(
    ...
    while still_choosing:
        ...
        ## change backend options in place
...
def report_options_as_dict_of_str(
    ...
    output_file_str = (
        "Output to path & file: %s" % report_options.print_options.filename
    )
...
def modify_a_single_reporting_option(
    ...
    if key_of_option_chosen == "landscape":
        ...
        report_options.print_options.filename = os.path.join(
            path_name, file_name
        )
        ...
        raise Exception(
            "Key %s not recognised as backend option" % key_of_option_chosen
        )
