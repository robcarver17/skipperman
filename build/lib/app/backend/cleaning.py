...
def clean_sensitive_data_for_event(interface: abstractInterface, event: Event):
    ...
    clean_sensitive_data_for_event_from_cadets_at_event_data(interface=interface, event=event)

def clean_sensitive_data_for_event_from_mapped_data(interface: abstractInterface, event: Event):
    mapped_wa_data = MappedEventsData(interface.data)
    mapped_wa_data.clear_mapped_event_data(event)

def clean_sensitive_data_for_event_from_cadets_at_event_data(interface: abstractInterface, event: Event):
