...
class Store(dict):
    ...
    def mark_stored_item_as_unchanged(self, key):
        ...
        storage_item = StorageItem(contents=storage_item.contents, data_access_method=storage_item.data_access_method,
                               changed=False)
        self.put_storage_item(storage_item)
    ...
    def _write_to_data(self, contents, data_access_method: DataAccessMethod):
        ...
        print("Writing changed to %s with %s" % (str(data_access_write_method), str(kwargs)))
