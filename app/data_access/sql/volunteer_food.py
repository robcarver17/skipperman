...
class SqlDataListOfVolunteersWithFoodRequirementsAtEvent(GenericSqlData):
    ...
    def add_new_volunteer_with_food_to_event(
        ...
        if self.is_volunteer_with_already_at_event_with_food(
    ...
    def update_volunteer_food_data(
        ...
        self.remove_food_requirements_for_volunteer_at_event(
            volunteer_id=volunteer_id, event_id=event_id
        )
    ...
    def read(self, event_id: str) -> ListOfVolunteersWithFoodRequirementsAtEvent:
        ...
        for raw_item in raw_list:
            ...
            food_requirements_this_volunteer = new_dict.get(
                volunteer_id,
                VolunteerWithFoodRequirementsAtEvent(
                    volunteer_id=volunteer_id, food_requirements=FoodRequirements()
                ),
            )

            if food_required_key == OTHER_KEY:
                food_requirements_this_volunteer.food_requirements.other = (
                    food_required_text_vale
                )
            else:
                setattr(
                    food_requirements_this_volunteer.food_requirements,
                    food_required_key,
    ...
    def write(
        ...
        try:
            ...
            self.cursor.execute(
                ...
                % (VOLUNTEER_FOOD_REQUIRED_TABLE, EVENT_ID, int(event_id))
            )

            for volunteer_with_food in list_of_volunteers_with_food:
                self._write_volunteer_with_food_without_commit_or_checks(
                    event_id=event_id, volunteer_with_food=volunteer_with_food
                )
            self.conn.commit()
        except Exception as e1:
            raise Exception("Error %s when writing food at event" % str(e1))
    ...
    def _write_volunteer_with_food_without_commit_or_checks(
        self, event_id: str, volunteer_with_food: VolunteerWithFoodRequirementsAtEvent
    ):
        ...
        for key, value in as_dict_not_other.items():
            ...
            self.cursor.execute(
                insertion, (int(event_id), volunteer_id, key, bool_value, text_value)
            )
    ...
    def create_table(self):
        ...
        index_creation_query = "CREATE UNIQUE INDEX %s ON %s (%s, %s, %s)" % (
            INDEX_VOLUNTEER_FOOD_REQUIRED_TABLE,
            VOLUNTEER_FOOD_REQUIRED_TABLE,
            EVENT_ID,
            VOLUNTEER_ID,
            FOOD_REQUIRED_KEY,
