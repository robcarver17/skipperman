...
from app.frontend.shared.add_edit_or_choose_volunteer_form import (
    ...
    availability_in_form_set_to_no,
)
...
class ParametersForGetOrSelectVolunteerForm:
    ...
    def was_extra_button_pressed(self, button_pressed: str):
        if self.extra_buttons is arg_not_passed:
            ...
            return check_if_button_in_list_was_pressed(
                last_button_pressed=button_pressed, list_of_buttons=self.extra_buttons
            )
    ...
    def get_values_from_state(self, interface: abstractInterface):
        self.sort_by = interface.get_persistent_value(
            SORT_BY_STATE, SORT_BY_NAME_SIMILARITY
        )
        self.final_add_button = interface.get_persistent_value(FINAL_ADD, False)
        self.see_all_volunteers = interface.get_persistent_value(
...
def get_volunteer_and_verification_text(
    interface: abstractInterface,
...
def get_footer_buttons_add_or_select_existing_volunteer_form(
    ...
    return ListOfLines(
        [main_buttons, extra_buttons, _______________] + volunteer_buttons
    )
...
def get_list_of_volunteer_buttons(
    ...
    list_of_volunteers = sort_list_of_volunteers(
        list_of_volunteers, sort_by=parameters.sort_by, similar_volunteer=volunteer
    )
    sort_order_buttons = get_sort_order_buttons(
        parameters=parameters, list_of_volunteers=list_of_volunteers
    )
    volunteer_choice_buttons = break_up_buttons(
        [Button(str(volunteer)) for volunteer in list_of_volunteers]
    )

    return ListOfLines(
        [
            _______________,
            Line([msg, state_button] + sort_order_buttons),
            _______________,
            volunteer_choice_buttons,
        ]
    ).add_Lines()


def get_sort_order_buttons(
    parameters: ParametersForGetOrSelectVolunteerForm, list_of_volunteers: list
):
    ...
    list_of_buttons = [sort_msg] + [
        Button(sort_label) for sort_label in possible_sort_labels
    ]
...
possible_sorts = [SORT_BY_FIRSTNAME, SORT_BY_SURNAME, SORT_BY_NAME_SIMILARITY]
...
def generic_post_response_to_add_or_select_volunteer(
    interface: abstractInterface, parameters: ParametersForGetOrSelectVolunteerForm
    ...
    if parameters.was_extra_button_pressed(last_button_pressed):
        ...
        return ResultFromAddOrSelectVolunteer(
            volunteer=volunteer, volunteer_was_added=True
        )
        ...
        return ResultFromAddOrSelectVolunteer(
            volunteer=volunteer,
            volunteer_was_added=False,
            no_availability=on_site_parent,
        )
...
def generic_post_response_to_add_or_select_volunteer_when_returning_new_form(
    ...
    if is_button_a_sort_button(last_button_pressed):
    ...
    elif see_similar_volunteers_button.pressed(last_button_pressed):
        ...
        last_button_pressed=last_button_pressed,
        list_of_buttons=[check_for_me_volunteer_button, check_confirm_volunteer_button],
    ):
        parameters.final_add_button = True
...
def get_existing_volunteer_selected_from_button(
    interface: abstractInterface,
) -> Volunteer:
...
def form_as_result(
    ...
    return ResultFromAddOrSelectVolunteer(form=form)
