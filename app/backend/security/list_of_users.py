...
def change_password_for_user(
    ...
    if interface is arg_not_passed:
        ...
        change_password_for_user_from_inside_SM(
            interface=interface, username=username, new_password_hash=password_hash
        )


def change_password_for_user_from_home_page(username: str, new_password_hash: str):
    object_store = default_object_store

    ## UNIQUELY READ ONLY CAN'T AFFECT PASSWORD CHANGES
    object_store.data_api.data_list_of_users.change_password_for_user(
...
def add_user(interface: abstractInterface, user: SkipperManUser):
    interface.update(
        interface.object_store.data_api.data_list_of_users.add_user, user=user
    )
