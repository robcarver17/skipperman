...
from app.objects_OLD.events import (
    Event,
    default_event,
...
def form_fields_for_add_event(
    ...
    dict_of_checked = dict(
        [
            (label, getattr(event, get_event_attribute_given_label(label)))
...
def get_event_from_form(interface) -> Event:
    ...
    add_list_of_contains_flags_to_event(
        event=event,
        list_of_contained_labels_set_to_true=list_of_contained_labels_set_to_true,
    )
...
def add_list_of_contains_flags_to_event(
    event: Event, list_of_contained_labels_set_to_true: list
):
