from app.backend.mapping.list_of_field_mappings import delete_mapping_given_skipperman_field, save_new_mapping_pairing
...
def add_skipperman_field_to_mapping(interface: abstractInterface):
    skipperman_field = interface.value_from_form(ADD_SKIPPERMAN_FIELD_DROPDOWN_NAME, default=ADD_SKIPPERMAN_FIELD_DROPDOWN_EMPTY)
    if skipperman_field == ADD_SKIPPERMAN_FIELD_DROPDOWN_EMPTY:
        clear_current_skipperman_field_to_add_from_state(interface)
        return

    save_current_skipperman_field_to_add_from_state(interface=interface, skipperman_field=skipperman_field)
...
def add_WA_and_skipperman_field_to_mapping(interface: abstractInterface):
    ...
    if not valid_inputs_for_mapping(skipperman_field=skipperman_field,
                                    wa_field=wa_field):
        clear_current_skipperman_field_to_add_from_state(interface)
        return

    try:
        save_new_mapping_pairing(object_store=interface.object_store, event=event, skipperman_field=skipperman_field, wa_field=wa_field)
    except Exception as e:
        interface.log_error("Cannot add mapping pair %s/%s because %s" % (skipperman_field, wa_field, str(e)))

    clear_current_skipperman_field_to_add_from_state(interface)
    interface.flush_cache_to_store()

def get_wa_field_mapping_from_form(interface: abstractInterface):
    wa_field_text_input = interface.value_from_form(ADD_WA_FIELD_INPUT_FIELD_NAME, default='')
    wa_field_dropdown = interface.value_from_form(ADD_WA_FIELD_DROPDOWN_NAME, default = ADD_WA_FIELD_DROPDOWN_EMPTY)
...
def valid_inputs_for_mapping(skipperman_field: str,
                             wa_field: str):
...
def delete_mapping(interface: abstractInterface):
    event = get_event_from_state(interface)
    skipperman_field = get_skipperman_field_from_delete_button(interface.last_button_pressed())
    try:
        delete_mapping_given_skipperman_field(object_store=interface.object_store, event=event, skipperman_field=skipperman_field)
    except Exception as e:
        interface.log_error("Error deleting mapping for %s, %s" % (skipperman_field, str(e)))
    interface.flush_cache_to_store()
