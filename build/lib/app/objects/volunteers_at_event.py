...
class ListOfIdentifiedVolunteersAtEvent(GenericListOfObjects):
    ...
    def list_of_identified_volunteers_with_volunteer_id(self, volunteer_id: str) -> 'ListOfIdentifiedVolunteersAtEvent':
        items = [item for item in self if item.volunteer_id ==volunteer_id if item.is_allocated]
...
class VolunteerAtEventWithId(GenericSkipperManObject):
    ...
    def from_dict(cls, dict_with_str):
        ...
        if len(list_of_cadet_ids_as_str)==0:
            ...
            list_of_cadet_ids = [str(int(float(cadet_id))) for cadet_id in list_of_cadet_ids]
...
class VolunteerAtEvent(GenericSkipperManObject):
    ...
    @classmethod
    def from_volunteer_and_voluteer_at_event_with_id(cls, volunteer: Volunteer, volunteer_at_event_with_id = VolunteerAtEventWithId):
        return cls(
            volunteer=volunteer,
...
class ListOfVolunteersAtEventWithId(GenericListOfObjects):
    ...
    def list_of_volunteers_associated_with_cadet_id(self, cadet_id: str):
        return ListOfVolunteersAtEventWithId([volunteer_at_event for volunteer_at_event in self if cadet_id in volunteer_at_event.list_of_associated_cadet_id])
    ...
    def add_volunteer_with_just_id(self, volunteer_id: str, availability: DaySelector):
        if volunteer_id in self.list_of_volunteer_ids:
            return
        new_volunteer = VolunteerAtEventWithId(volunteer_id=volunteer_id, availablity=availability,
    ...
    def sort_by_list_of_volunteer_ids(self, list_of_ids) -> 'ListOfVolunteersAtEventWithId':
    ...
    def list_of_volunteers_available_on_given_day(self, day: Day) -> 'ListOfVolunteersAtEventWithId':
        new_list_of_volunteers_at_event = [volunteer for volunteer in self if volunteer.available_on_day(day)]
        return ListOfVolunteersAtEventWithId(new_list_of_volunteers_at_event)

def add_cadet_association_to_existing_volunteer(existing_volunteer_at_event: VolunteerAtEventWithId, new_volunteer_at_event: VolunteerAtEventWithId):
    try:
...
def add_cadet_association_to_existing_volunteer_with_cadet_id(existing_volunteer_at_event: VolunteerAtEventWithId, cadet_id: str):
