...
def html_button(button_text, button_name=HTML_BUTTON_NAME):
    return Html('<input type="submit" name="%s" value="%s" />' % (button_name, button_text))

def html_form_text_input(input_label: str, input_name: str, value:str = arg_not_passed):
    if value is not arg_not_passed:
        value_html = 'value="%s"' % value
    else:
        value_html=""

    return Html('%s: <input type="text" name="%s" %s />' % (input_label, input_name, value_html))

def html_date_input(input_label: str, input_name: str,
    ...
    return Html('%s: <input type="date" name="%s" %s %s %s/>' %
...
def html_radio_input(input_label: str, input_name: str, dict_of_options: dict,
                     default_label: str = DEFAULT_LABEL):
    options_str_as_list = [
        html_single_radio_button(input_name=input_name,
...
def html_single_radio_button(input_name: str, option_label: str, option_value: str,
                             default_label: str = DEFAULT_LABEL):
