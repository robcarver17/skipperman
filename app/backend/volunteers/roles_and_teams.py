...
from app.data_access.store.object_definitions import object_definition_for_list_of_teams, \
    object_definition_for_list_of_roles_with_skill_ids, \
    object_definition_for_list_of_roles_with_skills, object_definition_for_dict_of_teams_with_roles
...
def reorder_roles_for_team_given_list_of_names(object_store : ObjectStore,
                                               new_order_of_role_names: List[str],
                                               team: Team):

    dict_of_teams_and_role = get_dict_of_teams_and_roles(object_store)
    dict_of_teams_and_role.reorder_roles_for_team_given_list_of_names(team=team, new_order_of_role_names=new_order_of_role_names)
    update_dict_of_teams_and_roles(object_store=object_store, dict_of_teams_and_roles=dict_of_teams_and_role)

def add_new_named_role_to_team(object_store : ObjectStore,
                               team: Team,
                               new_role_name:str):

    dict_of_teams_and_role = get_dict_of_teams_and_roles(object_store)
    dict_of_teams_and_role.add_new_named_role_to_team(team=team, new_role_name=new_role_name)
    update_dict_of_teams_and_roles(object_store=object_store, dict_of_teams_and_roles=dict_of_teams_and_role)
...
def modify_list_of_roles_with_skills(object_store: ObjectStore, existing_object: RoleWithSkills, new_object: RoleWithSkills):
    ...
    try:
        ...
        update_list_of_roles_with_skills(object_store=object_store, list_of_roles_with_skills=list_of_roles_with_skills)
    except:
        raise Exception("One or more duplicated role names in list - role names must be unique")

def add_to_list_of_roles_with_skills(object_store: ObjectStore,  name_of_entry_to_add: str):
    list_of_roles_with_skills = get_list_of_roles_with_skills(object_store)
...
def update_list_of_roles_with_skills(
    ...
    object_store.update(list_of_roles_with_skills, object_definition=object_definition_for_list_of_roles_with_skills)


def get_list_of_roles_with_skills(
...
def modify_team(
    object_store: ObjectStore, existing_object: Team, new_object: Team
):
...
def update_list_of_teams(object_store: ObjectStore, list_of_teams: ListOfTeams):
    object_store.update(new_object=list_of_teams, object_definition=object_definition_for_list_of_teams)
...
def update_list_of_roles(object_store: ObjectStore, list_of_roles: ListOfRolesWithSkillIds):
    object_store.update(new_object=list_of_roles, object_definition=object_definition_for_list_of_roles_with_skill_ids)
...
def update_dict_of_teams_and_roles(object_store: ObjectStore, dict_of_teams_and_roles: DictOfTeamsWithRoles):
    object_store.update(new_object=dict_of_teams_and_roles, object_definition=object_definition_for_dict_of_teams_with_roles)
