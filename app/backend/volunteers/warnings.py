...
def warn_about_volunteer_availablity_at_event_with_connected_cadets(
    ...
    warnings = add_notes_to_warnings_on_availability(
        volunteer=volunteer,
        volunteer_registration_data=volunteer_registration_data,
        warnings=warnings,
    )
...
def warning_about_volunteer_availability_on_specific_day(
    ...
    if missing_volunteers_if_true_otherwise_missing_cadets:
        if not volunteer_available_on_day and any_cadet_available_on_day:
            new_warning = warning_if_cadet_present_and_no_volunteers(
                list_of_cadets_available_on_day=list_of_cadets_available_on_day, day=day
            )
            warnings.append(new_warning)
    else:
        if volunteer_available_on_day and not any_cadet_available_on_day:
            new_warning = warning_if_volunteer_present_and_not_cadet(
                active_connected_cadets=active_connected_cadets,
...
def warning_if_volunteer_present_and_not_cadet(
    active_connected_cadets: ListOfCadets,
    list_of_cadets_available_on_day: ListOfCadets,
    day: Day,
) -> str:
    missing_cadet_names = ", ".join(
        get_cadet_names_who_are_connected_but_not_available_on_day(
            active_connected_cadets=active_connected_cadets,
            list_of_cadets_available_on_day=list_of_cadets_available_on_day,
        )
    )
    return "On %s volunteer attending; but associated sailors %s are not attending" % (
        day.name.upper(),
        missing_cadet_names,
    )


def get_cadet_names_who_are_connected_but_not_available_on_day(
    active_connected_cadets: ListOfCadets,
    list_of_cadets_available_on_day: ListOfCadets,
):
    missing_cadet_names = [
...
def warning_if_cadet_present_and_no_volunteers(
    list_of_cadets_available_on_day: ListOfCadets, day: Day
) -> str:
    names_of_attending_cadets = [
        cadet.name for cadet in list_of_cadets_available_on_day
    ]
    joined_names = ", ".join(names_of_attending_cadets)

    return "On %s, sailors are attending but volunteer is not: %s" % (
        day.name.upper(),
        joined_names,
    )


def add_notes_to_warnings_on_availability(
    volunteer: Volunteer,
    volunteer_registration_data: RegistrationDataForVolunteerAtEvent,
    warnings: List[str],
) -> str:
...
def warning_for_specific_temporary_skip_volunteer_id_at_event(
    object_store: ObjectStore, event: Event, row_id_and_index: RowIDAndIndex
) -> str:
...
def get_all_saved_warnings_for_volunteer_rota(
    object_store: ObjectStore, event: Event
) -> ListOfEventWarnings:
    all_warnings = (
        get_list_of_warnings_at_event_for_categories_sorted_by_category_and_priority(
