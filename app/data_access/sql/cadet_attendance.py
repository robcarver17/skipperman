...
class SqlDataAttendanceAtEventsForSpecificCadet(GenericSqlData):
    ...
    def delete_attendance_data_for_cadet(self, cadet: Cadet):
        try:
            ...
            self.cursor.execute(
                ...
                % (CADET_ATTENDANCE_TABLE, CADET_ID, int(cadet.id))
            )
    ...
    def delete_attendance_data_at_event(self, event: Event):
        try:
            ...
            self.cursor.execute(
                "DELETE FROM %s WHERE %s=%d "
                % (CADET_ATTENDANCE_TABLE, EVENT_ID, int(event.id))
            )
    ...
    def update_attendance_for_cadet_on_day_at_event(
        self, cadet_id: str, raw_attendance: RawAttendanceItem
    ):
        try:
            if self.table_does_not_exist(CADET_ATTENDANCE_TABLE):
                self.create_table()
    ...
    def get_attendance_at_event_for_list_of_cadets(
        ...
        return AttendanceAtEventAcrossCadets(
            [
                (cadet, self.get_attendance_dict_for_cadet(cadet=cadet, event=event))
                for cadet in list_of_cadets
            ]
        )

    def get_attendance_dict_for_cadet(
        self, cadet: Cadet, event: Event
    ) -> AttendanceAcrossDays:
        raw_list = self.get_raw_attendance_at_event_for_cadet(
            cadet_id=cadet.id, event_id=event.id
        )
        list_of_days = raw_list.list_of_days()
        return AttendanceAcrossDays(
            dict(
                [
                    (
                        day,
                        AttendanceOnDay.create_from_subset_of_list_of_attendance(
                            list_of_attendance=raw_list, day=day
                        ),
                    )
    ...
    def write(
        ...
        try:
            ...
            self.cursor.execute(
                ...
                % (CADET_ATTENDANCE_TABLE, CADET_ID, int(cadet_id))
            )

            for attendance in list_of_attendance:
                self._add_row_without_checks_or_commits(
                    cadet_id=cadet_id, attendance=attendance
                )
            self.conn.commit()
        except Exception as e1:
            raise Exception("Error %s when writing attendance" % str(e1))
    ...
    def _add_row_without_checks_or_commits(
        self, cadet_id: str, attendance: RawAttendanceItem
    ):
