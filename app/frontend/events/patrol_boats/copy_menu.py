...
from app.frontend.events.patrol_boats.copying import copy_across_all_boats, copy_across_all_boats_and_roles, \
    overwrite_allocation_across_all_boats, overwrite_copy_across_all_boats_and_roles
...
def display_form_patrol_boat_copy_menu(interface: abstractInterface):
    ...
    contents_of_form = ListOfLines(
        [
            navbar,
            _______________,
            Heading("Copying volunteer patrol boats and optionally roles for %s " % event.event_name, centred=False),
            _______________,
...
def post_form_patrol_boat_copy_menu(interface: abstractInterface) -> Union[Form, NewForm]:
    ...
    return interface.get_new_display_form_for_parent_of_function(display_form_patrol_boat_copy_menu)



COPY_ALL_BOATS_BUTTON_LABEL = "Copy and fill any empty boat allocations from earliest day when allocated"
