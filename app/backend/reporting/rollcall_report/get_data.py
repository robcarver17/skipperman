...
def get_dict_of_df_for_reporting_rollcalls_with_flags(
    ...
    for group in list_of_groups:
        if group is GROUP_UNALLOCATED:
            list_of_cadet_ids_with_groups = group_allocations_data.active_cadet_ids_at_event_for_unallocated_cadets(event=event)
        else:
            list_of_cadet_ids_with_groups = group_allocations_data.active_cadet_ids_with_groups_for_group_at_event(event=event, group=group)

        list_of_cadets_with_groups = group_allocations_data.get_list_of_cadets_with_group_given_list_of_cadets_with_ids_and_groups(list_of_cadet_ids_with_groups=list_of_cadet_ids_with_groups)

        if add_asterix_for_club_boats:
            list_of_cadets_with_groups = add_club_boat_asterix(interface=interface,
                                                                  list_of_cadets_with_groups=list_of_cadets_with_groups, event=event)
        ...
        df = pd.concat([df, pd.DataFrame(list_of_cadet_ids_this_group, columns=[ID])], axis=1)
