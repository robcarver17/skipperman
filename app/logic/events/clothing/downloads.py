...
def export_committee_clothing(interface: abstractInterface) -> File:
    ...
    list_of_cadets_with_clothing = get_list_of_active_cadet_objects_with_clothing_at_event(interface=interface, event=event, only_committee=True)
    new_list = []
...
def export_clothing_colours(interface: abstractInterface) -> File:
    event = get_event_from_state(interface)
    list_of_cadets_with_clothing = get_list_of_active_cadet_objects_with_clothing_at_event(interface=interface, event=event, only_committee=False)
    list_of_cadets_with_clothing_committee = get_list_of_active_cadet_objects_with_clothing_at_event(interface=interface, event=event, only_committee=True)
    colour_dict = {}
    for colour in list_of_cadets_with_clothing.get_colour_options():
        list_this_colour = list_of_cadets_with_clothing.filter_for_colour(colour)
        list_this_colour_committee = list_of_cadets_with_clothing_committee.filter_for_colour(colour)

        list_this_colour_committee = list_this_colour_committee.sort_by_dob_asc()

        list_this_colour = list_this_colour.remove_if_in_list_of_cadet_ids(list_this_colour_committee.list_of_cadet_ids())
        list_this_colour = list_this_colour.sort_by_firstname()

        list_of_cadets_with_clothing_this_colour = list_this_colour_committee+list_this_colour
        list_of_names = [object.cadet.name for object in list_of_cadets_with_clothing_this_colour]
