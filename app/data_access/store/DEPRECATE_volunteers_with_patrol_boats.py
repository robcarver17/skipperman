...
from app.data_access.store.DEPRECATE_volunteers_at_event_with_roles import (
    VolunteersAtEventWithRolesData,
)
...
class PatrolBoatData:
    ...
    def get_list_of_volunteers_at_event_with_patrol_boat_and_role(
        self, event: Event
    ) -> ListOfVolunteersAtEventWithSkillsAndRolesAndPatrolBoats:
        all_volunteers_at_event = (
            self.get_list_of_volunteers_at_event_with_skills_and_roles(event)
        )
        ...
        list_of_volunteers_at_event_with_skills_patrol_boat_and_role = [
            self.get_volunteers_at_event_with_skills_patrol_boat_and_role_on_day_for_specific_volunteer(
                volunteer_at_event_with_skills_and_roles
            )
            for volunteer_at_event_with_skills_and_roles in all_volunteers_at_event
        ]
    ...
    def get_volunteers_at_event_with_skills_patrol_boat_and_role_on_day_for_specific_volunteer(
        self,
        volunteer_at_event_with_skills_and_roles: VolunteerAtEventWithSkillsAndRoles,
    ) -> VolunteerAtEventWithSkillsAndRolesAndPatrolBoats:
    ...
    def get_patrol_boat_by_day_dict_for_volunteer_at_event(
        self,
        volunteer_at_event_with_skills_and_roles: VolunteerAtEventWithSkillsAndRoles,
    ) -> PatrolBoatByDayDict_DONOTUSE:
        days_attending = (
            volunteer_at_event_with_skills_and_roles.volunteer_event_data.availablity
        )
        dict_of_boats_by_day = dict(
            [
                (
                    day,
                    self.get_patrol_boat_on_day_for_volunteer_at_event(
                        volunteer_at_event_with_skills_and_roles=volunteer_at_event_with_skills_and_roles,
                        day=day,
                    ),
                )
        ...
        dict_of_boats_by_day_as_list = [
            (day, patrol_boat)
            for day, patrol_boat in dict_of_boats_by_day.items()
            if not patrol_boat.is_empty
        ]

        return PatrolBoatByDayDict_DONOTUSE(dict_of_boats_by_day_as_list)

    def get_patrol_boat_on_day_for_volunteer_at_event(
        self,
        volunteer_at_event_with_skills_and_roles: VolunteerAtEventWithSkillsAndRoles,
        day: Day,
    ) -> PatrolBoat:
        list_of_voluteers_at_event_with_patrol_boats = self.get_list_of_voluteers_at_event_with_patrol_boats(
            event=volunteer_at_event_with_skills_and_roles.volunteer_event_data.event
        )
        try:
            boat_id = list_of_voluteers_at_event_with_patrol_boats.which_boat_id_is_volunteer_on_today(
                volunteer_id=volunteer_at_event_with_skills_and_roles.volunteer.id,
                day=day,
            )
        except MissingData:
    ...
    def get_list_of_volunteers_at_event_with_skills_and_roles(
        ...
        return self.cache.get_from_cache(
            get_list_of_volunteers_at_event_with_skills_and_roles, event=event
        )
    ...
    def get_list_of_voluteers_at_event_with_patrol_boats(
        ...
        return self.data_api.get_list_of_voluteers_at_event_with_patrol_boats(
            event=event
        )
...
def get_list_of_volunteers_at_event_with_skills_and_roles(
    ...
    return volunteers_with_roles_at_event_data.get_list_of_volunteers_at_event_with_skills_and_roles(
        event
    )
