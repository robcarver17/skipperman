...
def update_if_copy_previous_role_button_pressed(
    ...
    event = get_event_from_state(interface)
    previous_role_and_group = DEPRECATE_get_last_role_for_volunteer_id(
        interface=interface, volunteer_id=volunteer_id, avoid_event=event
    )
...
def save_volunteer_matrix_and_return_filename(interface: abstractInterface) -> str:
    ...
    volunteer_matrix = get_volunteer_matrix(
        data_layer=interface.data, event=event, sorts_and_filters=sorts_and_filters
    )
