...
from app.objects.utilities.transform_data import str_from_list, list_from_str
...
def get_checkbox_for_event_selection(interface: abstractInterface, event: Event):
    picklist = get_picklist_of_all_events_excluding_current(
...
def get_prior_events_to_show(
    ...
    if event_id_selection is missing_data:
        ...
        return get_list_of_events_given_selection(
            object_store=interface.object_store,
            event=event,
            event_id_selection=event_id_selection,
        )


def get_list_of_events_given_selection(
    object_store: ObjectStore, event: Event, event_id_selection: List[str]
) -> ListOfEvents:
    all_events = get_list_of_events(object_store)
    list_of_events = [
        all_events.event_with_id(event_id)
        for event_id in event_id_selection
        if not event.id == event_id
    ]
...
def get_picklist_of_all_events_excluding_current(
...
def save_event_selection_from_form_if_saved_button_pressed(
    interface: abstractInterface,
):
    picklist = get_list_of_events(interface.object_store)
    event_names_chosen = interface.value_of_multiple_options_from_form(
        event_selection_form_value, default=[]
    )
    if len(event_names_chosen) == 0:
        interface.log_error(
            "Need to select at least one previous event to show. Defaulting to last %d events"
            % DEFAULT_EVENT_COUNT
        )
        clear_prior_event_selection_in_state(interface)
        return
    ...
    store_prior_event_selection_in_state(
        interface=interface, list_of_event_ids=new_list_of_ids
    )
...
def store_prior_event_selection_in_state(
    ...
    as_str = str_from_list(list_of_event_ids)
...
revert_to_default_event_button = Button(
    "Use default (last %d events)" % DEFAULT_EVENT_COUNT,
    get_button_value_given_type_and_attributes(button_type, "revertToDefault"),
)


save_changes_event_button = Button(
    "Save changes to event selection",
    get_button_value_given_type_and_attributes(button_type, "SaveChanges"),
)
