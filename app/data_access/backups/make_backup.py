...
def remove_oldest_backup_if_too_old(backup_data_path):
    ...
    if (
        oldest + 2
    ) > NUMBER_OF_BACKUPS:  ## plus 1 because zero indexing, plus another so we end with the right number
...
def delete_backup_number(backup_number: int, backup_data_path: str):
    backup_dir = get_backup_directory(
        datapath=backup_data_path, backup_number=backup_number
    )
...
def make_first_backup(backup_data_path: str, master_data_path: str):
    backup_directory_for_this_backup = get_backup_directory(
        backup_number=0, datapath=backup_data_path
    )
    try:
