...
mixed_api = MixedSqlAndCsvDataApi(
    master_data_path=master_data_path,
    backup_data_path=backup_data_path,
    user_data_path=user_data_path,
)
db_connection = mixed_api.db_connection
object_store = ObjectStore(data_api=mixed_api)
mixed_api.add_object_store(object_store)

sql_persistent_groups = SqlDataListOfGroupNamesForEventsAndCadetPersistentVersion(
    db_connection, object_store
)
sql_cadets = SqlDataListOfCadets(db_connection, object_store)
sql_groups = SqlDataListOfGroups(db_connection, object_store)
sql_groups_at_events = SqlDataListOfCadetsWithGroups(db_connection, object_store)
sql_list_of_dinghies = SqlDataListOfDinghies(db_connection, object_store)
sql_cadets_and_dinghies_at_event = SqlDataListOfCadetAtEventWithDinghies(
    db_connection, object_store
)
sql_qualifications = SqlDataListOfQualifications(db_connection, object_store)
sql_cadets_with_qualifications = SqlListOfCadetsWithQualifications(
    db_connection, object_store
)
sql_cadets_at_event = SqlDataListOfCadetsAtEvent(db_connection, object_store)
sql_cadets_on_committee = SqlDataListOfCadetsOnCommitte(db_connection, object_store)
sql_events = SqlDataListOfEvents(db_connection, object_store)
sql_list_of_volunteers = SqlDataListOfVolunteers(db_connection, object_store)
sql_associations = SqlDataListOfCadetVolunteerAssociations(db_connection, object_store)
sql_skills = SqlDataListOfSkills(db_connection, object_store)
sql_volunteers_with_skills = SqlDataListOfVolunteerSkills(db_connection, object_store)
sql_patrol_boats = SqlDataListOfPatrolBoats(db_connection, object_store)
sql_club_dinghies = SqlDataListOfClubDinghies(db_connection, object_store)
sql_roles = SqlDataListOfRoles(db_connection, object_store)
sql_teams = SqlDataListOfTeams(db_connection, object_store)
sql_volunteers_with_roles = SqlDataListOfVolunteersInRolesAtEvent(
    db_connection, object_store
)
sql_teams_and_roles = SqlDataListOfTeamsAndRolesWithIds(db_connection, object_store)
sql_club_dinghies_with_cadets = SqlDataListOfCadetAtEventWithClubDinghies(
    db_connection, object_store
)
sql_club_dinghies_with_volunteers = SqlDataListOfVolunteersAtEventWithClubDinghies(
    db_connection, object_store
)
sql_event_mappings = SqlDataWAEventMapping(db_connection, object_store)
sql_field_mappings = SqlDataWAFieldMapping(db_connection, object_store)
sql_template_field_mappings = SqlDataWAFieldMappingTemplates(
    db_connection, object_store
)
sql_mapped_registration_data = SqlDataMappedRegistrationData(
    db_connection, object_store
)
sql_event_warnings = SqlDataListOfEventWarnings(db_connection, object_store)
sql_identified_cadets_at_event = SqlDataListOfIdentifiedCadetsAtEvent(
    db_connection, object_store
)
sql_print_options = sqlDataListOfPrintOptions(db_connection, object_store)
sql_arrangement_options = SqlDataListOfArrangementOptions(db_connection, object_store)
sql_list_of_volunteers_at_event = SqlDataListOfVolunteersAtEvent(
    db_connection, object_store
)
sql_identified_volunteers_at_event = SqlDataListOfIdentifiedVolunteersAtEvent(
    db_connection, object_store
)
sql_club_boat_limits = SqlDataListOfClubDinghyLimits(db_connection, object_store)
sql_volunteers_with_patrol_boats = SqlDataListOfVolunteersAtEventWithPatrolBoats(
    db_connection, object_store
)
sql_substages = SqlDataListOfTickSubStages(db_connection, object_store)
sql_tick_sheet_items = SqlDataListOfTickSheetItems(db_connection, object_store)
sql_targets_at_event = SqlDataListOfTargetForRoleAtEvent(db_connection, object_store)
sql_list_of_cadets_with_food_requirements = (
    SqlDataListOfCadetsWithFoodRequirementsAtEvent(db_connection, object_store)
)
sql_list_of_volunteers_with_food_requirements = (
    SqlDataListOfVolunteersWithFoodRequirementsAtEvent(db_connection, object_store)
)
sql_clothing = SqlDataListOfCadetsWithClothingAtEvent(db_connection, object_store)
sql_group_notes = SqlDataListOfGroupNotesAtEvent(db_connection, object_store)
sql_notes = SqlDataListOfNotes(db_connection, object_store)
sql_patrol_boat_labels = SqlDataListOfPatrolBoatLabelsAtEvent(
    db_connection, object_store
)
sql_last_volunteer_roles = SqlDataListOfLastRolesAcrossEventsForVolunteers(
    db_connection, object_store
)
sql_attendance = SqlDataAttendanceAtEventsForSpecificCadet(db_connection, object_store)
sql_ticks = SqlDataListOfCadetsWithTickListItems(db_connection, object_store)
...
def transfer_from_csv_to_sql():
    ...
    print(csv_api.data_list_of_tick_sheet_items.read())
    sql_tick_sheet_items.write(csv_api.data_list_of_tick_sheet_items.read())
    print(csv_api.data_list_of_tick_sub_stages.read())
...
def transfer_from_sql_to_csv():
    ...
    for event in events:
        ...
        csv_api.data_list_of_cadets_with_groups.write(
            list_of_cadets_with_groups, event_id
        )
        csv_api.data_list_of_cadets_with_dinghies_at_event.write(
            sql_cadets_and_dinghies_at_event.read(event_id), event_id=event_id
        )
        csv_api.data_list_of_volunteers_in_roles_at_event.write(
            sql_volunteers_with_roles.read(event_id), event_id=event_id
        )
        csv_api.data_wa_field_mapping.write(
            sql_field_mappings.read(event_id), event_id=event_id
        )
        csv_api.data_event_warnings.write(
            sql_event_warnings.read(event_id), event_id=event_id
        )
        csv_api.data_identified_cadets_at_event.write(
            sql_identified_cadets_at_event.read(event_id), event_id=event_id
        )
        csv_api.data_list_of_volunteers_at_event.write(
            sql_list_of_volunteers_at_event.read(event_id), event_id=event_id
        )
        csv_api.data_list_of_identified_volunteers_at_event.write(
            sql_identified_volunteers_at_event.read(event_id), event_id=event_id
        )
        csv_api.data_list_of_volunteers_at_event_with_patrol_boats.write(
            sql_volunteers_with_patrol_boats.read(event_id=event_id), event_id=event_id
        )
        csv_api.data_list_of_targets_for_role_at_event.write(
            sql_targets_at_event.read(event_id), event_id=event_id
        )
        csv_api.data_list_of_cadets_with_food_requirement_at_event.write(
            sql_list_of_cadets_with_food_requirements.read(event_id=event_id),
            event_id=event_id,
        )
        csv_api.data_list_of_volunteers_with_food_requirement_at_event.write(
            sql_list_of_volunteers_with_food_requirements.read(event_id),
            event_id=event_id,
        )
        csv_api.data_list_of_cadets_with_clothing_at_event.write(
            sql_clothing.read(event_id=event_id), event_id=event_id
        )
    ...
    for cadet in list_of_cadets:
        csv_api.data_attendance_at_events_for_specific_cadet.write(
            sql_attendance.read(cadet.id), cadet_id=cadet.id
        )
        csv_api.data_list_of_cadets_with_tick_list_items.write(
            sql_ticks.read(cadet.id), cadet_id=cadet.id
        )
    ...
    csv_api.data_list_of_group_names_for_events_and_cadets_persistent_version.write(
        sql_persistent_groups.read()
    )
    ...
    csv_api.data_list_of_cadets_with_qualifications.write(
        sql_cadets_with_qualifications.read()
    )
    ...
    csv_api.data_list_of_last_roles_across_events_for_volunteers.write(
        sql_last_volunteer_roles.read()
    )
    ...
    for report_name in [
        ...
        "Patrol boat report",
        "Rollcall report",
        "Sailors with boats report",
        "Volunteer rota report",
    ]:
        csv_api.data_arrangement_and_group_order_options.write(
            sql_arrangement_options.read(report_name), report_name=report_name
        )
        for default in ["", "_default"]:
            full_name = "%s%s" % (report_name, default)
            csv_api.data_print_options.write(
                sql_print_options.read(full_name), report_name=full_name
            )

    list_of_templates = sql_template_field_mappings.list_of_template_names()
    for template_name in list_of_templates:
        csv_api.data_wa_field_mapping_templates.write(
            sql_template_field_mappings.read(template_name), template_name=template_name
        )
