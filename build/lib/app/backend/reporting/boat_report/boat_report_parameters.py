...
def get_specific_parameters_for_boat_report(
    object_store: ObjectStore, event: Event = arg_not_passed
) -> SpecificParametersForTypeOfReport:
    ...
    if event is arg_not_passed:
        ...
        group_annotations = get_dict_of_group_annotations(
            object_store=object_store, event=event, list_of_groups=list_of_groups
        )
...
def get_dict_of_group_annotations(
    object_store: ObjectStore, event: Event, list_of_groups: ListOfGroups
) -> GroupAnnotations:
    list_of_roles = get_list_of_instructor_type_roles_at_event_sorted_by_seniority(
        object_store, event
    )
