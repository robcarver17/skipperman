...
from app.objects.composed.volunteers_at_event_with_patrol_boats import DictOfVolunteersAtEventWithPatrolBoatsByDay, PatrolBoatByDayDict
...
class SqlDataListOfVolunteersAtEventWithPatrolBoats(
    ...
    def delete_volunteer_from_patrol_boat_on_day_at_event(
        ...
        try:
            ...
            self.cursor.execute("DELETE FROM %s WHERE %s=%d AND %s=%d AND %s='%s' " % (PATROL_BOATS_AND_VOLUNTEERS_TABLE,
                                                                  EVENT_ID,
                                                                  int(event_id),
                                                                VOLUNTEER_ID,
                                                                int(volunteer_id),
    ...
    def remove_patrol_boat_and_all_associated_volunteers_from_event(
        ...
        try:
            ...
            self.cursor.execute("DELETE FROM %s WHERE %s=%d AND %s=%d  " % (PATROL_BOATS_AND_VOLUNTEERS_TABLE,
                                                                  EVENT_ID,
                                                                  int(event_id),
                                                                PATROL_BOAT_ID,
                                                                int(patrol_boat_id)))
    ...
    def add_new_boat_day_volunteer_allocation(self,
            event_id: str,
            patrol_boat_id: str,
            day: Day,
            volunteer_id: str,
        ...
        existing_boat_id = self.existing_boat_id_for_volunteer_on_day(event_id=event_id, day=day, volunteer_id=volunteer_id, default=missing_data)
        if existing_boat_id is missing_data:
            self._add_boat_id_for_volunteer_on_day_without_checks(event_id=event_id, day=day, patrol_boat_id=patrol_boat_id, volunteer_id=volunteer_id)
        else:
            if existing_boat_id == patrol_boat_id:
                return
            raise Exception("volunteer already on boat that day")

    def is_boat_allocated_to_an_id_for_volunteer_on_day(self,
            event_id: str,
            day: Day,
            volunteer_id: str,
        ):
    ...
    def existing_boat_id_for_volunteer_on_day(self,
            event_id: str,
            day: Day,
            volunteer_id: str,
            default = missing_data
        ...
        try:
            cursor = self.cursor
            cursor.execute('''SELECT %s  FROM %s WHERE %s=%d AND %s=%d AND %s='%s' ''' % (
                PATROL_BOAT_ID,
    ...
    def _add_boat_id_for_volunteer_on_day_without_checks(self,
                                                         event_id: str,
                                                         day: Day,
                                                         volunteer_id: str,
                                                         patrol_boat_id: str
        ...
        try:
            if self.table_does_not_exist(PATROL_BOATS_AND_VOLUNTEERS_TABLE):
                ...
                self._add_volunteer_with_boat_without_commit_or_checks(event_id=event_id, volunteer_and_boat=volunteer_and_boat)
            self.conn.commit()
        except Exception as e1:
            raise Exception("Error %s when writing volunteers on patrol boats at event" % str(e1))
        finally:
    ...
    def delete_boat_id_for_volunteer_on_day(self,
            event_id: str,
            day: Day,
            volunteer_id: str,
        ):
    ...
    def is_boat_empty_on_day(self,
        ...
        try:
            cursor = self.cursor
            cursor.execute('''SELECT *  FROM %s WHERE %s=%d AND %s=%s AND %s=%d AND %s<>%d ''' % (
                PATROL_BOATS_AND_VOLUNTEERS_TABLE,
                EVENT_ID, int(event_id),
                DAY, day,
                PATROL_BOAT_ID, int(patrol_boat_id),
    ...
    def get_dict_of_patrol_boats_by_day_for_volunteer_at_event(self, event_id: str, ignore_empty: bool = False)-> DictOfVolunteersAtEventWithPatrolBoatsByDay:
        ...
        for raw_item in raw_list:
            ...
            dict_this_volunteer[raw_item.day] = self.list_of_patrol_boats.boat_given_id(raw_item.patrol_boat_id)
    ...
    def write(
        ...
        try:
            ...
            self.cursor.execute("DELETE FROM %s WHERE %s=%d" % (PATROL_BOATS_AND_VOLUNTEERS_TABLE,
                                                                  EVENT_ID,
                                                                  int(event_id)))

            for volunteer_and_boat in people_and_boats:
                self._add_volunteer_with_boat_without_commit_or_checks(event_id=event_id,
                                                                       volunteer_and_boat=volunteer_and_boat)
            self.conn.commit()
        except Exception as e1:
            raise Exception("Error %s when writing volunteers on patrol boats at event" % str(e1))
        finally:
    ...
    def _add_volunteer_with_boat_without_commit_or_checks(self, event_id: str, volunteer_and_boat: VolunteerWithIdAtEventWithPatrolBoatId):
