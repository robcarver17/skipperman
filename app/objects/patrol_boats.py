...
class PatrolBoat(GenericSkipperManObjectWithIds):
    ...
    def from_dict(cls, dict_with_str):
        return cls(name=dict_with_str['name'], id=make_id_as_int_str(dict_with_str['id']))
...
class VolunteerAtEventWithPatrolBoat(GenericSkipperManObject):
    ...
    def from_dict(cls, dict_with_str):
        ...
        if volunteer_id==EMPTY_VOLUNTEER_ID:
            ...
            return cls(volunteer_id=make_id_as_int_str(volunteer_id),
                       patrol_boat_id=make_id_as_int_str(patrol_boat_id),
                       day=Day[day])

    @classmethod
    def create_unallocated_boat(cls, patrol_boat_id: str):
        return cls(volunteer_id=EMPTY_VOLUNTEER_ID, patrol_boat_id=patrol_boat_id,
                   day=ARBITRARY_DAY)
...
class ListOfVolunteersAtEventWithPatrolBoats(GenericListOfObjectsWithIds):
    ...
    def swap_boats_for_volunteers_in_allocation(self, original_day: Day,
                                                                           original_volunteer_id: str,
                                                                           day_to_swap_with: Day,
                                                                           volunteer_id_to_swap_with: str):

        original_boat_id = self.which_boat_id_is_volunteer_on_today(volunteer_id=original_volunteer_id, day=original_day)
        swapping_boat_id = self.which_boat_id_is_volunteer_on_today(volunteer_id=volunteer_id_to_swap_with, day=day_to_swap_with)

        self.remove_volunteer_from_patrol_boat_on_day_at_event(volunteer_id=original_volunteer_id, day=original_day)
        self.remove_volunteer_from_patrol_boat_on_day_at_event(volunteer_id=volunteer_id_to_swap_with, day=day_to_swap_with)
    ...
    def list_of_volunteer_ids_assigned_to_boat_and_day(self, patrol_boat: PatrolBoat, day: Day) -> List[str]:
        all_items = self.list_of_all_items_excluding_unallocated()
        list_of_ids = [item.volunteer_id for item in all_items if item.patrol_boat_id==patrol_boat.id and item.day == day]
        return list_of_ids

    def remove_patrol_boat_id_and_all_associated_volunteer_connections_from_event(self, patrol_boat_id: str):
        for item in self:
    ...
    def is_volunteer_already_on_a_boat_on_day(self, volunteer_id: str,  day: Day) -> bool:
        matches = [item for item in self if item.volunteer_id == volunteer_id and item.day == day]
    ...
    def which_boat_id_is_volunteer_on_today(self, volunteer_id: str, day: Day) -> str:
        ...
        if len(matches)==0:
            ...
            raise Exception("Volunteer %s day %s is on more than one boat at event shouldn't be possible!" % (volunteer_id, day.name))

        return matches[0].patrol_boat_id


    def list_of_unique_boats_at_event_including_unallocated(self, list_of_patrol_boats: ListOfPatrolBoats):
        ## sorted according to order of list of patrol boats
        list_of_boat_ids_at_event_including_unallocated = self.list_of_unique_boat_ids_at_event_including_unallocated()
        list_of_boats = [boat for boat in list_of_patrol_boats if make_id_as_int_str(boat.id) in list_of_boat_ids_at_event_including_unallocated]
    ...
    def list_of_all_items_excluding_unallocated(self):
        return ListOfVolunteersAtEventWithPatrolBoats([item for item in self if not item.is_empty])