...
from objects.groups import ListOfCadetsWithGroups, ALL_GROUPS_INCLUDING_UNALLOCATED, Group


def allocate_cadet(cadet: Cadet,
    ...
    interface.message("Cadet %s, currently in group %s" % (cadet.name, previous_group_name))
    interface.message("Choose group to allocate into")

    chosen_group_name = interface.get_choice_from_adhoc_menu(ALL_GROUPS_INCLUDING_UNALLOCATED)
    chosen_group = Group(chosen_group_name)

    list_of_cadets_with_groups.update_group_for_cadet(cadet=cadet, chosen_group=chosen_group)