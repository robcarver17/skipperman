...
from app.interface.html.url import HOME, INDEX_URL, get_menu_url, get_action_url
...
## recursive
LEVEL_SEPERATOR = "-"
def parse_menu_option_into_html(menu_option: str, current_menu_definition: dict,
                                breadcrumbs: str = "",
                                top_level:bool = True) -> Html:
    ...
    for name_of_option, contents_of_option in current_menu_definition.items():
        ...
        if type(contents_of_option) is str:
            ## return a non menu link for action
            link_this_option = get_action_url(contents_of_option)
            html_this_option = html_link_in_list_item(string=name_of_option,
                                                      url=link_this_option)
        else:
            ...
            if first_part_of_menu_option==option_with_underscores:
                ...
                breadcrumbs_to_prefix_submenu_names = breadcrumbs+option_with_underscores+LEVEL_SEPERATOR
                ...
                html_this_option = html_link_in_list_item(url=link_this_menu, string=name_of_option + ":")
