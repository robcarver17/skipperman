from app.backend.administration.delete_volunteers import delete_volunteer_in_data_and_return_warnings
from app.frontend.form_handler import button_error_and_back_to_initial_state_form
from app.frontend.shared.volunteer_state import get_volunteer_from_state, update_state_for_specific_volunteer
...
def display_deleting_volunteer_process(interface: abstractInterface):
    ...
    warnings = delete_volunteer_in_data_but_do_not_save_cache_and_return_warnings(interface,
                                                                     volunteer_to_delete=volunteer_to_delete)


    return Form(
        ListOfLines(
        [
           Heading("Deleting %s" % (volunteer_to_delete)),
            _______________
        ]+warnings+[
...
def delete_volunteer_in_data_but_do_not_save_cache_and_return_warnings(interface: abstractInterface,
                                                          volunteer_to_delete: Volunteer) -> ListOfLines:

    warnings = delete_volunteer_in_data_and_return_warnings(interface.object_store, volunteer_to_delete=volunteer_to_delete)
...
def set_volunteer_to_delete_in_state(interface: abstractInterface, volunteer: Volunteer):
