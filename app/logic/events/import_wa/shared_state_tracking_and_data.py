from app.backend.data.mapped_events import  load_mapped_wa_event_delta_rows
...
def get_and_save_next_row_id_in_event_data(interface: abstractInterface) -> str:
...
def get_first_row_id_in_event_data(interface: abstractInterface) -> str:
    ...
    list_of_ids = list_of_row_ids_in_main_event_delta_file(event)
    id = list_of_ids[0]
...
def list_of_row_ids_in_main_event_delta_file(event: Event) -> list:
    event_delta = load_mapped_wa_event_delta_rows(event)

    all_ids = event_delta.all_row_ids_sorted_by_type()
