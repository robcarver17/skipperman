...
def display_form(state_data: StateDataForAction, form_values:Event = default_event,
    ...
    start_date = html_date_input(input_label="Start date", input_name=EVENT_START_DATE,
                          max_date_years=2, min_date_years=2,
                          value=form_values.start_date)
    end_date = html_date_input(input_label="End date", input_name=EVENT_END_DATE,
                               max_date_years=2, min_date_years=2,
                          value=form_values.end_date)
    event_type = html_radio_input(input_label="Type of event", input_name=EVENT_TYPE,
...
def process_form_when_checking_event(state_data: StateDataForAction) -> Tuple[Html, Event]:
    ...
    try:
        ...
        verify_text = html_bold(verify_event_and_warn(event=event, data=data))
    except Exception as e:
        verify_text = html_bold("Doesn't appear to be a valid event (wrong date time in old browser?) error code %s" % str(e))
        event = default_event
...
def get_event_from_form(state_data: StateDataForAction) -> Event:
    ...
    return Event(event_name=event_name, event_type=event_type, start_date=start_date, end_date=end_date)


def process_form_when_event_verified(state_data: StateDataForAction):
    try:
        event = get_event_from_form(state_data)
        add_new_verified_event(event=event, data=data)
    except Exception as e:
