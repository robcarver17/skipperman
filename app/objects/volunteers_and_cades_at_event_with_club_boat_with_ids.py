...
@dataclass
class VolunteerAtEventWithClubDinghyWithId(GenericSkipperManObject):
    volunteer_id: str
    club_dinghy_id: str
    day: Day
...
class ListOfVolunteerAtEventWithIdAndClubDinghies(GenericListOfObjectsWithIds):
    ...
    def update_allocation_for_volunteer_on_day(
        ...
        if self.has_allocation_for_volunteer_on_day(volunteer_id=volunteer_id, day=day):
            self._update_existing_allocation_for_volunteer_on_day(
                volunteer_id=volunteer_id, day=day, club_dinghy_id=club_dinghy_id
            )
        else:
            self._add_new_allocation_for_volunteer_on_day(
                volunteer_id=volunteer_id, day=day, club_dinghy_id=club_dinghy_id
            )

    def _update_existing_allocation_for_volunteer_on_day(
        self, volunteer_id: str, day: Day, club_dinghy_id: str
    ):
    ...
    def _add_new_allocation_for_volunteer_on_day(
        self, volunteer_id: str, day: Day, club_dinghy_id: str
    ):
        if club_dinghy_id == no_club_dinghy_id:
            return
    ...
    def has_allocation_for_volunteer_on_day(self, volunteer_id: str, day: Day) -> bool:
        idx = self.index_of_item_for_volunteer_id_on_day(
            volunteer_id=volunteer_id, day=day, default=index_not_found
        )
        return not (idx is index_not_found)

    def delete_allocation_for_volunteer_on_day(self, volunteer_id: str, day: Day):
        ## allowed to fail
        idx = self.index_of_item_for_volunteer_id_on_day(
            volunteer_id=volunteer_id, day=day, default=index_not_found
        )
    ...
    def index_of_item_for_volunteer_id_on_day(
    ...
    def item_for_volunteer_id_on_day(
        self, volunteer_id: str, day: Day, default=arg_not_passed
    ) -> VolunteerAtEventWithClubDinghyWithId:
        return get_unique_object_with_multiple_attr_in_list(
            some_list=self,
