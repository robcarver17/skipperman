...
from app.backend.groups.cadets_with_groups_at_event import add_or_upate_group_for_cadet_on_day_if_cadet_available_on_day
from app.frontend.events.group_allocation.render_allocation_form import get_list_of_all_cadets_with_event_data
...
def do_allocation_for_cadet_at_event_on_day(
    ...
    chosen_group = get_group_with_name(object_store=interface.object_store, group_name=allocation_str)
...
def get_cadet_notes_for_row_in_form_and_alter_registration_data(
    ...
    update_notes_for_existing_cadet_at_event(
        object_store=interface.object_store, event=event,cadet=cadet, new_notes=new_notes
    )
...
def update_boat_info_for_all_cadets_in_form_on_day(
    ...
    update_boat_info_for_cadets_at_event(
        object_store=interface.object_store, event=event, list_of_updates=list_of_updates, day=day
    )
