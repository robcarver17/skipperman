...
class DictOfClubDinghyLimits(Dict[Event, DictOfClubDinghyLimitsForEvent]):
    def __init__(
        self,
        raw_dict: Dict[Event, DictOfClubDinghyLimitsForEvent],
    ...
    def dict_of_limits_for_all_visible_club_boats(self, event: Event) -> Dict[str, int]:
        return dict(
            [
                (
                    boat.name,
                    self.get_limit_at_event(event=event, club_boat=boat, default=0),
                )
    ...
    def dict_of_limits_for_all_boats_at_event(
        self, event: Event
    ) -> Dict[ClubDinghy, int]:
        return dict(
            [
                (boat, self.get_limit_at_event(event=event, club_boat=boat))
                for boat in self.list_of_club_dinghies
            ]
        )

    def get_limit_at_event(
        self, event: Union[Event, object], club_boat: ClubDinghy, default=0
    ) -> int:
        limit = self.get_underlying_limit_at_event(
            event, club_boat, default=missing_data
        )
    ...
    def get_generic_limit(self, club_boat: ClubDinghy, default=0) -> int:
        return self.get_underlying_limit_at_event(
            event=generic_limit_event, club_boat=club_boat, default=default
        )

    def get_underlying_limit_at_event(
        self, event: Union[Event, object], club_boat: ClubDinghy, default=0
    ) -> int:
    ...
    def clear_and_set_generic_limit(
        self, original_boat: ClubDinghy, new_boat: ClubDinghy, new_limit: int
    ):
    ...
    def set_generic_limit(self, club_boat: ClubDinghy, limit: int):
        self.set_limit_at_event(
            event=generic_limit_event, club_boat=club_boat, limit=limit
        )

    def set_limit_at_event(
        self, event: Union[Event, object], club_boat: ClubDinghy, limit: int
    ):
        limits_at_event = self.get_limits_for_event(event)
        limits_at_event.set_limit_for_boat(club_dinghy=club_boat, limit=limit)

        if event is generic_limit_event:
            self.list_of_club_dinghy_limits.update_general_limit_for_club_dinghy_id(
                club_dinghy_id=club_boat.id, limit=limit
            )
        else:
            self.list_of_club_dinghy_limits.update_limit_for_event_id_and_club_dinghy_id(
                event_id=event.id, club_dinghy_id=club_boat.id, limit=limit
            )
