...
def get_row_in_registration_data_for_cadet_both_cancelled_and_active(
    ...
    if len(all_rows) == 0:
        raise NoMoreData(
            "No data for identified cadet %s in raw registration data" % cadet
        )
...
def list_of_cadet_ids_in_event_data_and_identified_in_raw_registration_data_for_event(
    object_store: ObjectStore,
    event: Event,
    include_identified_in_raw_registration_data: bool = True,
) -> List[str]:
...
def identified_cadet_ids_in_raw_registration_data(
    ...
    identified_cadet_data = get_list_of_identified_cadets_at_event(
        object_store=object_store, event=event
    )

    row_ids = raw_registration_data.list_of_row_ids()
    list_of_cadet_ids = [
        identified_cadet_data.cadet_id_given_row_id(row_id) for row_id in row_ids
    ]
    list_of_cadet_ids = [
