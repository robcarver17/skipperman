...
from app.backend.mapping.check_field_mapping import get_field_mapping_or_empty_list_from_raw_event_file, \
    get_list_of_unused_skipperman_fields_at_event, get_list_of_unused_WA_fields_at_event_given_uploaded_file
...
def get_mapping_table(
    ...
    return Table(
        [top_row]+[adding_row]+rows_in_mapping,
...
def body_of_mapping_table(interface: abstractInterface, event: Event) -> List[RowInTable]:
    mapping = get_field_mapping_for_event(
        object_store=interface.object_store, event=event
    )
    mapping.sort_by_skipperman_field()

    rows_in_mapping = [row_in_mapping_table(
        event=event,
        wa_field_map=wa_field_map
...
def row_in_mapping_table( event: Event, wa_field_map: WAFieldMap)->RowInTable:
    ...
    return RowInTable([
        ...
        entry_for_WA_field_in_mapping_table( event=event, wa_field=wa_field_map.wa_field),
        delete_button])
...
def entry_for_WA_field_in_mapping_table( event: Event, wa_field: str):
    flag = "(Missing from uploaded file)" if WA_Field_is_missing_from_uploaded_file( event=event, wa_field=wa_field) else ""
...
def add_fields_row_in_mapping_table(interface: abstractInterface,  event: Event) -> RowInTable:
    dropdown_skipperman = dropdown_of_unused_skipperman_fields(interface=interface, event=event)
    add_skipperman_or_input_WA = input_field_dropdown_of_unused_WA_fields_or_add_skipperman_button(interface=interface, event=event)
...
def dropdown_of_unused_skipperman_fields(interface: abstractInterface,  event: Event):
    dict_of_options = _get_dict_for_dropdown_of_unused_skipperman_fields(interface=interface, event=event)
...
def _get_dict_for_dropdown_of_unused_skipperman_fields(interface: abstractInterface,  event: Event)-> dict:
    list_of_unused_skipperman_fields_at_event = get_list_of_unused_skipperman_fields_at_event(object_store=interface.object_store, event=event)
    list_of_unused_skipperman_fields_at_event.sort()
    list_of_unused_skipperman_fields_at_event.append(ADD_SKIPPERMAN_FIELD_DROPDOWN_EMPTY)

    dict_of_options = dict([(field,field) for field in list_of_unused_skipperman_fields_at_event])

    return dict_of_options

def input_field_dropdown_of_unused_WA_fields_or_add_skipperman_button(interface: abstractInterface,  event: Event) -> Union[Button, dropDownInput, textInput]:
    current_field = get_current_skipperman_field_to_add_from_state(interface)
...
def dropdown_of_unused_WA_fields(interface: abstractInterface,   event: Event) ->dropDownInput:
    dict_of_options = _get_dict_for_dropdown_of_unused_WA_fields(interface=interface, event=event)
...
def _get_dict_for_dropdown_of_unused_WA_fields(interface: abstractInterface,   event: Event)-> dict:
    list_of_unused_WA_fields_at_event = get_list_of_unused_WA_fields_at_event_given_uploaded_file(event=event, object_store=interface.object_store)
    list_of_unused_WA_fields_at_event.sort()
    list_of_unused_WA_fields_at_event.append(ADD_WA_FIELD_DROPDOWN_EMPTY)

    dict_of_options = dict([(field,field) for field in list_of_unused_WA_fields_at_event])
...
add_skipperman_field_button= Button(
        label="Add selected skipperman field to mapping",
        value="add_skipperman_field"
    )

add_WA_field_button= Button(
        label="Add selected skipperman & WA field to mapping",
        value="add_wa_field"
    )
...
def delete_button_for_row_in_mapping_table(wa_field_map: WAFieldMap):
    return Button(
        label="Delete",
        value=delete_button_value_for_row_in_mapping_table(wa_field_map)
    )
...
def list_of_delete_button_values_in_mapping_table(interface: abstractInterface):
    ...
    list_of_buttons =  [delete_button_value_for_row_in_mapping_table(wa_field_map) for wa_field_map in mapping]
...
def save_current_skipperman_field_to_add_from_state(interface: abstractInterface, skipperman_field: str):
