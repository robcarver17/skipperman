...
def save_quick_note(interface: abstractInterface):
    ...
    add_quick_note(object_store=interface.object_store, text=text, volunteer_author=volunteer_author)
...
def update_existing_note(interface: abstractInterface, note: NoteWithVolunteer):
    assigned_volunteer = get_assigned_volunteer_from_form(interface=interface, note=note)
    completed = is_completed(interface=interface, note=note)
    priority = interface.value_from_form(get_input_name_for_note(note, PRIORITY), default=note.priority)
    text = interface.value_from_form(get_input_name_for_note(note, TEXT), default = note.text)

    update_note_with_new_data(interface.object_store,
                              note_id=note.id,
                              priority=priority,
...
def get_assigned_volunteer_from_form(interface: abstractInterface, note: NoteWithVolunteer):
...
def is_completed(interface: abstractInterface, note: NoteWithVolunteer):
    selected = interface.value_of_multiple_options_from_form(get_input_name_for_note(note, COMPLETED), default=MISSING_FROM_FORM)
