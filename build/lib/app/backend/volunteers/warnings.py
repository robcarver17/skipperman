...
def warn_on_all_volunteers_generic(
    interface: abstractInterface, warning_function: Callable
) -> List[str]:
    ...
    for volunteer_at_event in list_of_volunteers_at_event:
        warnings_for_volunteer = warning_function(
            interface=interface, volunteer_at_event=volunteer_at_event, event=event
        )
...
def warn_about_single_volunteer_groups_at_event(
    ...
    is_lake_volunteer = volunteer_is_on_lake(
        interface=interface, volunteer_id=volunteer_at_event.volunteer_id, event=event
    )
    ...
    list_of_groups_for_volunteer = groups_for_volunteer(
        interface=interface, event=event, volunteer_id=volunteer_at_event.volunteer_id
    )
...
def warn_about_single_volunteer_availablity_at_event(
    ...
    active_connected_cadets = ListOfCadets(
        [
            cadet
            for cadet in connected_cadets
            if cadet.id in cadets_at_event_data.list_of_active_cadet_ids_at_event(event)
        ]
    )
...
def warning_for_specific_cadet_at_event(
    ...
    no_volunteer = has_no_active_volunteer(
        interface=interface, event=event, cadet=cadet
    )
    warning = ""
    if no_volunteer:
...
def warn_about_single_volunteer_with_qualifications(
    ...
    volunteer = volunteer_data.volunteer_with_id(
        volunteer_at_event.volunteer_id
    )  ## FIXME SHOULD BE ABLE TO GET THIS FROM VolunteerAtEvent
