...
def get_sorted_volunteers_allocated_to_patrol_boat_at_event_on_days_sorted_by_role(
    ...
    all_volunteers = get_list_of_voluteers_at_event_with_patrol_boats_from_cache(cache=cache, event=event)
    volunteers_on_boat_on_day = all_volunteers.assigned_to_boat_on_day(patrol_boat=patrol_boat, day=day)
...
def sort_list_of_volunteers_for_day_and_event_by_role(day: Day,
volunteers_on_boat_on_day: ListOfVolunteersAtEventWithSkillsAndRolesAndPatrolBoats
    ...
    for role in volunteer_roles:
        list_of_volunteers_with_this_role = volunteers_on_boat_on_day.has_volunteer_role(day=day, role=role)
