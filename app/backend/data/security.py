...
class UserData:
    ...
    def get_list_of_users(self) -> ListOfSkipperManUsers:
        ...
        return (
            list_of_users_could_be_empty.list_of_users()
        )  ## will contain default if nothing else
...
def load_all_users(
    interface: abstractInterface = arg_not_passed,
) -> ListOfSkipperManUsers:
...
def regenerate_password_hash(
    ...
    user_data.change_password_for_user(
        username=user.username, new_password=user_entered_password
    )
...
def modify_volunteer_id_for_user(
    username: str, new_volunteer_id: str, interface: abstractInterface
):
    user_data = UserData(interface.data)
    user_data.modify_volunteer_id_for_user(
        username=username, new_volunteer_id=new_volunteer_id
    )
...
def get_volunteer_id_of_logged_in_user_or_superuser(
