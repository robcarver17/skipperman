...
from app.frontend.shared.add_edit_or_choose_volunteer_form import skills_form_entries, \
    get_and_save_volunteer_skills_from_form
from app.frontend.shared.volunteer_state import get_volunteer_from_state, update_state_with_volunteer_id, clear_volunteer_id_in_state
from app.backend.volunteers.list_of_volunteers import get_volunteer_with_matching_name
from app.backend.volunteers.refresh_skills_from_csv_import import load_skills_refresh_file, \
...
def process_current_volunteer_row_in_temp_file(
    ...
    volunteer = get_volunteer_with_matching_name(
        object_store=interface.object_store, volunteer=volunteer_from_form, default = missing_data
    )
    ...
    if volunteer is missing_data:
        ...
        return process_row_when_volunteer_has_been_identified(interface=interface,
                                                              volunteer=volunteer)



def display_volunteer_selection_in_skill_import_form(
    interface: abstractInterface
):
...
def get_header_text_for_volunteer_selection_form(
    ...
    # Custom header text
    progress_bar = ProgressBar('Refreshing skills data', percentage_of_volunteer_ids_done_in_skills_file(interface))
    return ListOfLines([progress_bar,
                 HorizontalLine(),
...
def post_volunteer_selection_in_skill_import_form(
    ...
    if result.is_button:
        ...
        return process_row_when_volunteer_has_been_identified(interface=interface, volunteer=volunteer)
    else:
        raise Exception("Can't hanlde result %s" % str(result))


def process_row_when_volunteer_has_been_identified(interface: abstractInterface, volunteer: Volunteer)-> Union[Form, NewForm]:
...
def display_skills_editing_form_when_mismatch(interface: abstractInterface):
    ...
    current_row = get_volunteer_row_from_temp_file_and_state(interface)
    warnings = compare_skills_for_volunteer_with_passed_and_return_warning(object_store=interface.object_store, volunteer=volunteer, current_row=current_row)
...
help_button = HelpButton("refresh_skills_with_import_help#reconciling-and-editing-skills")
...
def next_volunteer_in_temporary_file(interface: abstractInterface) -> Union[Form, NewForm]:
    try:
...
def return_to_calling_function(interface: abstractInterface):
    ...
    return interface.get_new_display_form_for_parent_of_function(display_volunteer_selection_in_skill_import_form)
...
def percentage_of_volunteer_ids_done_in_skills_file(interface: abstractInterface) -> int:
    list_of_volunteers_with_skills = load_skills_refresh_file()
    current_volunteer_idx = get_volunteer_idx_in_temporary_file_from_state(interface=interface)


    return percentage_of_x_in_y(current_volunteer_idx, list_of_volunteers_with_skills)

def reset_temp_volunteer_file_counter_to_first_value(interface: abstractInterface)-> RowInImportedSkillsFile:
    list_of_volunteers_with_skills = load_skills_refresh_file()
    ...
    update_state_for_specific_volunteer_id_in_temporary_file(
        interface=interface, idx=0
    )

    return first_row


def get_next_volunteer_row_and_store(interface: abstractInterface) ->RowInImportedSkillsFile:
    list_of_volunteers_with_skills = load_skills_refresh_file()
    current_volunteer_idx = get_volunteer_idx_in_temporary_file_from_state(interface=interface)
...
def clear_state_for_specific_volunteer_id_in_temporary_file(interface: abstractInterface):
    interface.clear_persistent_value(TEMP_VOLUNTEER_ID)


def get_volunteer_row_from_temp_file_and_state(interface: abstractInterface) -> RowInImportedSkillsFile:
