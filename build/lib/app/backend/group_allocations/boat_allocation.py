...
def update_boat_info_for_updated_cadets_at_event(interface: abstractInterface, event: Event,
...
def summarise_club_boat_allocations_for_event(interface: abstractInterface, event: Event) -> PandasDFTable:
    dinghies_data = DinghiesData(interface.data)
    cadets_at_event_data = CadetsAtEventData(interface.data)

    cadets_with_club_dinghies_at_event = dinghies_data.get_list_of_cadets_at_event_with_club_dinghies(event)
    list_of_dinghy_ids = dinghies_data.unique_sorted_list_of_allocated_club_dinghy_ids_allocated_at_event(event)
...
def get_relevant_cadet_ids_for_club_dinghy_id(group: str,
                                              event: Event,
                                              list_of_ids_with_groups: ListOfCadetAtEventWithClubDinghies) -> Dict[Day, List[str]]:
    ...
    for day in event.weekdays_in_event():
        result_dict[day] = [cadet_id
                            for cadet_id in cadets_with_club_dinghies_at_event.list_of_unique_cadet_ids()
                            if cadets_with_club_dinghies_at_event.dinghy_for_cadet_id_on_day(cadet_id=cadet_id, day=day) == dinghy_id]

    return result_dict


def summarise_class_attendance_for_event(interface: abstractInterface, event: Event) -> PandasDFTable:
    dinghies_data = DinghiesData(interface.data)
    cadets_at_event_data = CadetsAtEventData(interface.data)

    cadets_with_dinghies_at_event = dinghies_data.get_list_of_cadets_at_event_with_dinghies(event)

    list_of_boat_class_ids = dinghies_data.unique_sorted_list_of_boat_class_ids_at_event(event)
...
def get_relevant_cadet_ids_for_boat_class_id(group: str,
                                     event: Event,
                                     list_of_ids_with_groups: ListOfCadetAtEventWithDinghies)-> Dict[Day, List[str]]:
