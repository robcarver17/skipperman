...
from app.objects.roles_and_teams import ListOfTeams, ListOfRolesWithSkillIds, Team, ListOfTeamsAndRolesWithIds
...
def reorder_roles_for_team_given_list_of_names(
    ...
    ordered_list_of_roles = ListOfRolesWithSkills([list_of_roles.matches_name(role_name) for role_name in new_order_of_role_names])
    new_list_of_roles_with_ids = ListOfTeamsAndRolesWithIds.create_new_list_for_team_from_ordered_role_ids(list_of_role_ids=ordered_list_of_roles.list_of_ids(),
                                                                              team_id=team.id)
...
def add_new_named_role_to_team(
    ...
    try:
        ...
        interface.log_error("Can't add role %s to team %s, error: %s" % (new_role_name, team, str(e)))
...
def modify_role_with_skills(
    ...
    try:
        ...
        interface.log_error("Cannot modify %s because %s" % (existing_object.name, str(e)))
...
def add_new_team(interface: abstractInterface,  name_of_entry_to_add: str):
    ...
    try:
        ...
        interface.log_error("can't add team %s error: %s" % (name_of_entry_to_add, str(e)))
...
def modify_team(interface: abstractInterface, existing_object: Team, new_object: Team):
    try:
        ...
        interface.log_error("can't modify team %s to %s error: %s" % (existing_object, new_object, str(e)))
...
def get_dict_of_teams_and_roles(object_store: ObjectStore) -> DictOfTeamsWithRoles:
    return object_store.get(object_store.data_api.data_list_of_teams_and_roles_with_ids.get_dict_of_teams_and_roles)
