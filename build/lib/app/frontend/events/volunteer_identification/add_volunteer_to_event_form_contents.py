...
VOLUNTEER_STATUS_IN_FORM = "vol_status"
NOTES = "Notes"
ALERT_FORM = "ALERT"
...
def display_form_to_confirm_volunteer_details(
    ...
    status_text = get_any_self_declared_status_text_input_box(
        list_of_relevant_information=list_of_relevant_information
    )
    ...
    return Form(
        ListOfLines(
            [
                ...
                alert_input,
                _______________,
                Line([save_button, do_not_add_volunteer, save_alert]),
            ]
        )
    )
...
def get_any_self_declared_status_text_input_box(
    list_of_relevant_information: ListOfRelevantInformationForVolunteer,
) -> textInput:
    list_of_self_declared = []
    ...
    for relevant_information in list_of_relevant_information:
        try:
            ...
            if len(self_declared) == 0:
                continue
            list_of_self_declared.append(self_declared)
        except:
            pass

    if len(list_of_self_declared)==0:
        self_declared = ""
        comment = "No volunteer status declared in form - assume can volunteer"
    elif check_identical(list_of_self_declared):
        self_declared = list_of_self_declared[0]
        comment = "Volunteer status in form: "
    else:
        self_declared = list_of_self_declared[0]
        comment = "Different status across multiple registrations: %s, enter status to use: " % ", ".join(list_of_self_declared)

    return textInput(
        input_name=VOLUNTEER_STATUS_IN_FORM,
        input_label=comment,
        value=self_declared
    )
...
def get_alert_box():
    return textInput(input_name=ALERT_FORM,
                     input_label="Enter warning text that will require confirmation (or click <%s> button for a generic warning)" % SAVE_CHANGES_WITH_GENERIC_ALERT,
                     value="")
...
SAVE_CHANGES_WITH_GENERIC_ALERT = "Save changes with generic 'check volunteer' confirmation required"
...
save_alert = Button(SAVE_CHANGES_WITH_GENERIC_ALERT)