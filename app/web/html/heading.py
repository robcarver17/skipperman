...
def get_html_header(
    ...
    if include_support_email_and_global_help:
        ...
        print(help_link)
        global_help = Html(
            '<a href="/%s" class="w3-bar-item w3-button w3-padding-16">Click here for general help</a>'
            % help_link
        )
        support_email_and_global_help = html_joined_list_as_lines(
            [support_email, global_help]
        )
    else:
