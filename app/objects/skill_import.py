...
class ConfigForSkillImportOneSkill:
    ...
    def all_columns_from_csv(self):
        my_columns = [
            self.expires_column_name,
            self.valid_from_column_name,
            self.starts_from_column_name,
        ]
...
def from_skills_dict_in_import_config_to_import_config(
...
def from_skills_dict_in_import_config_to_dict_of_skills_and_config(
    skills_config_from_file: dict,
) -> Dict[str, ConfigForSkillImportOneSkill]:
    return dict(
        [
            (skill_name, config_for_skill_from_dict(config))
...
class RowInImportedSkillsFile:
    ...
    def volunteer_name_as_tuple(self):
        ...
        return (
            self.series[skills_config.volunteer_first_name_column],
            self.series[skills_config.volunteer_surname_column],
        )
    ...
    def is_skill_valid(self, skill_name):
        valid_from_date = self.valid_from_date_for_skill(
            skill_name, default=missing_data
        )
        valid_to_date = self.end_date_for_skill(skill_name, default=missing_data)
        expiry_date = self.expiry_date_given_start_date_for_skill(
            skill_name, default=missing_data
        )

        if all(
            [
                valid_from_date is missing_data,
                valid_to_date is missing_data,
                expiry_date is missing_data,
            ]
        ):
    ...
    def explain_why_skill_invalid(self, skill_name: str):
        ...
        config_for_skill = self.skills_config.dict_of_skill_names_and_configs[
            skill_name
        ]
        ...
        if not valid_from_date is missing_data:
            if today < valid_from_date:
                msg += "Have not yet reached start date %s (in column %s). " % (
                    valid_from_date,
                    from_column,
                )
        ...
        if not expiry_date is missing_data:
            if today > expiry_date:
                msg += (
                    "Qualification runs for %d years (configured) from %s (in column %s) to %s and has run out. "
                    % (expiry_date_years, start_date, start_column, expiry_date)
                )
        ...
        if all(
            ...
            msg = (
                "None of date column(s) %s filled in so assume do not have"
                % ", ".join([from_column, to_column, start_column])
            )
    ...
    def explain_why_is_valid(self, skill_name: str):
        ...
        if not expiry_date is missing_data:
            msg += "Valid for %d years (configured) from %s onwards (column %s)" % (
                expiry_date_years,
                start_date,
                start_column,
            )
    ...
    def expiry_date_given_start_date_for_skill(
        self, skill_name: str, default=missing_data
    ):
        starts_from_date = self.starts_from_date_for_skill(
            skill_name=skill_name, default=missing_data
        )
    ...
    def end_date_for_skill(self, skill_name: str, default=missing_data):
        valid_to_date = self.a_date_for_skill(
            skill_name, "expires_column_name", default
        )

        return valid_to_date

    def valid_from_date_for_skill(self, skill_name: str, default=missing_data):
        valid_from_date = self.a_date_for_skill(
            skill_name, "valid_from_column_name", default
        )

        return valid_from_date

    def starts_from_date_for_skill(self, skill_name: str, default=missing_data):
        starts_from_date = self.a_date_for_skill(
            skill_name, "starts_from_column_name", default
        )
    ...
    def a_date_for_skill(self, skill_name: str, date_attr: str, default=missing_data):
        ...
        try:
            ...
            raise Exception(
                "Date %s not in expected format %s eg %s"
                % (
                    date_as_str,
                    date_format,
...
class ListOfRowsInSkillsFile(List[RowInImportedSkillsFile]):
    ...
    def from_df(cls, some_df: pd.DataFrame, skills_config: ImportSkillsconfig):
        ...
        return cls(
            [
                RowInImportedSkillsFile(row[1], skills_config=skills_config)
                for row in as_list
            ]
        )
