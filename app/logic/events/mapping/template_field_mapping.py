...
def display_form_for_upload_template_field_mapping(interface: abstractInterface):
    ...
    template_name_field = textInput(input_name=TEMPLATE_NAME, input_label="Enter template name", value=empty_name)
    file_select_field = fileInput(input_label=MAPPING_FILE, accept=".csv")

    list_of_lines = ListOfLines(["Choose .csv file to upload as a mapping template, providing template name",
                                template_name_field,
                                file_select_field
                                , buttons])
