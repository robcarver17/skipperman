...
PERSISTENT_CADETS_WITH_GROUP_ID_TABLE = (
    "list_of_group_names_for_events_and_cadet_persistent_versions"
)
...
class SqlDataListOfGroupNamesForEventsAndCadetPersistentVersion(GenericSqlData):
    def get_dict_of_group_names_for_events_and_cadets_persistent_version(
        ...
        for raw_item in raw_data_as_list:
            ...
            dict_of_event_ids_and_group_names = (
                raw_item.dict_of_event_ids_and_group_names
            )

            cadet = self.list_of_cadets.cadet_with_id(cadet_id)
            dict_this_cadet = dict(
                [
                    (self.list_of_events.event_with_id(event_id), group_name)
    ...
    def read(self) -> ListOfGroupNamesForEventsAndCadetPersistentVersionWithIds:
        if self.table_does_not_exist(PERSISTENT_CADETS_WITH_GROUP_ID_TABLE):
            return (
                ListOfGroupNamesForEventsAndCadetPersistentVersionWithIds.create_empty()
            )
    ...
    def write(
        self,
        list_of_cadet_ids_with_group_names: ListOfGroupNamesForEventsAndCadetPersistentVersionWithIds,
    ):
        try:
            if self.table_does_not_exist(PERSISTENT_CADETS_WITH_GROUP_ID_TABLE):
                self.create_table()
            ...
            self.cursor.execute(
                "DELETE FROM %s" % PERSISTENT_CADETS_WITH_GROUP_ID_TABLE
            )

            for cadet_id_with_group_names_dict in list_of_cadet_ids_with_group_names:
                self._insert_row_without_checks_or_commits(
                    cadet_id_with_group_names_dict
                )
    ...
    def _insert_row_without_checks_or_commits(
        self,
        cadet_id_with_group_names_dict: GroupNamesForEventsAndCadetPersistentVersionWithIds,
    ):
        cadet_id = int(cadet_id_with_group_names_dict.cadet_id)
        group_names_dict = dict_as_str(
            cadet_id_with_group_names_dict.dict_of_event_ids_and_group_names
        )
        ...
        self.cursor.execute(insertion, (cadet_id, group_names_dict))
    ...
    def create_table(self):
        ...
        index_creation_query = "CREATE UNIQUE INDEX %s ON %s (%s)" % (
            INDEX_NAME_PERSISTENT_CADETS_WITH_GROUP_ID_TABLE,
            PERSISTENT_CADETS_WITH_GROUP_ID_TABLE,
            CADET_ID,
        )
        try:
