...
def get_list_of_groups_volunteer_id_can_see(
    ...
    return volunteer_rota_data.get_list_of_groups_volunteer_is_instructor_for(
...
def can_see_all_groups_and_award_qualifications(
    ...
    volunteer_rota_data = VolunteerRotaData(interface.data)
    is_senior_instructor_at_event = volunteer_rota_data.is_senior_instructor(
        event=event, volunteer_id=volunteer_id
    )
...
def get_list_of_events_entitled_to_see(
    interface: abstractInterface, volunteer_id: str, sort_by: str
):
...
def is_volunteer_SI_or_super_user(interface: abstractInterface):
    ...
    if volunteer_id == SUPERUSER:
        return True
    return is_volunteer_with_id_qualified_as_SI(data_layer=interface.data, volunteer_id=volunteer_id)
...
def get_ticksheet_data(
    ...
    list_of_tick_sheet_items_for_this_qualification = (
        tick_sheet_data.list_of_tick_sheet_items_for_this_qualification(
            qualification.id
        )
    )
    list_of_substage_names = (
        tick_sheet_data.list_of_substage_names_give_list_of_tick_sheet_items(
            list_of_tick_sheet_items_for_this_qualification
        )
    )

    list_of_cadet_ids_with_qualification = (
        qualifications_data.list_of_cadet_ids_with_qualification(qualification)
    )
...
def save_ticksheet_edits_for_specific_tick(
    interface: abstractInterface, new_tick: Tick, cadet_id: str, item_id: str
):
    ticksheet_data = TickSheetsData(interface.data)
    ticksheet_data.add_or_modify_specific_tick(
