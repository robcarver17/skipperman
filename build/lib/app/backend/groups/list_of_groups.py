...
def add_new_sailing_group_given_name(
    ...
    try:
        ...
        interface.log_error(
            "Error: %s, when adding group %s" % (str(e), name_of_entry_to_add)
        )
...
def modify_sailing_group(
    ...
    try:
        interface.update(
            interface.object_store.data_api.data_list_of_groups.modify_sailing_group,
            existing_group_id=existing_object.id,
            new_group=new_object,
        )
    except Exception as e:
        interface.log_error(
            "Error: %s, when modifying %s to %s" % (str(e), existing_object, new_object)
        )


def get_group_with_id(
    object_store: ObjectStore, group_id: str, default=arg_not_passed
) -> Group:
