...
from app.objects.groups import ListOfGroups, Group,unallocated_group_id, unallocated_group
...
class SqlDataListOfCadetsWithGroups(GenericSqlData):
    def delete_cadet_group_on_day(self,
                                                    event_id: str,
                                                    cadet_id: str,
                                  day: Day
                                                    ) :
        ...
        try:
            ...
            self.cursor.execute("DELETE FROM %s WHERE %s=%d AND %s=%d AND %s='%s'" %
    ...
    def delete_cadet_from_event_and_return_messages(self,
                                                    event_id: str,
                                                    cadet_id: str,
                                                    ) -> List[str]:

        rows = len(self.days_and_group_at_event_for_cadet(event_id=event_id, cadet_id=cadet_id))
        ...
        return ["Removed %d days of group attendance at event with ID %s" % (rows, event_id)]

    def update_or_add_cadet_in_group_on_day(self,    event_id:str,
         cadet_id:str,
        day:Day,
        group_id:str,
        ):
        if group_id is unallocated_group_id:
            ...
            self._update_existing_cadet_in_group_on_day_without_checks(event_id=event_id,
                                                                       cadet_id=cadet_id,
                                                                       day=day,
                                                                       group_id=group_id)
        else:
            self.add_cadet_to_group_on_day(event_id=event_id,
            cadet_id=cadet_id,
            day=day,
                                           group_id=group_id)

    def set_cadet_to_unallocated_group_on_day(self,    event_id:str,
         cadet_id:str,
        day:Day,
    ...
    def _update_existing_cadet_in_group_on_day_without_checks(self, event_id:str,
                                                              cadet_id:str,
                                                              day:Day,
                                                              group_id:str,
                                                              ):
    ...
    def _add_cadet_to_group_on_day_without_checks(self,
                                  event_id: str,
                                  cadet_id: str,
                                  day: Day,
                                  group_id: str,
    ...
    def get_list_of_active_cadets_in_group(self, event_id:str, group: Group) -> ListOfCadets:
    ...
    def get_dict_of_cadets_with_groups_at_event(self, event_id: str) -> DictOfCadetsWithDaysAndGroupsAtEvent:
        list_of_cadet_ids = self.get_list_of_all_cadet_ids_with_groups_at_event(event_id)
    ...
    def days_and_group_at_event_for_cadet(self, event_id: str, cadet_id: str) -> DaysAndGroups:
        ...
        return DaysAndGroups(
            dict(
                [(day, self.list_of_groups.group_with_id(group_id)) for day, group_id in raw_dict.items()]
            )
        )


    def get_dict_of_days_and_group_ids_at_event_for_cadet(self, cadet_id: str, event_id: str) -> Dict[Day, str]:
        if self.table_does_not_exist(CADETS_WITH_GROUP_ID_TABLE):
            return dict()

        try:
            cursor = self.cursor
            cursor.execute('''SELECT %s, %s FROM %s WHERE %s=%d AND %s=%d ''' % (
                DAY, GROUP_ID,
                CADETS_WITH_GROUP_ID_TABLE,
                EVENT_ID, int(event_id),
                CADET_ID, int(cadet_id)
        ...
        return dict(
            [
                (Day[raw_item[0]], str(raw_item[1])) for raw_item in raw_list
            ]
        )

    def list_of_active_cadet_ids_at_event(self, event_id:str) -> List[str]:
       return self.object_store.get(self.object_store.data_api.data_cadets_at_event.get_list_of_active_cadets_at_event, event_id=event_id)
    ...
    def list_of_events(self) -> ListOfEvents:
        list_of_events = self.object_store.get(self.object_store.data_api.data_list_of_events.read)
    ...
    def get_list_of_all_group_ids_at_event(
            self, event_id:str
    ) -> List[str]:
        if self.table_does_not_exist(CADETS_WITH_GROUP_ID_TABLE):
            return []
        ...
        try:
            cursor = self.cursor
            cursor.execute('''SELECT DISTINCT %s FROM %s WHERE %s=%d  ''' % (
                GROUP_ID,
                CADETS_WITH_GROUP_ID_TABLE,
                EVENT_ID, int(event_id),
            ))
    ...
    def get_dict_of_most_common_group_allocations_for_single_cadet(
        ...
        raw_dict = self.get_dict_of_event_ids_and_most_common_group_ids_for_cadet(cadet_id)

        return dict([
            (self.list_of_events.event_with_id(event_id),
             self.list_of_groups.group_with_id(group_id)) for event_id, group_id in raw_dict.items()
        ])

    def get_dict_of_event_ids_and_most_common_group_ids_for_cadet(self, cadet_id:str)-> [str, str]:
        list_of_event_ids = self.get_list_of_event_ids_where_cadet_is_allocated(cadet_id)

        return dict(
            [
                (event_id, self.get_most_common_group_id_at_event_for_cadet(
                    cadet_id=cadet_id, event_id=event_id
                )) for event_id in list_of_event_ids
            ]
        )

    def get_most_common_group_id_at_event_for_cadet(self, cadet_id: str, event_id: str, default=unallocated_group.id) ->str:
        return most_common(self.get_list_of_group_ids_at_event_for_cadet(cadet_id=cadet_id, event_id=event_id),
                    default=default)
    ...
    def get_list_of_group_ids_at_event_for_cadet(self, cadet_id: str, event_id: str):
        ...
        try:
            cursor = self.cursor
            cursor.execute('''SELECT DISTINCT %s FROM %s WHERE %s=%d AND %s=%d ''' % (
                GROUP_ID,
                CADETS_WITH_GROUP_ID_TABLE,
                EVENT_ID, int(event_id),
                CADET_ID, int(cadet_id)
    ...
    def write(
        ...
        try:
            ...
            self.cursor.execute("DELETE FROM %s WHERE %s=%d " % (CADETS_WITH_GROUP_ID_TABLE, EVENT_ID, int(event_id)))

            for cadet_with_group_ids in list_of_cadets_with_groups:
                self._add_row_without_commits_or_checks(
                    event_id=event_id,
                    cadet_with_group_ids=cadet_with_group_ids
                )
    ...
    def _add_row_without_commits_or_checks(self, event_id:str, cadet_with_group_ids: CadetIdWithGroup):
