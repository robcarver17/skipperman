...
class FoodRequirements(GenericSkipperManObject):
    ...
    def describe(self):
        description_list = []
        keys = ['vegetarian', 'vegan', 'pescatarian', 'nut_allergy', 'lactose_intolerant', 'gluten_intolerant', 'kosher', 'halal']
        for key in keys:
            if getattr(self, key):
...
class ListOfCadetsWithFoodRequirementsAtEvent(GenericListOfObjects):
    ...
    def subset_matches_food_required_description(self, food_required_description: str) -> 'ListOfCadetsWithFoodRequirementsAtEvent':
        return ListOfCadetsWithFoodRequirementsAtEvent([object for object in self if object.food_requirements.describe()==food_required_description])
...
class ListOfVolunteersWithFoodRequirementsAtEvent(GenericListOfObjects):
    ...
    def subset_matches_food_required_description(self, food_required_description: str) -> 'ListOfVolunteersWithFoodRequirementsAtEvent':
        return ListOfVolunteersWithFoodRequirementsAtEvent([object for object in self if object.food_requirements.describe()==food_required_description])
...
def unique_list_of_food_requirements(list_of_food_requirements: Union[ListOfCadetsWithFoodRequirementsAtEvent, ListOfVolunteersWithFoodRequirementsAtEvent]) -> List[str]:
    all_required = [required.food_requirements.describe() for required in list_of_food_requirements]
    all_required = [food_required for food_required in all_required if len(food_required)>0]
