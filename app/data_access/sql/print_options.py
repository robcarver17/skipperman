    ...
    def write(self, print_options: PrintOptions, report_name: str):
        try:
            ...
            self.cursor.execute(
                ...
                % (PRINT_OPTIONS_TABLE, REPORT_NAME, report_name)
            )
            ...
            for key, value in as_dict.items():
                ...
                self.cursor.execute(insertion, (report_name, key, str(value)))

            self.conn.commit()
        except Exception as e1:
            raise Exception(
                "Error %s when writing to print options for report name %s"
                % (str(e1), report_name)
            )
        finally:
            self.close()
...
class SqlDataListOfArrangementOptions(GenericSqlData):
    def read(self, report_name: str) -> ArrangementOptionsAndGroupOrder:
        ...
        as_dict = dict(columns=raw_item[0], method=raw_item[1], group_order=raw_item[2])
        print(as_dict)
