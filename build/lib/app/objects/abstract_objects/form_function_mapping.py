...
class FormNameFunctionNameMapping(object):
    ...
    def get_form_name_for_function(self, func: Callable):
        possible_form_names = [form_name for form_name, func_in_dict in self.mapping_dict.items() if func_in_dict == func]
        if len(possible_form_names)==0:
            raise MissingFormName("Function %s not found in mapping" % str(func))
        elif len(possible_form_names)>1:
            raise MissingFormName("Function %s found multiple times in mapping" % str(func))
...
class NestedDictOfMappings(object):
    ...
    def parent_child_dict_mapping_back_to_display_parent(self,
                                              current_dict:dict = arg_not_passed,
                                              current_parent: Callable = arg_not_passed) -> dict:
        if current_parent is arg_not_passed:
            at_top_level = True

            current_parent = self.get_top_level_tuple()[DISPLAY_IDX] ## parents are always the display as that is what we want back
    ...
    def _get_mapping_dict_given_index(self, which_index: int = DISPLAY_IDX) -> Dict[(str, Callable)]:
        mapping_dict = {}
...
class DisplayAndPostFormFunctionMaps:
    ...
    def get_display_form_name_for_parent_of_function(self, func: Callable) -> str:
        parent_display_func = [parent_func for parent_func, list_of_children in self.parent_child_mapping.items() if func in list_of_children]
        ...
        display_form_name = self.display_mappings.get_form_name_for_function(parent_display_func[0])
    ...
    def get_form_name_for_function(self, func: Callable):
        try:
            ...
            try:
                ...
                raise MissingFormName("Can't find function %s in display or post mappings, errors %s and %s" % (str(func), str(e1), str(e2)))
    ...
    def from_nested_dict_of_functions(cls, nested_dict: NestedDictOfMappings):
        ...
        parent_child_mapping = nested_dict.parent_child_dict_mapping_back_to_display_parent()

        display_mappings = FormNameFunctionNameMapping(mapping_dict=display_mapping_dict)
        post_mappings = FormNameFunctionNameMapping( mapping_dict=post_mapping_dict)

        return cls(display_mappings=display_mappings, post_mappings=post_mappings, parent_child_mapping=parent_child_mapping)
