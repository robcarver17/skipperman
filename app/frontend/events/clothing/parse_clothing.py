...
from app.backend.clothing.dict_of_clothing_for_event import change_clothing_size_for_cadet, change_colour_group_for_cadet, clear_colour_group_for_cadet, \
    distribute_colour_groups_at_event, NotEnoughColours
...
def save_clothing_data_for_cadet(
    ...
    new_size, new_colour = get_size_and_colour_from_form(interface=interface, cadet=cadet)
    ...
    if not new_colour == clothing.colour:
        change_colour_group_for_cadet(
            object_store=interface.object_store, event=event, cadet=cadet, colour=new_colour
        )

def get_size_and_colour_from_form(interface: abstractInterface, cadet: Cadet) -> Tuple[str,str]:
    cadet_id = cadet.id
...
def distribute_colour_groups(interface: abstractInterface):
    event = get_event_from_state(interface)
    try:
        distribute_colour_groups_at_event(object_store = interface.object_store, event=event)
