...
from app.data_access.sql.shared_column_names import VOLUNTEER_ID, DAY, DINGHY_ID, EVENT_ID
...
from app.objects.volunteers_and_cades_at_event_with_club_boat_with_ids import VolunteerAtEventWithClubDinghyWithId, \
...
class SqlDataListOfVolunteersAtEventWithClubDinghies(
    ...
    def allocate_club_dinghy_to_volunteer_on_day(self,
        ...
        if self.is_volunteer_already_allocated_on_day(event_id=event_id, day=day, volunteer_id=volunteer_id):
            if allow_overwrite:
                ...
                raise Exception("Can't allocate club dinghy for that day as already allocated and overwrite not permitted")
        else:
    ...
    def is_volunteer_already_allocated_on_day(self,
                                              event_id: str,
                                              day: Day,
                                              volunteer_id: str,
                                              ):
    ...
    def _allocate_club_dinghy_to_volunteer_on_day_no_checks(self,
        event_id: str,
        day: Day,
        volunteer_id: str,
        club_dinghy_id: str
        ...
        try:
            ...
            self._add_row_no_checks_or_commits(event_id=event_id, volunteer_and_boat=volunteer_and_boat)
    ...
    def _update_club_dinghy_for_volunteer_on_day_no_checks(self,
        event_id: str,
        day: Day,
        volunteer_id: str,
        club_dinghy_id: str
    ...
    def write(
        ...
        try:
            ...
            self.cursor.execute("DELETE FROM %s WHERE %s=%d " % (VOLUNTEERS_AND_CLUB_DINGHIES_TABLE,
                                                                 EVENT_ID,
                                                                 int(event_id)))
    ...
    def _add_row_no_checks_or_commits(self, event_id: str, volunteer_and_boat: VolunteerAtEventWithClubDinghyWithId):
