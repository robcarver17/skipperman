...
class WAEventMapping(object):
    ...
    def from_df(cls, some_df: pd.DataFrame):
        try:
            events_list = some_df[EVENT_ID_LABEL]
            wa_list = some_df[WA_ID_LABEL]
        except KeyError:
            raise Exception("WA/Event mapping needs to have columns %s and %s" % (
                EVENT_ID_LABEL, WA_ID_LABEL
            ))

        return cls(list_of_event_ids=events_list,
                   list_of_wa_ids=wa_list)

    def to_df(self) -> pd.DataFrame:
        return pd.DataFrame({EVENT_ID_LABEL: self.list_of_event_ids,
                             WA_ID_LABEL: self.list_of_wa_ids})
    ...
    def is_event_in_mapping_list(self, event_id: str) -> bool:
        return event_id in self.list_of_event_ids

    def is_wa_id_in_mapping_list(self, wa_id: str) -> bool:
        return wa_id in self.list_of_wa_ids
