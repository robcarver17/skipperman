...
from app.frontend.administration.data.deleting_volunteers_process import set_volunteer_to_delete_in_state, \
    get_volunteer_to_delete_from_state, display_deleting_volunteer_process
from app.frontend.administration.data.merging_volunteers_process import set_volunteer_to_merge_with_in_state
from app.frontend.form_handler import button_error_and_back_to_initial_state_form
from app.frontend.shared.buttons import is_button_volunteer_selection, volunteer_from_button_pressed
...
from app.frontend.volunteers.ENTRY_view_volunteers import get_list_of_volunteers_with_buttons
...
def post_form_merge_delete_volunteers(interface: abstractInterface) -> Union[Form, NewForm]:
    button_pressed = interface.last_button_pressed()
    ## note we don't need to handle help and menu buttons

    if back_menu_button.pressed(button_pressed):
        interface.flush_cache_to_store()
        return interface.get_new_display_form_for_parent_of_function(display_form_merge_delete_volunteers)
...
def view_specific_volunteer_form(interface: abstractInterface):
    ...
    return interface.get_new_form_given_function(display_form_merge_delete_individual_volunteer)
...
def post_form_merge_delete_individual_volunteer(interface: abstractInterface) -> Union[Form, NewForm]:
    button_pressed = interface.last_button_pressed()
    ## note we don't need to handle help and menu buttons
...
def launch_merge_volunteer_process(interface: abstractInterface)-> Union[Form, NewForm]:
    volunteer_to_merge_with = volunteer_from_button_pressed(value_of_button_pressed=interface.last_button_pressed(),
                                      object_store=interface.object_store)
    set_volunteer_to_merge_with_in_state(interface=interface, volunteer=volunteer_to_merge_with)
