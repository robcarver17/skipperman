...
from app.backend.registration_data.identified_volunteers_at_event import \
    list_of_identified_volunteers_with_volunteer_id, get_list_of_relevant_information_for_volunteer_in_registration_data
from app.backend.volunteers.connected_cadets import get_list_of_cadets_associated_with_volunteer, \
    add_list_of_cadets_to_volunteer_connection, is_cadet_already_associated_with_volunteer, \
...
def update_cadet_connections_when_volunteer_already_at_event(
    ...
    list_of_associated_cadets_in_mapped_data = (
    ...
    update_cadet_connections_for_volunteer_already_at_event_given_list_of_cadets(
        object_store=object_store, volunteer=volunteer, list_of_cadets_to_connect=list_of_associated_cadets_in_mapped_data
    )


def update_cadet_connections_for_volunteer_already_at_event_given_list_of_cadets(
    object_store: ObjectStore, volunteer: Volunteer, list_of_cadets_to_connect: ListOfCadets
):
...
def get_list_of_active_associated_cadets_in_mapped_event_data_given_identified_volunteer(
    ...
    list_of_active_cadets_for_volunteer = [
        cadet for cadet in active_cadets if cadet.id in unique_list_of_all_cadet_ids_for_volunteer
    ]
...
def get_unique_list_of_cadet_ids_in_registration_data_given_identified_volunteer(
    ...
    identified_cadets_at_event = (
        get_list_of_identified_cadets_at_event(object_store=object_store, event=event)
    )
...
def get_list_of_volunteers_associated_with_cadet_at_event(
    ...
    list_of_volunteers = get_list_of_volunteers_associated_with_cadet(object_store=object_store, cadet=cadet)
    volunteer_data = get_dict_of_all_event_data_for_volunteers(object_store=object_store, event=event)
    volunteers_at_event =        volunteer_data.dict_of_registration_data_for_volunteers_at_event.list_of_volunteers_at_event()
    volunteers = [
        volunteer for volunteer in list_of_volunteers if volunteer in volunteers_at_event
    ]
...
def get_cadet_location_string_for_volunteer(
        ...
        cadets_at_event_data: DictOfAllEventInfoForCadet,
...
def list_of_cadet_groups_associated_with_volunteer(
    ...
    list_of_cadets = volunteer_data_at_event.associated_cadets
    list_of_groups = []
    for cadet in list_of_cadets:
        list_of_groups+= cadets_at_event_data.dict_of_cadets_with_days_and_groups.get_days_and_groups_for_cadet(cadet).list_of_groups
...
def get_list_of_cadets_associated_with_volunteer_at_event(
    ...
    list_of_cadets = get_list_of_cadets_associated_with_volunteer(volunteer=volunteer, object_store=object_store)
    ...
    list_of_cadets_at_event = [cadet for cadet in list_of_cadets if cadet in active_cadets_at_event]
