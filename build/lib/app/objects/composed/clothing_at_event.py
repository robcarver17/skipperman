...
from app.objects.clothing import UNALLOCATED_COLOUR, UNALLOCATED_SIZE, ListOfCadetsWithClothingAndIdsAtEvent
...
class DictOfCadetsWithClothingAtEvent(Dict[Cadet, ClothingAtEvent]):
    def __init__(self, raw_dict: Dict[Cadet, ClothingAtEvent], list_of_cadets_with_clothing_and_ids: ListOfCadetsWithClothingAndIdsAtEvent):
    ...
    def as_list(self) -> ListOfCadetsWithClothingAtEvent:
        return ListOfCadetsWithClothingAtEvent([
            CadetWithClothingAtEvent.from_dict_tuple(cadet_with_clothing) for cadet_with_clothing in self.items()
        ])
    ...
    def change_colour_group_for_cadet(
            self, cadet: Cadet, colour: str
    ):
        clothing_for_cadet = self.clothing_for_cadet(cadet)
        clothing_for_cadet.colour= colour
        self.list_of_cadets_with_clothing_and_ids.change_colour_group_for_cadet(cadet_id=cadet.id, colour=colour)

    def remove_clothing_for_cadet_at_event(self,
                                           cadet: Cadet,
                                           ):
    ...
    def clear_colour_group_for_cadet(
        ...
        self.list_of_cadets_with_clothing_and_ids.clear_colour_group_for_cadet(cadet_id=cadet.id)

    def change_clothing_size_for_cadet(
            self,
            cadet: Cadet, size: str
    ):
        clothing_for_cadet = self.clothing_for_cadet(cadet)
        clothing_for_cadet.size = size
        self.list_of_cadets_with_clothing_and_ids.change_clothing_size_for_cadet(cadet_id=cadet.id, size=size)
    ...
    def filter_for_list_of_cadets(self, list_of_cadets: ListOfCadets):
        raw_dict = dict([(cadet, self.clothing_for_cadet(cadet)) for cadet in list_of_cadets])
        return self._create_with_new_raw_dict(raw_dict)

    def filter_for_colour(self, colour: str):
        raw_dict = dict([(cadet, clothing) for cadet, clothing in self.items() if clothing.colour==colour])
    ...
    def _create_with_new_raw_dict(self, raw_dict: Dict[Cadet, ClothingAtEvent]):
        ...
        subset_list_of_cadets_with_clothing_and_ids = self.list_of_cadets_with_clothing_and_ids.filter_for_list_of_cadet_ids(list_of_ids)
    ...
    def remove_if_in_list_of_cadets(self, list_of_cadets: ListOfCadets):
        raw_dict = dict([(cadet, clothing) for cadet, clothing in self.items() if cadet not in list_of_cadets])
        return self._create_with_new_raw_dict(raw_dict)

    def filter_for_surname(self, surname: str):
        raw_dict = dict([(cadet, clothing) for cadet, clothing in self.items() if cadet.surname ==surname])
        return self._create_with_new_raw_dict(raw_dict)
    ...
    @property
    def list_of_cadets_with_clothing_and_ids(self) -> ListOfCadetsWithClothingAndIdsAtEvent:
...
def compose_dict_of_cadets_with_clothing_at_event(list_of_cadets: ListOfCadets,
