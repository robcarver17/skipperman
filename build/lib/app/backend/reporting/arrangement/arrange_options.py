...
class ArrangeGroupsOptions:
    ...
    def change_arrangement_options_given_new_method(self, arrangement_method: ArrangementMethod):
        self._arrangement_method = arrangement_method
...
class ArrangementOptionsAndGroupOrder:
    ...
    @classmethod
    def from_group_order_and_column_arrangements(cls,
                                                 group_order: GroupOrder,
                                                 arrangement_of_columns: ArrangementOfColumns):
    ...
    def remove_empty_groups_from_group_order_and_arrangement(self, empty_groups:GroupOrder):
    ...
    def add_missing_groups_to_group_order_and_arrangement(self, missing_groups: GroupOrder,
                                                          ):
        current_group_order = self.group_order
        current_columns = self.arrangement_options.arrangement_of_columns  ###uses indices
        for group in missing_groups:
            if group not in current_group_order:
                current_group_order.append(group)
                index_of_group_in_group_order = current_group_order.index(group) ## should be a new one
    ...
    def subset_if_in_other_group_order(self, other_group_order: List):
        ...
        for group in starting_group_order:
            if group not in other_group_order:
                ...
                current_columns.remove_value_and_reset_indices(index_of_group_in_group_order)
    ...
    def to_df_of_str(self) -> pd.DataFrame:
        ...
        return pd.DataFrame(dict(method=[method], columns=[columns], group_order=[group_order]))
    ...
    def from_df_of_str(cls, df: pd.DataFrame):
        ...
        return cls(arrangement_options=ArrangeGroupsOptions(
            arrangement_method=method,
            arrangement_of_columns=columns),
                   group_order=group_order)
