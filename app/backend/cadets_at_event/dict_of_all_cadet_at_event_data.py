...
def get_health_notes_for_list_of_cadets_at_event(
    ...
    for cadet_at_event in list_of_cadets:
        registration_data = get_registration_data_for_single_cadet_at_event(
            object_store=object_store, event=event, cadet=cadet_at_event
        )
...
def get_dict_of_all_event_info_for_cadets(
    ...
    return object_store.get(
        object_store.data_api.dict_of_all_event_data_for_cadets.get_dict_of_all_event_info_for_cadets,
        event=event,
    )
...
def delete_cadet_from_event_and_return_messages(
    ...
    messages += interface.update(
        interface.object_store.data_api.data_cadets_at_event.delete_cadet_from_event_and_return_messages,
        event_id=event.id,
        cadet_id=cadet.id,
    )

    messages += interface.update(
        interface.object_store.data_api.data_identified_cadets_at_event.delete_cadet_from_identified_data_and_return_messages,
        event_id=event.id,
        cadet_id=cadet.id,
    )
