...
class SqlDataListOfCadetsWithClothingAtEvent(
    ...
    def delete_cadet_from_event_and_return_messages(self,  event_id: str, cadet_id:str):
        if not self.does_cadet_have_clothing_record_at_event(event_id=event_id, cadet_id=cadet_id):
            return []

        self.remove_clothing_requirements_for_cadet_at_event(event_id=event_id, cadet_id=cadet_id)

        return ["Removed clothing requirements for cadet %s at event %s." % (cadet_id, event_id)]
    ...
    def does_cadet_have_clothing_record_at_event(self, event_id: str, cadet_id:str):
        try:
            ...
            raise Exception("Error %s when reading cadet clothing table event# %s" % (str(e1), event_id))
        finally:
            self.close()
    ...
    def remove_clothing_requirements_for_cadet_at_event(self, event_id: str, cadet_id: str):
        try:
            if self.table_does_not_exist(CADET_WITH_CLOTHING_AT_EVENT_TABLE):
                self.create_table()
    ...
    def write(
        ...
        try:
            ...
            self.cursor.execute("DELETE FROM %s WHERE %s=%d" % (CADET_WITH_CLOTHING_AT_EVENT_TABLE, EVENT_ID, int(event_id)))

            for cadet_with_clothing in list_of_cadets_with_clothing:
                self._write_row_without_checks_or_commits(event_id=event_id, cadet_with_clothing=cadet_with_clothing)
    ...
    def _write_row_without_checks_or_commits(self, event_id: str, cadet_with_clothing: CadetWithClothingAndIdsAtEvent):
