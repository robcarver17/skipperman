...
from app.objects_OLD.events import SORT_BY_START_DSC, ListOfEvents
...
def display_form_for_event_cleaning_sort_order_passed(
    ...
    contents_of_form = ListOfLines(
        [
            ...
            Heading(
                "WARNING CANNOT BE UNDONE EXCEPT VIA BACKUP RESTORE! You will not be asked to confirm your choice.",
                size=4,
            ),
            _______________,
            sort_buttons_for_event_list,
...
def action_when_event_button_clicked(interface: abstractInterface) -> Form:
    ...
    return form_with_message_and_finished_button(
        "Cleaned data for event %s" % str(event),
        interface=interface,
