from app.objects.cadets import ListOfCadets
from app.objects.composed.people_at_event_with_club_dinghies import (
    DictOfPeopleAndClubDinghiesAtEvent,
    DictOfDaysAndClubDinghiesAtEventForPerson,
)
from app.objects.volunteers_and_cades_at_event_with_club_boat_with_ids import (
    ListOfCadetAtEventWithIdAndClubDinghies,
    CadetAtEventWithClubDinghyWithId,

)
from app.objects.club_dinghies import no_club_dinghy_id
from app.data_access.sql.generic_sql_data import GenericSqlData
from app.data_access.sql.shared_column_names import *
from app.objects.day_selectors import Day

"""
CADETS AT EVENT WITH CLUB DINGHIES
"""

CADETS_AND_CLUB_DINGHIES_TABLE = "cadets_and_club_dinghies_table"
INDEX_NAME_CADETS_AND_CLUB_DINGHIES_TABLE = "cadets_and_club_dinghies_table_index"


class SqlDataListOfCadetAtEventWithClubDinghies(GenericSqlData):
    def update_or_add_cadet_with_club_dinghy_on_day(
        self, event_id: str, cadet_id: str, day: Day, club_dinghy_id: str
    ):
        if club_dinghy_id == no_club_dinghy_id:
            return self.set_club_dinghy_on_day_for_cadet_as_no_club_dinghy(
                event_id=event_id, cadet_id=cadet_id, day=day
            )
        elif self.is_a_club_dinghy_allocated_for_cadet_on_day_of_event(
            event_id=event_id, cadet_id=cadet_id, day=day
        ):
            self._update_club_dinghy_allocated_for_cadet_on_day_of_event_no_checks(
                event_id=event_id,
                cadet_id=cadet_id,
                day=day,
                club_dinghy_id=club_dinghy_id,
            )
        else:
            self._add_club_dinghy_allocated_for_cadet_on_day_of_event_without_checks(
                event_id=event_id,
                cadet_id=cadet_id,
                day=day,
                club_dinghy_id=club_dinghy_id,
            )

    def set_club_dinghy_on_day_for_cadet_as_no_club_dinghy(
        self, event_id: str, cadet_id: str, day: Day
    ):
        if self.is_a_club_dinghy_allocated_for_cadet_on_day_of_event(
            event_id=event_id, cadet_id=cadet_id, day=day
        ):
            self.delete_club_dinghy_allocated_for_cadet_on_day_of_event(
                event_id=event_id, cadet_id=cadet_id, day=day
            )
        else:
            return

    def _update_club_dinghy_allocated_for_cadet_on_day_of_event_no_checks(
        self, event_id: str, cadet_id: str, day: Day, club_dinghy_id: str
    ):
        try:
            if self.table_does_not_exist(CADETS_AND_CLUB_DINGHIES_TABLE):
                return

            self.cursor.execute(
                "UPDATE %s SET %s='%d' WHERE %s='%s' AND %s=%d AND %s=%d "
                % (
                    CADETS_AND_CLUB_DINGHIES_TABLE,
                    DINGHY_ID,
                    int(club_dinghy_id),
                    DAY,
                    day.name,
                    EVENT_ID,
                    int(event_id),
                    CADET_ID,
                    int(cadet_id),
                )
            )

            self.conn.commit()
        except Exception as e1:
            raise Exception("Error %s when writing club dinghies at event" % str(e1))
        finally:
            self.close()

    def _add_club_dinghy_allocated_for_cadet_on_day_of_event_without_checks(
        self, event_id: str, cadet_id: str, day: Day, club_dinghy_id: str
    ):
        try:
            if self.table_does_not_exist(CADETS_AND_CLUB_DINGHIES_TABLE):
                self.create_table()
            self._add_cadet_with_club_dinghy_without_commits_or_checks(
                event_id=event_id,
                cadet_and_boat=CadetAtEventWithClubDinghyWithId(
                    cadet_id=cadet_id, club_dinghy_id=club_dinghy_id, day=day
                ),
            )
            self.conn.commit()
        except Exception as e1:
            raise Exception("Error %s when writing club dinghies at event" % str(e1))
        finally:
            self.close()

    def delete_club_dinghy_allocated_for_cadet_on_day_of_event(
        self, event_id: str, cadet_id: str, day: Day
    ):
        try:
            if self.table_does_not_exist(CADETS_AND_CLUB_DINGHIES_TABLE):
                return

            ## NEEDS TO DELETE OLD
            ## TEMPORARY UNTIL CAN DO PROPERLY
            self.cursor.execute(
                "DELETE FROM %s WHERE %s='%s' AND %s=%d AND %s=%d "
                % (
                    CADETS_AND_CLUB_DINGHIES_TABLE,
                    DAY,
                    day.name,
                    EVENT_ID,
                    int(event_id),
                    CADET_ID,
                    int(cadet_id),
                )
            )

            self.conn.commit()
        except Exception as e1:
            raise Exception("Error %s when writing club dinghies at event" % str(e1))
        finally:
            self.close()

    def delete_cadet_from_event_and_return_messages(self, event_id: str, cadet_id: str):
        if not self.is_a_club_dinghy_allocated_for_cadet_on_any_day_at_event(
            event_id=event_id, cadet_id=cadet_id
        ):
            return []

        self.remove_all_allocations_for_cadet_at_event(
            event_id=event_id, cadet_id=cadet_id
        )

        return ["Removed club dinghies for cadet %s at event %s" % (cadet_id, event_id)]

    def remove_all_allocations_for_cadet_at_event(self, event_id: str, cadet_id: str):
        try:
            if self.table_does_not_exist(CADETS_AND_CLUB_DINGHIES_TABLE):
                return

            ## NEEDS TO DELETE OLD
            ## TEMPORARY UNTIL CAN DO PROPERLY
            self.cursor.execute(
                "DELETE FROM %s WHERE %s=%d AND %s=%d "
                % (
                    CADETS_AND_CLUB_DINGHIES_TABLE,
                    EVENT_ID,
                    int(event_id),
                    CADET_ID,
                    int(cadet_id),
                )
            )

            self.conn.commit()
        except Exception as e1:
            raise Exception("Error %s when writing club dinghies at event" % str(e1))
        finally:
            self.close()

    def is_a_club_dinghy_allocated_for_cadet_on_day_of_event(
        self, event_id: str, cadet_id: str, day: Day
    ) -> bool:
        if self.table_does_not_exist(CADETS_AND_CLUB_DINGHIES_TABLE):
            return False

        try:
            cursor = self.cursor
            cursor.execute(
                """SELECT * FROM %s WHERE %s=%d AND %s=%d AND %s='%s' """
                % (
                    CADETS_AND_CLUB_DINGHIES_TABLE,
                    EVENT_ID,
                    int(event_id),
                    CADET_ID,
                    int(cadet_id),
                    DAY,
                    day.name,
                )
            )

            raw_list = cursor.fetchall()
        except Exception as e1:
            raise Exception(
                "Error %s when reading cadets and dinghies at event" % str(e1)
            )
        finally:
            self.close()

        return len(raw_list) > 0

    def is_a_club_dinghy_allocated_for_cadet_on_any_day_at_event(
        self, event_id: str, cadet_id: str
    ) -> bool:
        if self.table_does_not_exist(CADETS_AND_CLUB_DINGHIES_TABLE):
            return False

        try:
            cursor = self.cursor
            cursor.execute(
                """SELECT * FROM %s WHERE %s=%d AND %s=%d """
                % (
                    CADETS_AND_CLUB_DINGHIES_TABLE,
                    EVENT_ID,
                    int(event_id),
                    CADET_ID,
                    int(cadet_id),
                )
            )

            raw_list = cursor.fetchall()
        except Exception as e1:
            raise Exception(
                "Error %s when reading cadets and dinghies at event" % str(e1)
            )
        finally:
            self.close()

        return len(raw_list) > 0

    def read_dict_of_cadets_and_club_dinghies_at_event(
        self, event_id: str
    ) -> DictOfPeopleAndClubDinghiesAtEvent:
        list_of_cadets_at_event_with_dinghies = self.read(event_id)
        new_dict = {}
        for cadet_with_id_and_dinghy in list_of_cadets_at_event_with_dinghies:
            cadet = self.list_of_cadets.cadet_with_id(cadet_with_id_and_dinghy.cadet_id)
            day = cadet_with_id_and_dinghy.day
            dinghy = self.list_of_club_dinghies.club_dinghy_with_id(
                cadet_with_id_and_dinghy.club_dinghy_id
            )
            existing_dict_of_days = new_dict.get(
                cadet, DictOfDaysAndClubDinghiesAtEventForPerson()
            )
            existing_dict_of_days[day] = dinghy
            new_dict[cadet] = existing_dict_of_days

        return DictOfPeopleAndClubDinghiesAtEvent(new_dict)

    @property
    def list_of_cadets(self) -> ListOfCadets:
        list_of_cadets = self.object_store.get(
            self.object_store.data_api.data_list_of_cadets.read
        )

        return list_of_cadets

    @property
    def list_of_club_dinghies(self):
        list_of_club_dinghies = self.object_store.get(
            self.object_store.data_api.data_List_of_club_dinghies.read
        )

        return list_of_club_dinghies

    def read(self, event_id: str) -> ListOfCadetAtEventWithIdAndClubDinghies:
        if self.table_does_not_exist(CADETS_AND_CLUB_DINGHIES_TABLE):
            self.create_table()

        try:
            cursor = self.cursor
            cursor.execute(
                """SELECT %s, %s, %s  FROM %s WHERE %s=%d """
                % (
                    CADET_ID,
                    DAY,
                    DINGHY_ID,
                    CADETS_AND_CLUB_DINGHIES_TABLE,
                    EVENT_ID,
                    int(event_id),
                )
            )

            raw_list = cursor.fetchall()
        except Exception as e1:
            raise Exception(
                "Error %s when reading cadets and dinghies at event" % str(e1)
            )
        finally:
            self.close()

        new_list = []
        for raw_cadet_with_id_at_event in raw_list:
            cadet_with_id_at_event = CadetAtEventWithClubDinghyWithId(
                cadet_id=str(
                    raw_cadet_with_id_at_event[0],
                ),
                day=Day[raw_cadet_with_id_at_event[1]],
                club_dinghy_id=str(raw_cadet_with_id_at_event[2]),
            )
            new_list.append(cadet_with_id_at_event)

        return ListOfCadetAtEventWithIdAndClubDinghies(new_list)  ## IGNORE warning

    def write(
        self,
        people_and_boats: ListOfCadetAtEventWithIdAndClubDinghies,
        event_id: str,
    ):
        try:
            if self.table_does_not_exist(CADETS_AND_CLUB_DINGHIES_TABLE):
                self.create_table()

            ## NEEDS TO DELETE OLD
            ## TEMPORARY UNTIL CAN DO PROPERLY
            self.cursor.execute(
                "DELETE FROM %s WHERE %s=%d "
                % (CADETS_AND_CLUB_DINGHIES_TABLE, EVENT_ID, int(event_id))
            )

            for cadet_and_boat in people_and_boats:
                self._add_cadet_with_club_dinghy_without_commits_or_checks(
                    event_id=event_id, cadet_and_boat=cadet_and_boat
                )

            self.conn.commit()
        except Exception as e1:
            raise Exception("Error %s when writing club dinghies at event" % str(e1))
        finally:
            self.close()

    def _add_cadet_with_club_dinghy_without_commits_or_checks(
        self, event_id: str, cadet_and_boat: CadetAtEventWithClubDinghyWithId
    ):
        cadet_id = cadet_and_boat.cadet_id
        day = cadet_and_boat.day.name
        club_dinghy_id = cadet_and_boat.club_dinghy_id

        insertion = "INSERT INTO %s (%s, %s, %s, %s) VALUES (?,?,?,?)" % (
            CADETS_AND_CLUB_DINGHIES_TABLE,
            EVENT_ID,
            CADET_ID,
            DAY,
            DINGHY_ID,
        )

        self.cursor.execute(
            insertion, (int(event_id), int(cadet_id), day, int(club_dinghy_id))
        )

    def create_table(self):
        # name: str
        # hidden: bool
        # id: str = arg_not_passed

        table_creation_query = """
            CREATE TABLE %s (
                %s INTEGER, 
                %s INTEGER,
                %s STR,
                %s INTEGER
            );
        """ % (
            CADETS_AND_CLUB_DINGHIES_TABLE,
            EVENT_ID,
            CADET_ID,
            DAY,
            DINGHY_ID,
        )

        index_creation_query = "CREATE UNIQUE INDEX %s ON %s (%s, %s, %s)" % (
            INDEX_NAME_CADETS_AND_CLUB_DINGHIES_TABLE,
            CADETS_AND_CLUB_DINGHIES_TABLE,
            EVENT_ID,
            CADET_ID,
            DAY,
        )

        try:
            self.cursor.execute(table_creation_query)
            self.cursor.execute(index_creation_query)
            self.conn.commit()
        except Exception as e1:
            raise Exception(
                "Error %s when creating club dinghies and cadets table" % str(e1)
            )
        finally:
            self.close()
