...
def save_field_mapping_for_event(
    ...
    return object_store.update(
        object_definition=object_definition_for_field_mappings_at_event, event_id=event.id,
        new_object=mapping
    )
...
def get_list_of_events_with_field_mapping(
    ...
    list_of_events = [
        ...
        if is_wa_field_mapping_setup_for_event(object_store=interface.object_store, event=event)
    ]
    list_of_events = [
