...
class SqlDataListOfVolunteersAtEvent(GenericSqlData):
    ...
    def make_volunteer_available_on_day(
        ...
        self._set_availability_on_day_for_volunteer(volunteer_id=volunteer_id,
                                                   event_id=event_id,
                                                   day=day,
                                                   available=True)
    ...
    def _set_availability_on_day_for_volunteer(self, volunteer_id: str, event_id:str, day: Day, available: bool):
        if not self.is_volunteer_already_at_event(volunteer_id=volunteer_id, event_id=event_id):
            raise Exception("volunteer not at event")

        current_availability = self.read_for_volunteer(event_id=event_id, volunteer_id=volunteer_id).availablity
        if available:
        ...
        try:
            ...
            self.cursor.execute("UPDATE %s SET %s='%s' WHERE %s=%d AND %s=%d " %
                                (VOLUNTEERS_AT_EVENT_TABLE,
                                 VOLUNTEER_AVAILABLITY, current_availability.as_str(),
    ...
    def delete_volunteer_at_event(
        ...
        try:
            ...
            self.cursor.execute("DELETE FROM %s WHERE %s=%d AND %s=%d" % (VOLUNTEERS_AT_EVENT_TABLE, EVENT_ID, int(event_id),
                                                                          VOLUNTEER_ID, int(volunteer_id)))
    ...
    def add_volunteer_to_event(self, event_id: str, volunteer_at_event_with_id: VolunteerAtEventWithId):
        if self.is_volunteer_already_at_event(event_id=event_id, volunteer_id=volunteer_at_event_with_id.volunteer_id):
            raise Exception("Can't add as at event already")

        self._add_volunteer_to_event_without_checks(event_id=event_id, volunteer_at_event_with_id=volunteer_at_event_with_id)

    def _add_volunteer_to_event_without_checks(self, event_id: str, volunteer_at_event_with_id: VolunteerAtEventWithId):
        try:
            if self.table_does_not_exist(VOLUNTEERS_AT_EVENT_TABLE):
                self.create_table()

            self._add_row_without_checks_or_commits(event_id=event_id, volunteer_at_event=volunteer_at_event_with_id)
    ...
    def update_volunteer_notes_at_event(self,
        event_id: str,
    volunteer_id: str,
    new_notes: str):
        try:
    ...
    def get_dict_of_registration_data_for_volunteers_at_event(
        ...
        for raw_item in raw_data:
            volunteer = self.list_of_volunteeers.volunteer_with_id(raw_item.volunteer_id)
            reg_data = RegistrationDataForVolunteerAtEvent.from_volunteer_at_event_with_id(
    ...
    def read_for_volunteer(self, event_id: str, volunteer_id: str, default = missing_data) -> VolunteerAtEventWithId:
        try:
            if self.table_does_not_exist(VOLUNTEERS_AT_EVENT_TABLE):
                return default

            cursor = self.cursor
            cursor.execute('''SELECT %s, %s, %s, %s, %s, %s FROM %s WHERE %s=%d AND %s=%d ''' % (
                VOLUNTEER_NOTES,
    ...
    def read(self, event_id: str) -> ListOfVolunteersAtEventWithId:
        try:
            ...
            cursor = self.cursor
            cursor.execute('''SELECT %s, %s, %s, %s, %s, %s, %s FROM %s WHERE %s=%d ''' % (
                VOLUNTEER_ID,
                VOLUNTEER_AVAILABLITY,
                PREFERRED_DUTIES,
                SAME_OR_DIFFERENT_DUTIES,
    ...
    def write(
        ...
        try:
            ...
            self.cursor.execute("DELETE FROM %s WHERE %s=%d" % (VOLUNTEERS_AT_EVENT_TABLE, EVENT_ID, int(event_id)))

            for volunteer_at_event in list_of_volunteers_at_event:
                self._add_row_without_checks_or_commits(event_id=event_id, volunteer_at_event=volunteer_at_event)
    ...
    def _add_row_without_checks_or_commits(self, event_id: str, volunteer_at_event: VolunteerAtEventWithId):
