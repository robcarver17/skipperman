...
from app.logic.events.constants import ATTENDANCE, ALLOCATION, CLUB_BOAT, BOAT_CLASS, SAIL_NUMBER, PARTNER
...
def get_days_attending_field(cadet: Cadet, allocation_data: AllocationData)-> checkboxInput:
    ...
    days_attending_field = get_availability_checkbox(availability=allocation_data.cadet_availability_at_event(cadet),
                                     ...
                                     input_name=input_name_from_column_name_and_cadet_id(column_name=ATTENDANCE,
                                                                                         cadet_id=cadet.id),
                                     line_break=True)
...
def get_input_fields_for_cadet_across_days(cadet: Cadet,  allocation_data: AllocationData) -> list:
    ...
    dropdown_input_for_partner_allocation= get_dropdown_input_for_partner_allocation_across_days(cadet=cadet, allocation_data=allocation_data)
    sail_number_field = get_sail_number_field_across_days(cadet=cadet, allocation_data=allocation_data)
...
def get_input_fields_for_cadet_on_day(cadet: Cadet, day: Day, allocation_data: AllocationData) -> list:
    ...
    if not allocation_data.is_cadet_available_on_this_day(cadet=cadet, day=day):
        padding = ['']*4
        return [
            Button(label=MAKE_CADET_AVAILABLE_ON_DAY_BUTTON, value=make_cadet_available_button_name(cadet))]+padding
...
def get_dropdown_input_for_group_allocation_across_days(cadet: Cadet,  allocation_data: AllocationData) -> Union[dropDownInput, str]:
    current_group = allocation_data.get_current_group_name_across_days_or_none_if_different(cadet=cadet)
...
def get_dropdown_input_for_group_allocation(cadet: Cadet, current_group: str, allocation_data: AllocationData) -> Union[dropDownInput, str]:
...
def get_dropdown_input_for_club_boat_allocation_across_days(cadet: Cadet,  allocation_data: AllocationData) -> Union[dropDownInput, str]:
    current_club_boat = allocation_data.get_current_club_boat_name_across_days_or_none_if_different(cadet=cadet)
    if current_club_boat is None:
        return allocation_data.get_string_describing_different_club_boats_across_days(cadet=cadet)
...
def get_dropdown_input_for_club_boat_allocation(cadet: Cadet, current_club_boat: str,  allocation_data: AllocationData) -> dropDownInput:
...
def get_dropdown_input_for_boat_class_allocation_across_days(cadet: Cadet, allocation_data: AllocationData) -> dropDownInput:
    current_boat_class = allocation_data.get_current_boat_class_across_days_or_none_if_different(cadet=cadet)
    if current_boat_class is None:
        return allocation_data.get_string_describing_different_boat_class_across_days(cadet=cadet)
...
def get_dropdown_input_for_boat_class_allocation(cadet: Cadet, current_boat_class: str,  allocation_data: AllocationData) -> dropDownInput:
...
def get_sail_number_field_across_days(cadet: Cadet,  allocation_data: AllocationData)-> textInput:
    current_number = allocation_data.get_current_sail_number_across_days_or_none_if_different(cadet=cadet)
    if current_number is None:
        return allocation_data.get_string_describing_different_sail_numbers_across_days(cadet=cadet)
    return get_sail_number_field(
        cadet=cadet,
...
def get_dropdown_input_for_partner_allocation_across_days(cadet: Cadet,  allocation_data: AllocationData) -> ListOfLines:
    current_partner_name = allocation_data.get_two_handed_partner_name_for_cadet_across_days_or_none_if_different(cadet=cadet)
    if current_partner_name is None:
        return allocation_data.get_string_describing_two_handed_partner_name_across_days(cadet=cadet)

    list_of_other_cadets = allocation_data.list_of_names_of_cadets_at_event_with_matching_schedules_excluding_this_cadet(cadet) ### needs to disapply cadets who aren't also available the whole week
...
def okay_to_have_partner_button_across_days(cadet: Cadet, allocation_data: AllocationData)-> bool:
    registration_split_allowed = allocation_data.event.reg_splitting_allowed
    boat_exists = allocation_data.cadet_at_event_with_dinghy_object_already_exists_for_cadet_across_days(cadet=cadet)
...
def get_dropdown_input_for_partner_allocation_on_day(cadet: Cadet, day: Day, allocation_data: AllocationData) -> ListOfLines:
    ...
    list_of_other_cadets = allocation_data.list_of_names_of_cadets_at_event_excluding_cadet_available_on_day(cadet=cadet, day=day)

    okay_to_have_partner_button = okay_to_have_partner_button_on_day(cadet_id=cadet.id, allocation_data=allocation_data, day=day)
...
def get_dropdown_input_for_partner_allocation(cadet: Cadet, list_of_other_cadets: List[str], current_partner_name: str,
                                              okay_to_have_partner_button: bool) -> ListOfLines:
    list_of_other_cadets = NO_PARTNERSHIP_LIST+list_of_other_cadets
    ...
    if okay_to_have_partner_button:
        button_to_add_partner = Button(value=button_name_for_add_partner(cadet_id=cadet.id), label="Add partner as new cadet")
    else:
