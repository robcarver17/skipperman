...
from app.objects.composed.roles_and_teams import list_of_all_roles_not_already_in_team, DictOfTeamsWithRoles

from app.objects.abstract_objects.abstract_tables import RowInTable

from app.frontend.forms.reorder_form import reorder_table, list_of_button_names_given_group_order, reorderFormInterface
...
from app.backend.volunteers.roles_and_teams import  add_new_named_role_to_team, reorder_roles_for_team_given_list_of_names

from app.frontend.form_handler import (
    button_error_and_back_to_initial_state_form,
)
from app.frontend.configuration.generic_list_modifier import ( add_button,
)
...
def display_form_edit_individual_team_page(interface: abstractInterface) -> Form:
    ...
    names = get_list_of_current_role_names(dict_of_teams_and_roles=dict_of_teams_and_roles, team=team)
    navbar = ButtonBar([cancel_menu_button])
    heading = Heading("Add, remove or re-order members of volunteer team: %s " % team.name, centred=True, size=3)
    subhead = Heading("[First name is team leader]", centred=True, size=4)

    table = reorder_table(starting_list=names, include_delete=True)
    add_line = RowInTable([dropdown_to_add_volunteer_to_team(
...
def get_team_and_dict_of_teams_and_roles(interface: abstractInterface) -> Tuple[Team, DictOfTeamsWithRoles]:
...
def get_list_of_current_role_names(dict_of_teams_and_roles: DictOfTeamsWithRoles, team: Team):
...
def dropdown_to_add_volunteer_to_team(dict_of_teams_and_roles: DictOfTeamsWithRoles, team: Team) -> dropDownInput:
    all_roles = list_of_all_roles_not_already_in_team(dict_of_teams_and_roles=dict_of_teams_and_roles,
                                          team=team)
...
def post_form_edit_individual_team_page(
    ...
    team, dict_of_teams_and_roles = get_team_and_dict_of_teams_and_roles(interface)
    role_names = get_list_of_current_role_names(dict_of_teams_and_roles=dict_of_teams_and_roles, team=team)

    reordering_buttons = list_of_button_names_given_group_order(role_names)
    if cancel_menu_button.pressed(last_button):
        interface.clear_cache()
        return interface.get_new_display_form_for_parent_of_function(post_form_edit_individual_team_page)
        ...
        reorder_ordered_list_of_roles(interface=interface, role_names=role_names, team=team)
    else:
...
def reorder_ordered_list_of_roles(interface: abstractInterface, role_names: List[str], team: Team):
    ...
    try:
        reorder_roles_for_team_given_list_of_names(object_store = interface.object_store,
...
def add_new_role_to_team(interface: abstractInterface,  team: Team):
    ...
    add_new_named_role_to_team(object_store = interface.object_store,
                               team = team,
