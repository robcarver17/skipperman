...
class SqlListOfCadetsWithQualifications(GenericSqlData):
    ...
    def does_cadet_have_qualification(self, cadet_id: str, qualification_id: str):
        ...
        try:
            ...
            raise Exception(
                "Error %s when reading cadets with qualifications" % str(e1)
            )
        finally:
            self.close()
    ...
    def get_ordered_list_of_qualifications_for_cadet(
        ...
        list_of_qualifications_for_cadet = self.get_unordered_qualifications_for_cadet(
            cadet_id
        )
    ...
    def get_unordered_qualifications_for_cadet(
        self, cadet_id: str
    ) -> QualificationsForCadet:
        ...
        return QualificationsForCadet(
            [
                QualificationAndDate(
                    qualification=self.list_of_qualifications.qualification_given_id(
                        raw_item.qualification_id
                    ),
    ...
    def get_qualifications_for_cadet_with_ids(
        self, cadet_id: str
    ) -> ListOfCadetsWithIdsAndQualifications:
        if self.table_does_not_exist(CADETS_WITH_QUALIFICATION_TABLE):
    ...
    def _write_qualification_for_cadet_without_checks_or_commit(
        self, cadet_with_qualifications: CadetWithIdAndQualification
    ):
        ...
        self.cursor.execute(
            insertion, (cadet_id, qualification_id, qualification_date, awarded_by)
        )
    ...
    def create_table(self):
        ...
        index_creation_query = "CREATE UNIQUE INDEX %s ON %s (%s, %s)" % (
            INDEX_NAME_CADETS_WITH_QUALIFICATION_TABLE,
            CADETS_WITH_QUALIFICATION_TABLE,
            CADET_ID,
            QUALIFICATION_ID,
        )
        try:
            ...
            raise Exception(
                "Error %s when creating cadets with qualification table" % str(e1)
            )
