...
from app.frontend.configuration.generic_list_modifier import hide_button_for_entry, up_button_for_entry, \
    down_button_for_entry, hidden_box_name, \
    display_form_edit_generic_list, post_form_edit_generic_list, BACK_BUTTON_PRESSED, \
    BUTTON_NOT_KNOWN

from app.objects.composed.volunteer_roles import ListOfRolesWithSkills, RoleWithSkills
from app.objects.exceptions import arg_not_passed

from app.backend.volunteers.roles_and_teams import  get_list_of_roles_with_skills, update_list_of_roles_with_skills, modify_list_of_roles_with_skills, add_to_list_of_roles_with_skills

from app.objects.abstract_objects.abstract_form import Form, NewForm

from app.objects.abstract_objects.abstract_interface import abstractInterface
from app.frontend.forms.form_utils import yes_no_radio, checked_and_labels_dict_for_skills_form, is_radio_yes_or_no, \
...
def get_row_for_existing_entry(entry: RoleWithSkills, **ignored_kwargs) -> RowInTable:
    if entry.protected:
        ...
        return RowInTable([entry.name,
                      "Can associate with sailing group" if entry.associate_sailing_group else "Not associated with sailing group",
                    skills_str,
...
def associate_sailing_group_button_for_entry(entry: RoleWithSkills):
    ...
    return yes_no_radio(default_to_yes=default_to_yes,
...
def get_modified_role_from_form(interface: abstractInterface, existing_object: RoleWithSkills, **ignored_kwargs) -> RoleWithSkills:

    existing_role = existing_object

    new_role_name = interface.value_from_form(name_of_text_box_for_role(existing_role))
    new_associated_or_not = is_radio_yes_or_no(interface=interface, input_name=name_of_associate_group_for_role(existing_role))
    new_skills_dict = get_dict_of_skills_from_form(interface =interface, field_name=name_of_skills_checkbox_for_role(existing_role))
    new_hidden = is_radio_yes_or_no(interface=interface, input_name=hidden_box_name(existing_role))

    modified_role= RoleWithSkills(name=new_role_name,
...
def save_from_ordinary_list_of_roles(
    ...
    update_list_of_roles_with_skills(object_store=object_store,
                                     list_of_roles_with_skills=ListOfRolesWithSkills.from_list_of_roles_with_skills(list_of_roles_with_skills=new_list))
