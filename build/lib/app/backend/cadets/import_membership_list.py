...
from app.backend.file_handling import get_staged_adhoc_filename
from app.backend.file_handling import create_local_file_from_uploaded_and_return_filename, load_spreadsheet_file_and_clear_nans
from app.data_access.csv.generic_csv_data import write_object_as_csv_file, read_object_of_type
from app.objects.abstract_objects.abstract_interface import abstractInterface
from app.objects.cadets import ListOfCadets, Cadet
from app.objects.membership_status import current_member


def create_temp_file_with_list_of_cadets(interface: abstractInterface, file_marker_name: str) -> str:
    original_filename = create_local_file_from_uploaded_and_return_filename(interface=interface, file_marker_name=file_marker_name)
    try:
...
def cadet_from_row_in_imported_list(cadet_row: pd.Series, row_id: int) -> Cadet:
    ...
    return Cadet(
        first_name=first_name, surname=surname, date_of_birth=dob.date(), id=str(row_id),
...
DESCRIBE_ALL_FIELDS_IN_CADET_MEMBERSHIP_LIST_FILE = ", ".join(ALL_FIELDS_IN_CADET_MEMBERSHIP_LIST_FILE)
