...
class DictOfNotesForGroupsAtEvent(Dict[Group, str]):
    ...
    def update_group_notes(self, group: Group, notes: str):
        self[group] = notes
        self.list_of_group_notes_with_ids.update_or_add_note_for_event_and_id(
            event_id=self.event.id, group_id=group.id, notes=notes
        )
...
def compose_dict_of_notes_for_groups_at_event(
    ...
    return DictOfNotesForGroupsAtEvent(
        raw_dict=raw_dict,
        list_of_group_notes_with_ids=list_of_group_notes_with_ids,
