...
from app.logic.events.constants import USE_ORIGINAL_DATA_BUTTON_LABEL, USE_DATA_IN_FORM_BUTTON_LABEL, USE_NEW_DATA_BUTTON_LABEL, CADET_ID
...
from app.logic.events.import_wa.update_existing_master_event_data_forms import \
    display_form_for_update_to_existing_row_of_event_data, update_mapped_wa_event_data_with_form_data, \
    update_mapped_wa_event_data_with_new_data, any_important_difference_between_rows, get_current_cadet_id, \
    get_row_in_mapped_event_for_cadet_id, get_row_in_master_event_for_cadet_id

from app.logic.events.backend.update_master_event_data import get_row_of_master_event_from_mapped_row_with_idx_and_status, add_new_row_to_master_event_data,update_row_in_master_event_data
...
def iterative_process_updates_to_master_event_data(interface: abstractInterface) -> Union[Form, NewForm]:
...
def process_update_to_cadet_in_event_data(interface: abstractInterface,
                                             event: Event,
                                          cadet_id: str) -> Form:
    ...
    cadet_present_in_mapped_event_data = is_cadet_present_in_mapped_event_data(event=event, cadet_id=cadet_id)

    if cadet_present_in_mapped_event_data and cadet_already_present_in_master_data:
        ## consider duplicates
        return process_update_to_existing_cadet_in_event_data(event=event, cadet_id=cadet_id, interface=interface)
    elif cadet_present_in_mapped_event_data and not cadet_already_present_in_master_data:
        ## consider duplicates
        return process_update_to_cadet_new_to_master_data(event=event, cadet_id=cadet_id, interface=interface)
    elif not cadet_present_in_mapped_event_data and cadet_already_present_in_master_data:
        return process_update_to_deleted_cadet(event=event, cadet_id=cadet_id, interface=interface)
    else:
        interface.log_error("Cadet ID %d was next ID but now can't find in any file?")
        return initial_state_form


def process_update_to_existing_cadet_in_event_data(interface: abstractInterface,
                                             event: Event,
                                          cadet_id: str) -> Form:
    try:
        row_in_mapped_event = get_row_in_mapped_event_for_cadet_id(cadet_id=cadet_id, event=event)
    except DuplicateCadets:
        interface.log_message("ACTION REQUIRED: Cadet %s appears more than once in WA file with an active registration - ignoring for now - go to WA and cancel one of the registrations please!" % cadet_name_from_id(cadet_id))
        return iterative_process_updates_to_master_event_data(interface)

    existing_row_in_master_event  = get_row_in_master_event_for_cadet_id(event=event, cadet_id=cadet_id)
...
def process_update_to_cadet_new_to_master_data(interface: abstractInterface,
                                             event: Event,
                                          cadet_id: str) -> Form:
    print("Row in master data for cadet with id %s" % cadet_id)
    try:
        relevant_row = get_row_in_mapped_event_for_cadet_id(cadet_id=cadet_id, event=event)
    except DuplicateCadets:
        interface.log_message("ACTION REQUIRED: Cadet %s appears more than once in WA file with an active registration - ignoring for now - go to WA and cancel one of the registrations please!" % cadet_name_from_id(cadet_id))
        return iterative_process_updates_to_master_event_data(interface)

    add_new_row_to_master_event_data(event=event, row_in_mapped_wa_event_with_id=relevant_row)

    return iterative_process_updates_to_master_event_data(interface)


def process_update_to_deleted_cadet(interface: abstractInterface,
                                             event: Event,
                                          cadet_id: str) -> Form:
    existing_row_in_master_event = get_row_in_master_event_for_cadet_id(event=event, cadet_id=cadet_id)
...
def is_cadet_present_in_mapped_event_data(event: Event,
                                    cadet_id: str) -> bool:

    mapped_event = load_existing_mapped_wa_event_with_ids(event)
    cadet_present = mapped_event.is_cadet_id_in_event(
        cadet_id
    )
...
def process_update_to_existing_row_of_event_data(
    ...
    if not any_important_difference_between_rows(new_row_in_mapped_wa_event_with_status=new_row_in_mapped_wa_event_with_status,
                                                 existing_row_in_master_event=existing_row_in_master_event):
        ## nothing to do
...
def get_and_save_next_cadet_id(interface: abstractInterface) -> str:
    ...
    if current_id is missing_data:
        ...
        new_id = get_next_cadet_id_in_event_data(interface=interface, current_id=current_id)
...
def get_next_cadet_id_in_event_data(interface: abstractInterface, current_id: str) -> str:
    event = get_event_from_state(interface)
    list_of_ids = list_of_cadet_ids_in_master_and_mapped_data(event)
