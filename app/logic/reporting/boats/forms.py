...
from app.logic.reporting.boats.processes import load_additional_parameters_for_boat_report, EXCLUDE_UNALLOCATED, EXCLUDE_LAKE, EXCLUDE_RIVER_TRAIN, DISPLAY_FULL_NAMES,INCLUDE_IN_OUT
...
def explain_additional_parameters_for_boat_report(interface: abstractInterface,
    ...
    include_in_out = "Include columns to check in and out" if additional_parameters.in_out_columns else "No extra columns"

    return ListOfLines([full_names, include_in_out, lake_text, river_text, unallocated]).add_Lines()
...
def reporting_options_form_for_boat_additional_parameters(
    ...
    in_out = yes_no_radio(input_name=INCLUDE_IN_OUT, input_label="Add extra columns to check boats in and out?", default_is_yes=additional_parameters.in_out_columns)
    event = get_event_from_state(interface)
    if event.contains_groups:
