...
def get_list_of_all_swap_buttons_in_boat_allocation(event: Event)-> list:
    swap_boats =get_list_of_generic_buttons_for_each_volunteer_day_combo(event=event, button_name_function=get_swap_boats_button_name)
    swap_both=get_list_of_generic_buttons_for_each_volunteer_day_combo(event=event, button_name_function=get_swap_both_button_name)
    swap_roles=get_list_of_generic_buttons_for_each_volunteer_day_combo(event=event, button_name_function=get_swap_roles_button_name)
...
def get_swap_buttons_for_boat_rota(interface: abstractInterface,
    ...
    if is_ready_to_swap(interface):
        ...
        return get_swap_buttons_when_not_ready_to_swap(day=day, volunteer_id=volunteer_id, event=event)


def get_swap_buttons_when_not_ready_to_swap(event: Event,
                                day: Day,
                                volunteer_id: str) -> list:

    okay_to_swap_roles = is_possible_to_swap_roles_on_one_day_for_non_grouped_roles_only(event=event, volunteer_id=volunteer_id, day=day)
...
def get_swap_button_when_ready_to_swap(interface: abstractInterface,
    ...
    boat_name_of_swapping_boat = get_boat_name_allocated_to_volunteer_on_day_at_event(event=event, day=day, volunteer_id=volunteer_id_to_swap)
...
def get_swap_button_when_ready_to_swap_and_this_is_a_potential_swapper(
    ...
    valid_to_swap_boats = not boat_name_of_swapping_boat == boat_at_event.name
    valid_to_swap_roles = is_possible_to_swap_roles_on_one_day_for_non_grouped_roles_only(event=event,
                                                                                    day=day,
                                                                                    volunteer_id=volunteer_id)
...
def get_all_swap_buttons_for_boat_allocation(interface: abstractInterface):
    ...
    return get_list_of_all_swap_buttons_in_boat_allocation(event)
...
def update_if_swap_button_pressed_and_ready_to_swap(interface: abstractInterface, swap_button: str):
    ...
    do_swapping_for_volunteers_boats_and_possibly_roles_in_boat_allocation(swap_boats=swap_boats,
...
def do_swapping_for_volunteers_boats_and_possibly_roles_in_boat_allocation(swap_boats: bool,
                                                                           swap_roles: bool,
                                                                           event: Event,
                                                                           original_day: Day,
    ...
    no_swap_required_cancel_instead = original_day==day_to_swap_with and original_volunteer_id==volunteer_id_to_swap_with
    if no_swap_required_cancel_instead:
        return

    if swap_roles:
        swap_roles_for_volunteers_in_allocation(original_day=original_day,
                                                event=event,
                                                day_to_swap_with=day_to_swap_with,
                                                volunteer_id_to_swap_with=volunteer_id_to_swap_with,
                                                original_volunteer_id=original_volunteer_id)
    if swap_boats:
        swap_boats_for_volunteers_in_allocation(original_day=original_day,
                                                event=event,
                                                day_to_swap_with=day_to_swap_with,
                                                volunteer_id_to_swap_with=volunteer_id_to_swap_with,
                                                original_volunteer_id=original_volunteer_id)
