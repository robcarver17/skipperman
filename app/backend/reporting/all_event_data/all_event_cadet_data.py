...
def get_df_for_cadets_event_data_dump(object_store: ObjectStore, event: Event):
    ...
    list_of_cadets = ListOfCadets([
        get_cadet_from_id(object_store=object_store, cadet_id=identified_cadet.cadet_id) for identified_cadet in list_of_identified_cadets
    ])
    ...
    list_of_notes = [
        data_from_cadets_at_event_data_or_empty(
            object_store=object_store,
            event=event, cadet=cadet, keyname="notes"
        )
        for cadet in list_of_cadets
    ]
    list_of_health = [
        data_from_cadets_at_event_data_or_empty(
            object_store=object_store,
            event=event, cadet=cadet, keyname="health"
        )
        for cadet in list_of_cadets
    ]
    list_of_club_dinghy = [
        club_dinghy_for_cadet(
            object_store=object_store,
    ...
    list_of_volunteer_names = [
        names_of_volunteers_for_cadet(object_store=object_store, event=event, cadet=cadet)
        for cadet in list_of_cadets
    ]
    list_of_groups = [
...
def club_dinghy_for_cadet(object_store: ObjectStore, event: Event, cadet: Cadet):
    dinghy_data = get_dict_of_cadets_and_club_dinghies_at_event(object_store=object_store, event=event)
    day_item_dict = dict(
        [
            (
                day,
...
def boat_class_for_cadet(object_store: ObjectStore, event: Event, cadet: Cadet):
    dinghy_data = get_dict_of_cadets_and_boat_classes_and_partners_at_events(object_store=object_store, event=event)
    day_item_dict = dict(
        [
            (
                day,
...
def names_of_volunteers_for_cadet(
    ...
    dict_of_registration_data_for_volunteers_at_event = get_dict_of_registration_data_for_volunteers_at_event(object_store=object_store,
                                                                                                          event=event)
    volunteers_at_event = dict_of_registration_data_for_volunteers_at_event.list_of_volunteers_at_event()

    list_of_volunteers_associated_with_cadet = get_list_of_volunteers_associated_with_cadet(object_store=object_store, cadet=cadet)

    associated_volunteers_at_event = ListOfVolunteers([volunteer
