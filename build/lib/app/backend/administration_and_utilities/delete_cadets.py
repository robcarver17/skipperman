...
def delete_cadet_in_data_and_return_warnings(
    ...
    existing_membership = DEPRECATE_delete_cadet_from_committee_data(
        interface=id(), cadet=cadet_to_delete, areyousure=True
    )
    ...
    list_of_associated_volunteers = delete_all_connections_for_cadet(
        object_store=object_store, cadet=cadet_to_delete, areyousure=True
    )
    if len(list_of_associated_volunteers) > 0:
        volunteer_message = "Will delete associations with %s" % ", ".join(
            [volunteer.name for volunteer in list_of_associated_volunteers]
        )
    else:
    ...
    if len(existing_qualifications) > 0:
        qual_message = "Will delete qualifications: %s" % ", ".join(
            [qual.name for qual in existing_qualifications]
        )
    else:
    ...
    if len(attendance_at_events_deleted) > 0:
        attendance_message = (
            "Will delete instructor attendance records for %s at %s"
            % (
                cadet_to_delete,
                ", ".join([event.name for event in attendance_at_events_deleted]),
            )
        )
    else:
    ...
    for event in list_of_events:
        ...
        if rows_identified > 0:
            event_messages.append(
                "- will delete %d rows of identified registration data"
                % rows_identified
            )
    ...
    DEPRECATE_delete_cadet(object_store=object_store, cadet=cadet_to_delete, areyousure=True)
