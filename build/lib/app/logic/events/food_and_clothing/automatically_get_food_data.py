...
from app.OLD_backend.food import is_cadet_with_id_already_at_event_with_food, add_new_cadet_with_food_to_event, \
    is_volunteer_with_id_already_at_event_with_food, add_new_volunteer_with_food_to_event

from app.objects.events import Event

from app.OLD_backend.wa_import.update_cadets_at_event import list_of_cadet_ids_at_event_and_in_mapped_data_for_event, \
...
def get_and_save_food_for_cadets_from_registration_data(interface: abstractInterface):
    ...
    for cadet_id in list_of_ids:
        process_update_to_cadet_food_data(interface=interface, event=event, cadet_id=cadet_id)
...
def process_update_to_cadet_food_data(
    ...
    process_update_to_cadet_food_data_if_new_to_event(interface=interface, event=event, cadet_id=cadet_id)
...
def get_and_save_food_for_volunteers_from_registration_data(interface: abstractInterface):
    ...
    for volunteer_id in list_of_ids:
        process_update_to_volunteer_food_data(interface=interface, event=event, volunteer_id=volunteer_id)
...
def process_update_to_volunteer_food_data(
    ...
    process_update_to_volunteer_food_data_if_new_to_event(interface=interface, event=event, volunteer_id=volunteer_id)
...
def process_update_to_volunteer_food_data_if_new_to_event(
    ...
    interface.log_error("Added food for volunteer %s to event" % EPRECATE_get_volunteer_name_from_id(volunteer_id=volunteer_id, interface=interface))
