...
def display_view_of_events(state_data: StateDataForAction, sort_by: str = SORT_BY_START_DSC):
    ...
    html_inside_form = ListOfHtml([
        sort_buttons,
        Html("Click on any event to view/edit/delete"),
        add_button,
...
def display_list_of_events_with_buttons(sort_by=SORT_BY_START_DSC) -> Html:
    list_of_events = get_list_of_events(data, sort_by=sort_by)

    list_with_buttons = [row_of_form_for_event_with_buttons(
        event
