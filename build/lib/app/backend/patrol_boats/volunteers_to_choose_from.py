...
def sort_volunteer_data_by_role_on_day_and_skills_and_then_name(
    ...
    volunteers_in_boat_related_roles_on_day_of_event = (
        get_volunteers_in_boat_related_roles_on_day_of_event(
            event_data,
            day=day
        )
    )
    ...
    all_volunteers_allocated_to_any_boat_or_day = (
        get_all_volunteers_allocated_to_any_boat_or_day(
            event_data
        )
    )
    ...
    list_of_volunteers_to_choose_from_sorted_by_name = list_of_volunteers_to_choose_from.sort_by_firstname()
...
def move_volunteers_from_list_to_sorted_list( sorted_list_of_volunteers: ListOfVolunteers,
                                              volunteers_to_move: ListOfVolunteers) -> ListOfVolunteers:
...
def  get_volunteers_in_boat_related_roles_on_day_of_event(event_data: DictOfAllEventDataForVolunteers, day: Day) -> ListOfVolunteers:
    volunteer_list = []
...
def get_all_volunteers_allocated_to_any_boat_or_day(event_data: DictOfAllEventDataForVolunteers) -> ListOfVolunteers:
    volunteer_list = [volunteer for volunteer, event_data_for_volunteer in event_data.items()
                      if event_data_for_volunteer.patrol_boats.assigned_to_any_boat_on_any_day()]

    return ListOfVolunteers(volunteer_list)

def get_list_of_volunteers_who_can_drive_safety_boat(event_data: DictOfAllEventDataForVolunteers) -> ListOfVolunteers:
    volunteer_list = [volunteer for volunteer, event_data_for_volunteer in event_data.items()  if event_data_for_volunteer.volunteer_skills.can_drive_safety_boat]
...
def boat_related_role_str_and_group_on_day_for_volunteer_at_event(
    ...
    role_and_group = event_data_for_volunteer.roles_and_groups.role_and_group_on_day(day)
    ...
    if role_and_group.group.is_unallocated:
        ...
        role_str = "%s - %s" % (
            role_and_group.group.name,
            role_and_group.role.name
        )
