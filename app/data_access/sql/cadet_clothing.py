...
class SqlDataListOfCadetsWithClothingAtEvent(GenericSqlData):
    def delete_cadet_from_event_and_return_messages(self, event_id: str, cadet_id: str):
        ...
        self.remove_clothing_requirements_for_cadet_at_event(
            event_id=event_id, cadet_id=cadet_id
        )

        return [
            "Removed clothing requirements for cadet %s at event %s."
            % (cadet_id, event_id)
        ]

    def add_new_cadet_with_clothing_to_event(
        self, event_id: str, cadet_id: str, size: str
    ):
        try:
            if self.table_does_not_exist(CADET_WITH_CLOTHING_AT_EVENT_TABLE):
                self.create_table()

            if self.does_cadet_have_clothing_record_at_event(
                event_id=event_id, cadet_id=cadet_id
            ):
                raise MultipleMatches(
                    "Already have clothing record for cadet with ID %s at event ID %s"
                    % (event_id, cadet_id)
                )
    ...
    def does_cadet_have_clothing_record_at_event(self, event_id: str, cadet_id: str):
        try:
            ...
            raise Exception(
                "Error %s when reading cadet clothing table event# %s"
                % (str(e1), event_id)
            )
        finally:
            self.close()
        ...
        if count > 1:
            raise MultipleMatches(
                "Multiple clothing records for cadet with ID %s at event ID %s"
                % (event_id, cadet_id)
            )
        elif count == 0:
            return False
    ...
    def remove_clothing_requirements_for_cadet_at_event(
    ...
    def clear_colour_group_for_cadet(self, event_id: str, cadet_id: str):
        self.change_colour_group_for_cadet(
            event_id=event_id, cadet_id=cadet_id, colour=UNALLOCATED_COLOUR
        )
    ...
    def change_colour_group_for_cadet(self, event_id: str, cadet_id: str, colour: str):
        if not self.does_cadet_have_clothing_record_at_event(
            ...
            raise Exception("No record of existing clothing for cadet")
        ...
        try:
            ...
            self.cursor.execute(insertion, (colour,))
    ...
    def change_clothing_size_for_cadet(self, event_id: str, cadet_id: str, size: str):
        ...
        try:
            ...
            self.cursor.execute(insertion, (size,))
    ...
    def get_dict_of_cadets_with_clothing_at_event(
        ...
        for raw_item in raw_data:
            ...
            new_dict[cadet] = ClothingAtEvent(
                colour=raw_item.colour, size=raw_item.size
            )
    ...
    def write(
        ...
        try:
            ...
            for cadet_with_clothing in list_of_cadets_with_clothing:
                self._write_row_without_checks_or_commits(
                    event_id=event_id, cadet_with_clothing=cadet_with_clothing
                )
    ...
    def _write_row_without_checks_or_commits(
        self, event_id: str, cadet_with_clothing: CadetWithClothingAndIdsAtEvent
    ):
        ...
        self.cursor.execute(insertion, (int(event_id), cadet_id, size, colour))
    ...
    def create_table(self):
        ...
        try:
            ...
            raise Exception(
                "Error %s when creating identified cadets clothing table" % str(e1)
            )
        finally:
            self.close()
