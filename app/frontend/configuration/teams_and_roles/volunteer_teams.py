...
from app.objects.roles_and_teams import ListOfTeams, Team, all_role_locations, role_location_no_warning
from app.frontend.configuration.teams_and_roles.edit_individual_team import display_form_edit_individual_team_page

from app.data_access.store.object_store import ObjectStore

from app.backend.volunteers.roles_and_teams import get_list_of_teams, update_list_of_teams, \
    add_new_team, modify_team

from app.frontend.form_handler import (
    button_error_and_back_to_initial_state_form,
)
from app.frontend.shared.team_state import update_state_for_specific_team_given_team_as_str
from app.frontend.configuration.generic_list_modifier import (
    BACK_BUTTON_PRESSED,
    BUTTON_NOT_KNOWN,  up_button_for_entry, down_button_for_entry,
    text_box_name,  edit_contents_button,
    edit_button_pressed,
    display_form_edit_generic_list, post_form_edit_generic_list,
)
...
def get_row_for_existing_entry(entry: Team, **kwargs_ignored) -> RowInTable:
    if entry.protected:
        return RowInTable(["Cannot edit: %s" % entry.name, entry.location_for_cadet_warning.name, "", edit_contents_button(entry.name) ])
    line = [
...
def get_team_from_form(interface: abstractInterface, existing_object,  **kwargs_ignored) ->Team:
    ...
    new_team = Team(name=new_team_name, location_for_cadet_warning=new_location, protected=False)
...
def save_from_ordinary_list_of_teams(
        object_store: ObjectStore, new_list: List[Team]
):
    update_list_of_teams(object_store=object_store,
                         list_of_teams=ListOfTeams(new_list))
