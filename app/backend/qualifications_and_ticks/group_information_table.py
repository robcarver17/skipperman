...
def get_group_info_table(
    ...
    boat_class = get_boat_class_and_sail_number_column_for_list_of_cadets(
        all_event_info_for_cadets=all_event_info_for_cadets,
        list_of_cadets=cadets_in_group,
        group=group,
    )

    partners = get_partner_column_for_list_of_cadets(
        all_event_info_for_cadets=all_event_info_for_cadets,
        list_of_cadets=cadets_in_group,
        group=group,
    )

    qualis = get_qualification_column_for_list_of_cadets(
        object_store=object_store, list_of_cadets=cadets_in_group
    )

    health = get_health_column_for_list_of_cadets(
        all_event_info_for_cadets=all_event_info_for_cadets,
        list_of_cadets=cadets_in_group,
        group=group,
    )

    days_column = get_days_attending_column_for_list_of_cadets(
        all_event_info_for_cadets=all_event_info_for_cadets,
        list_of_cadets=cadets_in_group,
        group=group,
    )
    ...
    cadets_as_str = [
        "%s (%d)" % (cadet.name, cadet.approx_age_years()) for cadet in cadets_in_group
    ]
    df = pd.DataFrame(
        {
            "Boat": boat_class,
...
def get_days_attending_and_in_group_for_cadet(
    ...
    days_attending = all_event_info_for_cadets.event_data_for_cadet(
        cadet
    ).registration_data.availability.days_available()
    in_group = [
        day
        for day in days_attending
        if all_event_info_for_cadets.event_data_for_cadet(
            cadet
        ).days_and_groups.group_on_day(day=day)
        == group
    ]
...
def get_boat_ownership_column_for_cadet_on_day(
    all_event_info_for_cadets: DictOfAllEventInfoForCadets, cadet: Cadet, day: Day
):
    dinghy_on_day = all_event_info_for_cadets.event_data_for_cadet(
        cadet
...
def get_boat_class_and_sail_number_column_for_list_of_cadets(
    all_event_info_for_cadets: DictOfAllEventInfoForCadets,
    group: Group,
    list_of_cadets: ListOfCadets,
):
...
def get_boat_class_column_for_cadet_on_day(
    all_event_info_for_cadets: DictOfAllEventInfoForCadets, cadet: Cadet, day: Day
):
    boat_class = str(
        all_event_info_for_cadets.event_data_for_cadet(
            cadet
        ).days_and_boat_class.boat_class_on_day(day)
    )
    sail_number = all_event_info_for_cadets.event_data_for_cadet(
        cadet
    ).days_and_boat_class.sail_number_on_day(day)
    if len(sail_number) == 0:
        return boat_class
    return "%s (%s)" % (boat_class, sail_number)


def get_partner_column_for_list_of_cadets(
    all_event_info_for_cadets: DictOfAllEventInfoForCadets,
    group: Group,
    list_of_cadets: ListOfCadets,
):
...
def get_partner_column_for_cadet_on_day(
    all_event_info_for_cadets: DictOfAllEventInfoForCadets, cadet: Cadet, day: Day
):
    partner = all_event_info_for_cadets.event_data_for_cadet(
        cadet
...
def get_qualification_column_for_list_of_cadets(
    object_store: ObjectStore, list_of_cadets: ListOfCadets
):
    return [
        name_of_highest_qualification_for_cadet(object_store=object_store, cadet=cadet)
        for cadet in list_of_cadets
    ]


def get_health_column_for_list_of_cadets(
    all_event_info_for_cadets: DictOfAllEventInfoForCadets,
    group: Group,
    list_of_cadets: ListOfCadets,
):
...
def get_health_column_for_cadet_on_day(
    all_event_info_for_cadets: DictOfAllEventInfoForCadets, cadet: Cadet, day: Day
):  ## day not used
    return all_event_info_for_cadets.event_data_for_cadet(
        cadet
    ).registration_data.health


def get_days_attending_column_for_list_of_cadets(
    all_event_info_for_cadets: DictOfAllEventInfoForCadets,
    group: Group,
    list_of_cadets: ListOfCadets,
):
    return [
        get_days_attending_column_for_single_cadets(
            all_event_info_for_cadets=all_event_info_for_cadets,
            group=group,
            cadet=cadet,
        )
...
def get_days_attending_column_for_single_cadets(
    ...
    cadet_at_all_days_for_event = (
        len(set(event_days).difference(set(all_days_for_cadet))) == 0
    )
...
def day_annotation(dict_of_values: Dict[Day, str]):
    values_over_days = [
        "%s (%s)" % (str_value, day.name[:3])
        for day, str_value in dict_of_values.items()
    ]
