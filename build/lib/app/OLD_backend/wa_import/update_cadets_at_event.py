...
from app.OLD_backend.cadets import get_cadet_from_id, cadet_name_from_id
...
def is_cadet_with_id_already_at_event(
    ...
    return cadets_at_event_data.is_cadet_with_id_already_at_event(
...
def add_new_cadet_to_event(
    ...
    cadets_at_event_data = CadetsAtEventIdLevelData(interface.data)
    cadets_at_event_data.add_new_cadet_to_event(
        event=event, row_in_mapped_wa_event=row_in_mapped_wa_event, cadet_id=cadet_id
    )


def get_cadet_at_event_for_cadet_id(
    interface: abstractInterface, event: Event, cadet_id: str
) -> CadetWithIdAtEvent:
    cadets_at_event_data = CadetsAtEventIdLevelData(interface.data)
    return cadets_at_event_data.cadet_at_event_or_missing_data(
...
def replace_existing_cadet_at_event_where_original_cadet_was_inactive(
    ...
    cadets_at_event_data = CadetsAtEventIdLevelData(interface.data)
    cadets_at_event_data.replace_existing_cadet_at_event(
        new_cadet_at_event=new_cadet_at_event, event=event
    )


def update_payment_status_of_existing_cadet_at_event(
    interface: abstractInterface,
    event: Event,
    cadet_id: str,
    ...
    cadets_at_event_data = CadetsAtEventIdLevelData(interface.data)
    cadets_at_event_data.update_status_of_existing_cadet_at_event(
        event=event, cadet_id=cadet_id, new_status=new_status
    )


def update_status_of_existing_cadet_at_event_to_cancelled_or_deleted(
    interface: abstractInterface,
    event: Event,
    cadet_id: str,
    new_status: RegistrationStatus,
    ...
    for day in event.weekdays_in_event():
        remove_partner_and_log_if_present(
            interface=interface, event=event, cadet_id=cadet_id, day=day
        )

        dinghies_data.remove_club_boat_allocation_for_cadet_on_day(
            event=event, cadet_id=cadet_id, day=day
        )

    dinghies_data.remove_boat_and_partner_for_cadet_at_event(
        event=event, cadet_id=cadet_id
    )
...
def remove_partner_and_log_if_present(
    interface: abstractInterface, event: Event, cadet_id: str, day: Day
):
    dinghies_data = DinghiesData(interface.data)
    cadets_data = CadetData(interface.data)
    cadet_with_boat_at_event = dinghies_data.get_list_of_cadets_at_event_with_dinghies(
        event
    ).object_with_cadet_id_on_day(cadet_id=cadet_id, day=day)
    ...
    if cadet_with_boat_at_event.has_partner():
        ...
        partner_name = cadets_data.get_cadet_with_id(partner_cadet_id).name
        cadet_name = cadets_data.get_cadet_with_id(cadet_id).name
...
def make_cadet_available_on_day(
    ...
    cadets_at_event_data = CadetsAtEventIdLevelData(interface.data)
    cadets_at_event_data.make_cadet_available_on_day(
        event=event, cadet_id=cadet_id, day=day
    )
...
def update_availability_of_existing_cadet_at_event(
    ...
    cadets_at_event_data.update_availability_of_existing_cadet_at_event(
        cadet_id=cadet_id, new_availabilty=new_availabilty, event=event
    )
    ...
    for day in existing_availability.days_available():
        ...
        dinghies_data.remove_boat_and_partner_for_cadet_at_event_on_day(
            event=event, cadet_id=cadet_id, day=day
        )
        ...
        groups_data.remove_cadet_from_data_on_day(
            event=event, day=day, cadet_id=cadet_id
        )


def update_notes_for_existing_cadet_at_event(
    interface: abstractInterface, event: Event, cadet_id: str, new_notes: str
):
    cadets_at_event_data = CadetsAtEventIdLevelData(interface.data)
    cadets_at_event_data.update_notes_for_existing_cadet_at_event(
        cadet_id=cadet_id, new_notes=new_notes, event=event
    )


def update_health_for_existing_cadet_at_event(
    interface: abstractInterface, event: Event, cadet_id: str, new_health: str
):
    cadets_at_event_data = CadetsAtEventIdLevelData(interface.data)
    cadets_at_event_data.update_health_for_existing_cadet_with_id_at_event(
        cadet_id=cadet_id, new_health=new_health, event=event
    )
...
def update_data_row_for_existing_cadet_at_event(
    ...
    cadets_at_event_data.update_data_row_for_existing_cadet_at_event(
        event=event, cadet_id=cadet_id, new_data_in_row=new_data_in_row
    )


def no_important_difference_between_cadets_at_event(
    new_cadet_at_event: CadetWithIdAtEvent, existing_cadet_at_event: CadetWithIdAtEvent
) -> bool:
    ...
    available_matches = (
        new_cadet_at_event.availability == existing_cadet_at_event.availability
    )
...
def new_status_and_status_message(
    ...
    if old_status.is_cancelled and new_status.is_active:
        ...
        status_message = (
            "Cadet %s was still active but status has changed from %s to %s"
...
def list_of_cadet_ids_at_event_and_in_mapped_data_for_event(
    interface: abstractInterface, event: Event, include_mapped_data: bool = True
) -> list:
    cadets_event_data = CadetsAtEventIdLevelData(interface.data)
