...
def get_dict_of_df_for_reporting_rollcalls_with_flags(
    ...
    list_of_groups = dict_of_all_event_data.dict_of_cadets_with_days_and_groups.all_groups_at_event()
...
def get_block_of_df_for_group_at_event(        dict_of_all_event_data: DictOfAllEventInfoForCadets,
    ...
    if display_full_names:
        ...
        list_of_cadet_names = [cadet.initial_and_surname for cadet in list_of_cadets_in_group]

    names_as_series = pd.Series(list_of_cadet_names)
    group_as_series = pd.Series([group]*len(list_of_cadet_names))
    df = pd.concat(
        [names_as_series, group_as_series], axis=1
    )

    df = add_attendance_to_rollcall_df(df=df,
                                       dict_of_all_event_data=dict_of_all_event_data,
...
def add_club_boat_asterix_to_cadet_with_club_boat_on_any_day(
    ...
    has_dinghy = dict_of_cadets_at_event_with_club_dinghies.club_dinghys_for_cadet(cadet).has_dinghy_on_any_day()
...
def add_attendance_to_rollcall_df(df: pd.DataFrame,
                    dict_of_all_event_data: DictOfAllEventInfoForCadets,
                                  list_of_cadets_in_group: ListOfCadets) -> pd.DataFrame:

    attendance = get_attendance_matrix_for_list_of_cadets_at_event_with_passed_event_info(
...
def add_extra_to_reporting_df(
    ...
    if include_emergency_contacts:
        contact_list = dict_of_all_event_data.dict_of_cadets_with_registration_data.get_emergency_contact_for_list_of_cadets_at_event(list_of_cadets_in_group)
    ...
    if include_health_data:
        health_list = (
            dict_of_all_event_data.dict_of_cadets_with_registration_data.get_health_notes_for_list_of_cadets_at_event(
