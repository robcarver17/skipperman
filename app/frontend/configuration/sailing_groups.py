...
from app.backend.groups.list_of_groups import get_list_of_groups, update_list_of_groups, \
    modify_sailing_group, add_new_sailing_group_given_name
from app.frontend.configuration.generic_list_modifier import \
    BACK_BUTTON_PRESSED, BUTTON_NOT_KNOWN, up_button_for_entry, \
    down_button_for_entry, text_box_name, \
    hide_button_for_entry, display_form_edit_generic_list, \
    post_form_edit_generic_list, hidden_box_name
...
def get_group_from_form(interface: abstractInterface, existing_object,  **kwargs_ignored) -> Group:
    ...
    new_group = Group(name=new_group_name, location=new_location, protected=False, hidden=is_hidden)
    return new_group



def save_from_ordinary_list_of_groups(
        object_store: ObjectStore, new_list: List[Group]
):
    update_list_of_groups(
        object_store=object_store,
        updated_list_of_groups=ListOfGroups(new_list))
