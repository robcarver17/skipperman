...
def update_and_save_mapped_wa_event_data_with_cadet_ids(
    ...
    remove_deleted_cadets_from_event(mapped_wa_event_data=mapped_wa_event_data,
...
def remove_deleted_cadets_from_event(
    ...
    existing_mapped_wa_event_with_ids.delete_list_of_rows_with_timestamps(missing_timestamps)
...
def existing_timestamps_that_are_missing_from_mapped_wa_event_data(
    ...
    list_of_timestamps_in_existing_data = existing_mapped_wa_event_with_ids.list_of_timestamps()
    list_of_timestamps_in_mapped_data = mapped_wa_event_data.list_of_timestamps()

    missing_timestamps = list(set(list_of_timestamps_in_existing_data).difference(set(list_of_timestamps_in_mapped_data)))
...
def update_existing_wa_event_data_with_new_field_data(
    ...
    timestamps_in_both = list(set(list_of_timestamps_in_existing_data).intersection(set(list_of_timestamps_in_mapped_data)))
...
def add_new_rows_to_event(data_and_interface: DataAndInterface,
                          mapped_wa_event_data: MappedWAEventNoIDs,
                          existing_mapped_wa_event_with_ids: MappedWAEventWithIDs
                          ):
    ...
    existing_mapped_wa_event_with_ids.add_new_rows(
        new_mapped_wa_event_data_with_ids
    )
...
def only_new_rows_in_mapped_wa_event_data(
    ...
    list_of_timestamps_in_existing_wa_event = existing_mapped_wa_event_with_ids.list_of_timestamps()
    list_of_timestamps_in_mapped_data = mapped_wa_event_data.list_of_timestamps()
    new_timestamps = list(set(list_of_timestamps_in_mapped_data).difference(list_of_timestamps_in_existing_wa_event))
