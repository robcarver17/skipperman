...
from app.backend.club_boats.summary import summarise_club_boat_allocations_for_cadets_at_event, \
...
def get_top_row_in_club_dinghy_form(event: Event) -> RowInTable:
    ...
    return RowInTable(
        ["Club boat"]
        + list_of_days
        + ["", "Spaces in boat available at event (x2 boats for double handers)"],
        is_heading_row=True,
    )


def get_row_in_club_dinghy_form(
    event: Event,
    name_of_dinghy: str,
    club_dinghies_as_df: pd.DataFrame,
    limits_for_event: Dict[str, int],
) -> RowInTable:
    limit_value = limits_for_event[name_of_dinghy]
    day_values_in_df = get_boats_allocated_per_day(
        event, name_of_dinghy, club_dinghies_as_df, limit_value
    )
    input_cell = get_input_cell_for_boat_limits(
        current_limit=limit_value, name_of_dinghy=name_of_dinghy
    )

    return RowInTable([name_of_dinghy] + day_values_in_df + [input_cell])


def get_boats_allocated_per_day(
    event: Event,
    name_of_dinghy: str,
    club_dinghies_as_df: pd.DataFrame,
    limit_value: int,
    ...
    day_values_in_df = [
        _str_add_start(value, limit_value) for value in day_values_in_df
    ]
...
def get_input_cell_for_boat_limits(current_limit: int, name_of_dinghy: str) -> intInput:
    return intInput(
        input_name=get_cell_name_for_boat_limits(name_of_dinghy),
        input_label="",
...
def get_bottom_row_in_club_dinghy_form(event: Event) -> RowInTable:
    ...
    return RowInTable(pad_list_of_days + [update_limits_button])
...
def get_limit_for_boat_from_form(
    interface: abstractInterface, club_dinghy: ClubDinghy
) -> int:
