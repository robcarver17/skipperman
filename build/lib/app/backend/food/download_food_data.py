...
def get_food_data_for_download(
    ...
    return dict(
        ...
        cadets=get_list_of_cadets_with_food_as_df(object_store=object_store, event=event),
    )
    ...
    for food_required in list_of_food_requirements:
        list_of_required_df.append(
            get_allergy_list_as_df_for_cadets_and_volunteers(
                object_store=object_store, event=event, specific_food_required=food_required
            )
        )
...
def get_combined_list_of_food_requirement_items(object_store: ObjectStore, event: Event) -> List[FoodRequirements]:
    cadet_food_requirements = get_dict_of_active_cadets_with_food_requirements_at_event(object_store=object_store,
                                                                                        event=event)
    list_of_requirements_for_cadets = cadet_food_requirements.unique_list_of_food_requirements()

    volunteer_food_requirements = get_dict_of_active_volunteers_with_food_requirements_at_event(
        object_store=object_store, event=event)
    list_of_requirements_for_volunteers = volunteer_food_requirements.unique_list_of_food_requirements()
...
def get_allergy_list_as_df_for_cadets_and_volunteers(
    ...
    volunteer_df = get_allergy_list_as_df_for_volunteers(
        object_store=object_store, event=event, specific_food_required= specific_food_required
    )
    ...
    header_line = pd.Series(
        dict(type="", name="%s (%d)" % ( specific_food_required.describe(), len(both_df)))
    )
...
def get_list_of_cadets_with_food_as_df(
    ...
    cadets_with_food = get_dict_of_active_cadets_with_food_requirements_at_event(object_store=object_store, event=event)
...
def row_for_cadet_in_table(
        cadet: Cadet,
        food_requirements: FoodRequirements) -> pd.Series:
