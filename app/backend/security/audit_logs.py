...
from app.objects.audit_log import ListOfAuditLogUpdatesWithIds, AuditLogUpdateWithIds, ListOfAuditLogUpdatesWithEvents
from app.objects.events import Event

def get_list_of_audit_logs_for_all_events_newest_first(object_store:ObjectStore) -> ListOfAuditLogUpdatesWithEvents:
    return object_store.get(
        object_store.data_api.data_list_of_audit_updates.read_for_all_events
    )


def get_list_of_audit_logs_for_event_newest_first(object_store: ObjectStore, event: Event) -> ListOfAuditLogUpdatesWithIds:
    return object_store.get(
        object_store.data_api.data_list_of_audit_updates.read,
        event_id=event.id
    )
