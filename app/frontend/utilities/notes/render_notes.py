...
def get_quick_note_form() -> Line:
    return Line(["Enter new note here",
    textAreaInput(
        input_name=quick_note_text,
        value="",
        input_label=""
    )])


save_quick_note_button = Button("Save quick note", nav_button=False, shortcut=SAVE_KEYBOARD_SHORTCUT)
save_table_entries = Button("Save changes", nav_button=False, shortcut=SAVE_KEYBOARD_SHORTCUT)

def get_existing_incomplete_notes(interface: abstractInterface, list_of_notes: ListOfNotesWithVolunteers) -> Table:
    list_of_notes = list_of_notes.uncompleted_only()
    list_of_rows = [row_for_existing_note(existing_note_with_volunteer=existing_note_with_volunteer,
                                          interface=interface) for existing_note_with_volunteer in list_of_notes]

    return Table([top_row_of_incomplete_table()]+list_of_rows, has_column_headings=True)


def get_existing_complete_notes(interface: abstractInterface, list_of_notes: ListOfNotesWithVolunteers) -> Table:
...
def top_row_of_incomplete_table():
    return [sort_by_author, sort_by_date, 'Note', sort_by_assigned, sort_by_priority, '']
...
def row_for_existing_note(interface: abstractInterface, existing_note_with_volunteer: NoteWithVolunteer) -> RowInTable:
    ...
    text = note_text_box(interface, existing_note_with_volunteer=existing_note_with_volunteer)
    assigned = assigned_volunteer_dropdown(existing_note_with_volunteer=existing_note_with_volunteer, interface=interface)
    priority = priority_dropdown(interface, existing_note_with_volunteer=existing_note_with_volunteer)
    checkbox = completed_checkbox(interface, existing_note_with_volunteer=existing_note_with_volunteer)
...
def note_text_box(interface: abstractInterface,  existing_note_with_volunteer: NoteWithVolunteer):
    allow_edit = not existing_note_with_volunteer.completed and existing_note_with_volunteer.author_volunteer == get_loggged_in_volunteer(interface)
    if allow_edit:
...
def assigned_volunteer_dropdown(interface: abstractInterface, existing_note_with_volunteer: NoteWithVolunteer):
    allow_edit = not existing_note_with_volunteer.completed and existing_note_with_volunteer.author_volunteer == get_loggged_in_volunteer(interface)
    if allow_edit:
...
def priority_dropdown(interface: abstractInterface, existing_note_with_volunteer: NoteWithVolunteer):
    allow_edit = not existing_note_with_volunteer.completed and existing_note_with_volunteer.author_volunteer == get_loggged_in_volunteer(interface)
    if allow_edit:
        return dropDownInput(
...
def get_input_name_for_note(existing_note_with_volunteer: NoteWithVolunteer, attribute:str):
    return "%s_%s" % (existing_note_with_volunteer.id, attribute)

def completed_checkbox(interface: abstractInterface, existing_note_with_volunteer: NoteWithVolunteer):
    logged_in_volunteer =get_loggged_in_volunteer(interface)
    allow_edit = existing_note_with_volunteer.assigned_volunteer == logged_in_volunteer or existing_note_with_volunteer.author_volunteer == logged_in_volunteer
    if allow_edit:
        ...
        completed = "Completed" if existing_note_with_volunteer.completed else "Incomplete"
        return completed
...
def get_sorted_list_of_volunteer_notes(interface: abstractInterface):
    list_of_notes = get_list_of_notes_with_volunteers(object_store=interface.object_store)
