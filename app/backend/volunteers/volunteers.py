...
def get_volunteer_from_list_of_volunteers_given_volunteer_name(
    interface: abstractInterface, volunteer_name: str
) -> Volunteer:
    volunteer_data = VolunteerData(interface.data)
    return volunteer_data.get_volunteer_from_list_of_volunteers_given_name(
        volunteer_name=volunteer_name
    )


def DEPRECATE_get_dict_of_existing_skills(
...
def warning_for_similar_volunteers(
    interface: abstractInterface, volunteer: Volunteer
) -> str:
...
def verify_volunteer_and_warn(
    ...
    warn_text += warning_for_similar_volunteers(
        interface=interface, volunteer=volunteer
    )
...
def are_all_cadet_ids_in_list_already_connection_to_volunteer(
    interface: abstractInterface, volunteer: Volunteer, list_of_cadet_ids: List[str]
) -> bool:
...
def is_cadet_already_connected_to_volunteer_in_volunteer_list(
    interface: abstractInterface, cadet_id: str, volunteer: Volunteer
) -> bool:
    volunteer_data = VolunteerData(interface.data)
...
def boat_related_skill_for_volunteer(
    interface: abstractInterface, volunteer_id: str
) -> bool:
    volunteer_data = VolunteerData(interface.data)
    return volunteer_data._can_volunteer_with_id_drive_safety_boat(volunteer_id)
...
def add_boat_related_skill_for_volunteer(
    ...
    volunteer_data = VolunteerData(interface.data)
    volunteer_data.confirm_volunteer_with_id_can_drive(volunteer_id)
...
def remove_boat_related_skill_for_volunteer(
    ...
    volunteer_data = VolunteerData(interface.data)
    volunteer_data.remove_driving_qualification_for_volunteer_with_id(volunteer_id)
...
def save_skills_for_volunteer(
    ...
    volunteer_data = VolunteerData(interface.data)
    volunteer_data.save_skills_for_volunteer(
        volunteer=volunteer, dict_of_skills=dict_of_skills
    )


def DEPRECATE_load_list_of_volunteer_skills(
    interface: abstractInterface,
...
def DEPRECATE_get_sorted_list_of_volunteers(
...
def DEPRECATE_get_list_of_all_volunteers(
    interface: abstractInterface,
) -> ListOfVolunteers:
    volunteer_data = VolunteerData(interface.data)
...
def is_volunteer_with_id_qualified_as_SI(interface: abstractInterface, volunteer_id: str):
    volunteer_data = VolunteerData(interface.data)
