...
def get_object_from_form(interface: abstractInterface, existing_object):
    ...
    if has_hidden_attribute(existing_object):
        return get_object_from_form_with_hidden_attribute(
            interface=interface, existing_object=existing_object
        )

    new_name = interface.value_from_form(
...
def get_object_from_form_with_hidden_attribute(
    interface: abstractInterface, existing_object
):
    new_name = interface.value_from_form(
        text_box_name(existing_object), default=MISSING_FROM_FORM
    )
...
def add_new_entry_from_form(interface: abstractInterface, adding_function: Callable):
    name_of_entry_to_add = interface.value_from_form(
        ADD_ENTRY_TEXT_FIELD, default=MISSING_FROM_FORM
    )
