...
def save_dict_of_df_as_spreadsheet_file(dict_of_df: Dict[str, pd.DataFrame], path_and_filename_no_extension: str, write_index: bool = False):
    try:
        path_and_filename_with_extension = save_dict_of_df_as_xls(dict_of_df, path_and_filename_no_extension, write_index=write_index)
    except:
        path_and_filename_with_extension = save_dict_of_df_as_csv(dict_of_df, path_and_filename_no_extension, write_index=write_index)

    return path_and_filename_with_extension

def save_dict_of_df_as_xls(dict_of_df: Dict[str, pd.DataFrame], path_and_filename_without_extension: str, write_index: bool = False):
    path_and_filename_with_extension = path_and_filename_without_extension+".xlsx"
    with pd.ExcelWriter(path_and_filename_with_extension
                        ) as writer:
...
def save_dict_of_df_as_csv(dict_of_df: Dict[str, pd.DataFrame], path_and_filename_without_extension: str, write_index: bool = False):
