...
from app.frontend.events.volunteer_rota.button_values import (
    ...
    remove_role_button_type,
    remove_role_across_button_type,
    volunteer_from_remove_role_across_days_button,
)
...
from app.frontend.events.volunteer_rota.button_values import (
    ...
    volunteer_and_day_from_make_available_button,
    volunteer_from_make_unavailable_across_days_button,
...
def action_if_volunteer_button_pressed(
    ...
    volunteer = volunteer_from_button_pressed(
        object_store=interface.object_store, value_of_button_pressed=volunteer_button
    )
    if is_volunteer_id_set_in_state(interface):
        action_if_volunteer_button_pressed_and_volunteer_in_state(
            interface=interface, volunteer=volunteer
        )
    else:
        action_if_volunteer_button_pressed_and_no_volunteer_set_in_state(
            interface=interface, volunteer=volunteer
        )
...
def action_if_volunteer_button_pressed_and_volunteer_in_state(
    ...
    if volunteer_in_state == volunteer:
        event = get_event_from_state(interface)
        update_volunteer_availability_at_event_from_rota_with_form_contents(
            interface=interface, event=event, volunteer=volunteer_in_state
        )
...
def action_if_location_button_pressed(
    ...
    volunteer = from_location_button_to_volunteer(
        object_store=interface.object_store, location_button_name=location_button
    )
    update_state_with_volunteer_id(interface=interface, volunteer_id=volunteer.id)
...
def action_if_volunteer_skills_button_pressed(
    ...
    volunteer = from_skills_button_to_volunteer(
        object_store=interface.object_store, skills_button_name=volunteer_skills_button
    )
    update_state_with_volunteer_id(interface=interface, volunteer_id=volunteer.id)
...
def update_if_make_available_button_pressed(
    ...
    volunteer, day = volunteer_and_day_from_make_available_button(
        object_store=interface.object_store, button=available_button
    )
    event = get_event_from_state(interface)
    make_volunteer_available_on_day(
...
def update_if_make_unavailable_button_pressed(
    ...
    if last_button_pressed_was_make_all_days_unavailable_for_volunteer(
        unavailable_button
    ):
        update_if_make_unavailable_across_days_button_pressed(
            interface=interface, unavailable_button=unavailable_button
        )
    elif last_button_pressed_was_make_unavailable_for_specific_day_button(
        unavailable_button
    ):
        update_if_make_unavailable_on_specific_day_button_pressed(
            interface=interface, unavailable_button=unavailable_button
        )
    else:
...
def update_if_make_unavailable_across_days_button_pressed(
    ...
    volunteer = volunteer_from_make_unavailable_across_days_button(
...
def update_if_remove_role_button_pressed(
    ...
    if is_button_of_type(remove_button, remove_role_button_type):
        update_if_remove_role_button_pressed_on_specific_day(
            interface=interface, remove_button=remove_button
        )

    elif is_button_of_type(remove_button, remove_role_across_button_type):
        update_if_remove_role_button_pressed_across_days(
            interface=interface, remove_button=remove_button
        )
    else:
