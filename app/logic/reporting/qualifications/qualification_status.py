...
def display_form_for_qualification_status_report(interface: abstractInterface):
    ...
    title = Heading("Select to see qualification status of cadets at event", centred=True, size=4)
...
def download_expected_qualification_for_event(interface: abstractInterface, event: Event) -> File:
    filename = write_expected_qualifications_to_temp_csv_file_and_return_filename(interface=interface, event=event)
    return File(filename)


def previous_form(interface: abstractInterface):
    return interface.get_new_display_form_for_parent_of_function(post_form_for_qualification_status_report)



def write_expected_qualifications_to_temp_csv_file_and_return_filename(interface: abstractInterface, event: Event) -> str:

    df_of_qualifications = get_expected_qualifications_for_cadets_at_event(interface=interface, event=event)
