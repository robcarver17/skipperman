...
class SqlDataListOfCadetsOnCommitte(GenericSqlData):
    ...
    def is_cadet_on_committe(self, cadet_id: str):
        ...
        try:
            ...
            cursor.execute(
                ...
                % (CADET_ID, CADETS_ON_COMMITTEE_TABLE, CADET_ID, int(cadet_id))
            )
        ...
        if len(raw_list) == 0:
            ...
            raise MultipleMatches(
                "More than one cadet with id %s on committee" % cadet_id
            )
        else:
            return True

    def toggle_selection_for_cadet_committee_member(self, cadet_id: str):
        try:
    ...
    def _write_row_without_checks_or_commits(
        self, cadet_on_committee: CadetWithIdCommitteeMember
    ):
        ...
        self.cursor.execute(
            insertion, (cadet_id, date_term_starts, date_term_ends, deselected)
        )
    ...
    def create_table(self):
        ...
        index_creation_query = "CREATE UNIQUE INDEX %s ON %s (%s)" % (
            INDEX_CADETS_ON_COMMITTEE_TABLE,
            CADETS_ON_COMMITTEE_TABLE,
            CADET_ID,
        )
