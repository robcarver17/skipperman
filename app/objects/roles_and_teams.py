...
from app.objects.generic_list_of_objects import GenericListOfObjectsWithIds, GenericListOfObjects
...
class RolesWithSkillIds(GenericSkipperManObjectWithIds):
    ...
    def from_dict_of_str(cls, dict_with_str: dict):
        ...
        skill_ids_required = skill_ids_required,
            ...
            associate_sailing_group=from_str_to_bool(dict_with_str['associate_sailing_group']),
...
class Team(GenericSkipperManObjectWithIds):
    ...
    def __eq__(self, other):
        return self.name == other.name and self.protected == other.protected and self.location_for_cadet_warning == other.location_for_cadet_warning
...
class ListOfTeams(GenericListOfObjectsWithIds):
    ...
    def add(self, new_team_name: str):
        try:
            ...
            raise Exception("Can't add duplicate team name %s already exists" % new_team_name)
        team = Team(name=new_team_name, protected=False, location_for_cadet_warning=role_location_river)
        team.id = self.next_id()

        self.append(team)

    def replace(
        self, existing_team: Team, new_team: Team
        ):
        try:
    ...
    def matching_team_name(self, team_name: str) -> Team:
        list_of_matches = [matching_team for matching_team in self if team_name==matching_team.name]
...
class ListOfTeamsAndRolesWithIds(GenericListOfObjects):
    ...
    def remove_roles_for_team_id(self, team_id: str):
        new_list = [role_and_team for role_and_team in self if role_and_team.team_id != team_id]
        return ListOfTeamsAndRolesWithIds(new_list)

    def ordered_roles_for_team_id(self, team_id: str):
        raw_list = [team_and_role for team_and_role in self if team_and_role.team_id==team_id]
