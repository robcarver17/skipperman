...
from app.backend.volunteers.volunteer_rota_data import (
    ...
    RotaSortsAndFilters,
    DEPRECATE_get_sorted_and_filtered_list_of_volunteers_at_event,
)
...
def get_body_of_table_at_event(
    ...
    data_to_be_stored = DEPRECATE_get_data_to_be_stored_for_volunteer_rota_page(
...
def get_last_part_of_row_for_volunteer_at_event(
    volunteer_at_event: VolunteerAtEventWithId, ready_to_swap: bool = False
) -> list:
    if ready_to_swap:
        return ["", ""]
...
def get_allocation_inputs_for_day_and_volunteer_when_available(
    ...
    volunteer_in_role_at_event_on_day = (
        data_to_be_stored.volunteer_in_role_at_event_on_day(
            day=day, volunteer_id=volunteer_at_event.volunteer_id
        )
    )
...
def get_copy_buttons_for_volunteer(
    ...
    copy_fill_possible = (
        data_to_be_stored.volunteer_has_empty_available_days_without_role(
            volunteer_id=volunteer_in_role_at_event_on_day.volunteer_id
        )
    )
    copy_ovewrite_required = not data_to_be_stored.volunteer_has_at_least_one_day_in_role_and_all_roles_and_groups_match(
        volunteer_id=volunteer_in_role_at_event_on_day.volunteer_id
    )
