...
from app.backend.data.volunteer_allocation import   add_volunteer_and_cadet_association_for_existing_volunteer, get_volunteer_at_event, \
...
def display_form_edit_cadet_connections_from_rota(interface: abstractInterface):
    ...
    group_text= get_group_text(interface=interface, event=event, connected_cadets=connected_cadets)
    ...
    list_of_cadets_at_event = get_list_of_active_cadets_at_event(event)
...
def get_group_text(interface: abstractInterface, event: Event, connected_cadets: ListOfCadets) -> Line:
    groups = get_list_of_groups_at_event_given_list_of_cadets(interface=interface, event=event, list_of_cadets=connected_cadets)
...
def post_form_edit_cadet_connections_from_rota(
    ...
    list_of_delete_cadet_buttons = get_list_of_delete_cadet_buttons_with_connected_cadets(interface)
...
def get_list_of_delete_cadet_buttons_with_connected_cadets(interface: abstractInterface):
    connected_cadets = get_list_of_connected_cadets(interface)
...
def delete_event_connection_given_form(interface: abstractInterface):
    ...
    remove_volunteer_and_cadet_association_at_event(interface=interface,
                                                              cadet_id=cadet.id,
...
def add_event_specific_cadet_connection_from_form(interface: abstractInterface):
    ...
    event = get_event_from_state(interface)
    add_volunteer_and_cadet_association_for_existing_volunteer(interface=interface,
                                                               volunteer_id=volunteer_id,
