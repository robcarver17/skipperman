...
from app.logic.forms_and_interfaces.abstract_form import File, Form
...
def generate_action_page_html(action_name: str) -> Union[Html, Response]:
    ...
    abstract_form_for_action = site_actions.get_abstract_form_for_specific_action(action_name)

    if type(abstract_form_for_action) is File:
        return send_file(abstract_form_for_action.filename)

    html_code_for_action_in_layout = from_abstract_to_laid_out_html(abstract_form_for_action=abstract_form_for_action, action_name=action_name)

    return html_code_for_action_in_layout

def from_abstract_to_laid_out_html(abstract_form_for_action:Form, action_name: str) -> Html:
