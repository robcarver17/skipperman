...
from app.backend.patrol_boats.volunteers_to_choose_from import (
    ...
    boat_related_role_str_and_group_on_day_for_volunteer_at_event,
)
...
def from_allocation_dropdown_input_name_to_boat_and_day(
    ...
    boat = from_patrol_boat_name_to_boat(
        object_store=interface.object_store, boat_name=boat_name
    )
    day = Day[day_name]
...
def from_volunteer_data_to_string_of_volunteer_with_skill_and_role(
    volunteer: Volunteer, event_data_for_volunteer: AllEventDataForVolunteer, day: Day
) -> str:
    name = volunteer.name
    skill_str = string_if_volunteer_can_drive_else_empty(event_data_for_volunteer)

    role_str = boat_related_role_str_and_group_on_day_for_volunteer_at_event(
        event_data_for_volunteer=event_data_for_volunteer, day=day
    )
...
def volunteer_boat_role_dropdown(
    ...
    dict_of_roles_for_dropdown = get_dict_of_roles_for_dropdown(
        interface.object_store, set_role_name=current_role_name
    )
