...
def get_text_explaining_various_options_for_generic_report(
    ...
    print_options_as_text = ListOfLines([Line(bold("Print Options:")), print_options_as_text])
...
def get_arrangement_options_and_group_order_text(interface: abstractInterface,    report_generator: ReportGenerator
)-> ListOfLines:
    ...
    filtered_group_order = get_group_order_excluding_missing_groups(dict_of_df=dict_of_df, group_order=group_order,
                                                                    specific_parameters_for_type_of_report=report_generator.specific_parameters_for_type_of_report)
    ...
    if len(missing_groups)==0:
        ...
        warning = "FOLLOWING GROUPS ARE IN DATA, BUT NOT INCLUDED IN REPORT: %s (CHANGE ARRANGEMENT TO INCLUDE)" % order_of_missing_groups_as_text
        missing_line = Line(bold(warning))
        interface.log_error(warning)

    arrangement = get_stored_arrangement_and_group_order(interface=interface, report_type=report_generator.specific_parameters_for_type_of_report.report_type).arrangement_options
