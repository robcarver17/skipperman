...
from app.data_access.store.object_definitions import (
    object_definition_for_dict_of_cadets_with_attendance,
    object_definition_for_attendance_of_cadets_for_cadet_id,
)
...
def delete_raw_attendance_for_cadet_and_return_list_of_events(
    ...
    attendance = get_attendance_across_events_for_list_of_cadets(
        object_store, ListOfCadets([cadet])
    )
    events = attendance.attendance_for_cadet_across_days_and_events(
        cadet
    ).list_of_events
...
def update_attendance_across_events_for_list_of_cadets(
...
def get_attendance_across_events_for_cadets_in_group_at_event(
    ...
    return get_attendance_across_events_for_list_of_cadets(
        object_store=object_store, list_of_cadets=cadets_in_group
    )


def get_current_attendance_on_day_for_cadets_in_group(
    object_store: ObjectStore, event: Event, group: Group, day: Day
) -> Dict[Cadet, AttendanceOnDay]:
    attendance = get_attendance_across_events_for_cadets_in_group_at_event(
    ...
    attendance.mark_unknown_cadets_as_not_attending_or_unregistered(
        event=event, day=day, availability_dict=availability_dict
    )

    current_attendance = get_current_attendance_at_event_on_day(
        day=day,
        event=event,
...
def get_current_attendance_at_event_on_day(
    dict_of_attendance_across_events_for_list_of_cadets: DictOfAttendanceAcrossEvents,
    event: Event,
    day: Day,
) -> Dict[Cadet, AttendanceOnDay]:
    ...
    return dict(
        [
            (
                cadet,
                attendance_across_events.attendance_for_cadet_at_event(
                    event
...
def are_all_cadets_in_group_marked_in_registration_as_present_absent_or_late(
    ...
    list_of_current_attendance = [
        attendance_for_cadet.current_attendance
        for attendance_for_cadet in attendance_for_all_cadets
    ]
    requires_save = [
        current_attendance == registration_not_taken
