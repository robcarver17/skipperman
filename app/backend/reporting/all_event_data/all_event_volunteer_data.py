...
def get_df_for_volunteers_event_data_dump(interface: abstractInterface, event: Event):
    ...
    list_of_volunteer_ids = (
        list_of_identified_volunteers.list_of_volunteer_ids_including_unallocated()
    )
    list_of_volunteer_names = [
        get_volunteer_name_or_not_allocated(
    ...
    list_of_skills = [
        get_skills_string(interface=interface, volunteer_id=volunteer_id, default="")
        for volunteer_id in list_of_volunteer_ids
    ]
    list_of_role_group = [
        get_role_group(interface=interface, event=event, volunteer_id=volunteer_id)
        for volunteer_id in list_of_volunteer_ids
    ]
    list_of_boats = [
        get_patrol_boat(interface=interface, event=event, volunteer_id=volunteer_id)
        for volunteer_id in list_of_volunteer_ids
    ]
...
def get_volunteer_name_or_not_allocated(
...
def get_connected_cadet_names(
    interface: abstractInterface, event: Event, volunteer_id: str, default=""
):
    volunteers_at_event_data = VolunteerAllocationData(interface.data)
...
def get_role_group(
    interface: abstractInterface, volunteer_id: str, event: Event, default=""
):
    ...
    role_dict = dict(
        [
            (
                day,
                volunteer_rota_data.get_volunteer_role_at_event_on_day_for_volunteer_id(
                    event=event, day=day, volunteer_id=volunteer_id
                )
                + " "
                + volunteer_rota_data.get_volunteer_group_name_at_event_on_day(
                    event=event,
                    day=day,
                    volunteer_id=volunteer_id,
...
def get_patrol_boat(
    ...
    boat_name_dict = dict(
        [
            (
                day,
                patrol_boat_data.get_boat_name_allocated_to_volunteer_on_day_at_event(
...
def data_from_volunteers_at_event_data_or_empty(
    ...
    return getattr(
        list_of_volunteers_at_event.volunteer_at_event_with_id(volunteer_id), keyname
    )
