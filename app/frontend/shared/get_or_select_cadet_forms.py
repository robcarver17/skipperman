...
class ParametersForGetOrSelectCadetForm:
    ...
    def get_values_from_state(self, interface: abstractInterface):
        self.sort_by = interface.get_persistent_value(
            SORT_BY_STATE, SORT_BY_SIMILARITY_BOTH
        )
        self.final_add_button = interface.get_persistent_value(FINAL_ADD, False)
        self.see_all_cadets_button = interface.get_persistent_value(
...
def get_add_or_select_existing_cadet_form(
    ...
    footer_buttons = get_footer_buttons_add_or_select_existing_cadets_form(
        interface=interface, parameters=parameters, cadet_and_text=cadet_and_text
    )
    # Custom header text
...
def get_cadet_and_verification_text(
    interface: abstractInterface, cadet: Cadet = arg_not_passed
) -> CadetAndVerificationText:
...
def generic_post_response_to_add_or_select_when_returning_new_form(
    ...
    elif see_similar_cadets_only_button.pressed(last_button_pressed):
