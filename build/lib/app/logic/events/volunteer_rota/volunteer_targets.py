...
from app.backend.volunteers.volunteer_rota_summary import get_list_of_actual_and_targets_for_roles_at_event, \
    RowInTableWithActualAndTargetsForRole, save_new_volunteer_target
...
def get_volunteer_targets_table_and_save_button(event: Event,
    ...
    return DetailListOfLines(ListOfLines([table, button]).add_Lines(), name='Role numbers and targets')

def get_volunteer_targets_table(event: Event,
                        interface: abstractInterface,
                        )-> Table:

    top_row = get_top_row_of_volunteer_targets_table(event=event)
    other_rows = get_body_of_volunteer_targets_table(event=event,
                                            interface=interface)
...
def get_top_row_of_volunteer_targets_table(event: Event) -> RowInTable:
    actual_for_days_at_event_as_str = ["%s (allocated)" % day.name for day in event.weekdays_in_event()]
    return RowInTable([
        "Role"
...
def get_body_of_volunteer_targets_table(event: Event,
                               interface: abstractInterface
                               ) -> List[RowInTable]:

    data_for_table = get_list_of_actual_and_targets_for_roles_at_event(interface=interface, event=event)
    ready_to_swap = is_ready_to_swap(interface)

    other_rows = [get_row_for_volunteer_targets_at_event(row_of_data=row_of_data,
                                                         event=event,
...
def get_row_for_volunteer_targets_at_event(row_of_data: RowInTableWithActualAndTargetsForRole,
                                           event: Event,
    ...
    target_box = get_target_box_in_form(role=role, target=row_of_data.target, ready_to_swap=ready_to_swap)
...
def get_target_box_in_form(role: str, target: int, ready_to_swap:bool=False):
    if ready_to_swap:
        ...
        return intInput(value=target, input_label='', input_name=get_input_name_for_target_box(role))
...
def save_volunteer_targets(interface: abstractInterface):
    ...
    for row in data_for_table:
        try:
            save_volunteer_targets_for_specific_role(interface=interface, event=event, role=row.role)
        except:
            ## roles must have changed, no bother
            continue

def save_volunteer_targets_for_specific_role(interface: abstractInterface, event: Event, role: str):
    new_target = get_target_from_form(interface=interface, role=role)
    save_new_volunteer_target(interface=interface, event=event, role=role, target=new_target)
