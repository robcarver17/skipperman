...
def update_dict_of_volunteers_with_last_role_and_group_across_events(
    ...
    list_of_roles = (
        dict_of_volunteers_with_last_role_and_group_across_events.list_of_most_common_role_for_volunteers_across_events_with_id
    )
    interface.update(
...
def update_dict_of_volunteers_with_most_common_role_and_group_across_events_from_core_data(
    interface: abstractInterface,
):
    list_of_volunteers = get_list_of_volunteers(interface.object_store)
    dict_of_volunteers_with_most_common_role_and_group = (
        get_dict_of_volunteers_with_last_role_and_group_across_events(
            interface.object_store
        )
    )
    for volunteer in list_of_volunteers:
        most_common_role_for_volunteer_at_previous_events = (
            get_most_common_role_for_volunteer_at_previous_events_from_underlying_data(
                object_store=interface.object_store, volunteer=volunteer
            )
        )
        if (
            most_common_role_for_volunteer_at_previous_events is None
        ) or most_common_role_for_volunteer_at_previous_events.is_unallocated:
            continue
    ...
    update_dict_of_volunteers_with_last_role_and_group_across_events(
        interface=interface,
        dict_of_volunteers_with_last_role_and_group_across_events=dict_of_volunteers_with_most_common_role_and_group,
    )
