...
from app.backend.file_handling import get_staged_adhoc_filename, load_spreadsheet_file_and_clear_nans, \
    create_local_file_from_uploaded_and_return_filename
from app.backend.volunteers.skills import get_list_of_skills, get_dict_of_existing_skills_for_volunteer
...
def create_skills_refresh_file(interface: abstractInterface, file_marker_name: str):
    create_local_file_from_uploaded_and_return_filename(
        interface=interface, file_marker_name=file_marker_name, new_filename=skills_temporary_file
    )
...
def load_skills_refresh_file() -> ListOfRowsInSkillsFile:
    ...
    rows_in_skills_file = ListOfRowsInSkillsFile.from_df(df, skills_config=IMPORT_SKILLS_CONFIG)
...
def get_skills_dict_from_row_in_form(object_store: ObjectStore, current_row: RowInImportedSkillsFile) -> SkillsDict:
    ...
    for skill_name in list_of_skill_names:
        ...
        if skill is missing_data:
            raise Exception("Skill %s in import configuration is not a named skill in Skipperman, must be one of %s" % (skill_name, str(list_of_skills.list_of_names())))
...
def compare_skills_for_volunteer_with_passed_and_return_warning(object_store: ObjectStore, volunteer: Volunteer, current_row: RowInImportedSkillsFile) -> List[str]:
    existing_skills =get_dict_of_existing_skills_for_volunteer(object_store=object_store, volunteer=volunteer)
    passed_skills_dict = get_skills_dict_from_row_in_form(object_store, current_row)

    msgs = []
    for skill in existing_skills:
        add_messages_for_skill(skill=skill,
...
def add_messages_for_skill(skill: Skill, existing_skills: SkillsDict, passed_skills_dict: SkillsDict, current_row: RowInImportedSkillsFile, msgs: List[str]):
    ...
    if existing_has == new_has:
        ...
        msgs.append(
            "%s has skill %s in current skipperman data but not imported file: %s" % (current_row.volunteer_name(), skill.name, more))
