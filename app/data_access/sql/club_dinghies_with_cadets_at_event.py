...
from app.objects.volunteers_and_cades_at_event_with_club_boat_with_ids import (
    ListOfCadetAtEventWithIdAndClubDinghies, CadetAtEventWithClubDinghyWithId,
    no_club_dinghy_id
)
...
class SqlDataListOfCadetAtEventWithClubDinghies(
    ...
    def update_or_add_cadet_with_club_dinghy_on_day(self, event_id: str, cadet_id: str, day: Day, club_dinghy_id:str):
        if club_dinghy_id == no_club_dinghy_id:
            return self.set_club_dinghy_on_day_for_cadet_as_no_club_dinghy(event_id=event_id,
                                                                           cadet_id=cadet_id,
                                                                           day=day)
        elif self.is_a_club_dinghy_allocated_for_cadet_on_day_of_event(event_id=event_id,
                                                                       cadet_id=cadet_id,
                                                                       day=day):
            self._update_club_dinghy_allocated_for_cadet_on_day_of_event_no_checks(event_id=event_id,
                                                                                   cadet_id=cadet_id,
                                                                                   day=day, club_dinghy_id=club_dinghy_id)
        else:
            self._add_club_dinghy_allocated_for_cadet_on_day_of_event_without_checks(event_id=event_id,
                                                                                     cadet_id=cadet_id,
                                                                                     day=day,
                                                                                     club_dinghy_id=club_dinghy_id)


    def set_club_dinghy_on_day_for_cadet_as_no_club_dinghy(self,  event_id: str, cadet_id: str, day: Day):
        if self.is_a_club_dinghy_allocated_for_cadet_on_day_of_event(event_id=event_id, cadet_id=cadet_id, day=day):
            self.delete_club_dinghy_allocated_for_cadet_on_day_of_event(
                event_id=event_id,
                cadet_id=cadet_id,
                day=day
            )
    ...
    def _update_club_dinghy_allocated_for_cadet_on_day_of_event_no_checks(self, event_id: str, cadet_id: str, day: Day, club_dinghy_id:str):
        try:
            if self.table_does_not_exist(CADETS_AND_CLUB_DINGHIES_TABLE):
                return

            self.cursor.execute("UPDATE %s SET %s='%d' WHERE %s='%s' AND %s=%d AND %s=%d " % (CADETS_AND_CLUB_DINGHIES_TABLE,
                                                                    DINGHY_ID,
                                                                                                  int(club_dinghy_id),
                                                                                           DAY,
                                                                                           day.name,
    ...
    def _add_club_dinghy_allocated_for_cadet_on_day_of_event_without_checks(self, event_id: str, cadet_id: str, day: Day, club_dinghy_id:str):
        try:
            if self.table_does_not_exist(CADETS_AND_CLUB_DINGHIES_TABLE):
                self.create_table()
            self._add_cadet_with_club_dinghy_without_commits_or_checks(
                event_id=event_id,
                cadet_and_boat=
    ...
    def delete_club_dinghy_allocated_for_cadet_on_day_of_event(self,  event_id: str, cadet_id: str, day: Day):
        try:
            if self.table_does_not_exist(CADETS_AND_CLUB_DINGHIES_TABLE):
                return
            ...
            self.cursor.execute("DELETE FROM %s WHERE %s='%s' AND %s=%d AND %s=%d " % (CADETS_AND_CLUB_DINGHIES_TABLE,
                                                                                           DAY,
                                                                                           day.name,
                                                                 EVENT_ID,
                                                                 int(event_id),
    ...
    def delete_cadet_from_event_and_return_messages(self, event_id: str, cadet_id: str):
        if not self.is_a_club_dinghy_allocated_for_cadet_on_any_day_at_event(event_id=event_id, cadet_id=cadet_id):
            return []

        self.remove_all_allocations_for_cadet_at_event(event_id=event_id, cadet_id=cadet_id)

        return ["Removed club dinghies for cadet %s at event %s" % (cadet_id, event_id)]

    def remove_all_allocations_for_cadet_at_event(
        self,
        event_id: str, cadet_id: str
    ):
        try:
            if self.table_does_not_exist(CADETS_AND_CLUB_DINGHIES_TABLE):
                return
            ...
            ## NEEDS TO DELETE OLD
            ## TEMPORARY UNTIL CAN DO PROPERLY
            self.cursor.execute("DELETE FROM %s WHERE %s=%d AND %s=%d " % (CADETS_AND_CLUB_DINGHIES_TABLE,
                                                                 EVENT_ID,
                                                                 int(event_id),
                                                                               CADET_ID,
                                                                               int(cadet_id)))
    ...
    def is_a_club_dinghy_allocated_for_cadet_on_day_of_event(
            self, event_id: str, cadet_id: str, day: Day
    ) -> bool:
    ...
    def write(
        ...
        try:
            ...
            ## NEEDS TO DELETE OLD
            ## TEMPORARY UNTIL CAN DO PROPERLY
            self.cursor.execute("DELETE FROM %s WHERE %s=%d " % (CADETS_AND_CLUB_DINGHIES_TABLE,
                                                                 EVENT_ID,
                                                                 int(event_id)))

            for cadet_and_boat in people_and_boats:
                self._add_cadet_with_club_dinghy_without_commits_or_checks(event_id=event_id, cadet_and_boat=cadet_and_boat)
    ...
    def _add_cadet_with_club_dinghy_without_commits_or_checks(self, event_id: str, cadet_and_boat: CadetAtEventWithClubDinghyWithId):
