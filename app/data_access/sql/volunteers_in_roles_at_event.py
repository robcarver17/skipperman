...
class SqlDataListOfVolunteersInRolesAtEvent(GenericSqlData):
    ...
    def existing_role_id_on_day(
        ...
        try:
            ...
            raise Exception(
                "Error %s when reading volunteers in role at event %s"
                % (str(e1), event_id)
            )
        finally:
            self.close()
    ...
    def get_dict_of_volunteers_with_roles_and_groups_at_event(
        ...
        for raw_item in raw_list:
            ...
            entry_for_volunteer = new_dict.get(
    ...
    def get_role_and_groups_for_event_and_volunteer(
        ...
        try:
            cursor = self.cursor
            cursor.execute(
                """SELECT %s, %s, %s FROM %s WHERE %s=%d AND %s=%d """
                % (
                    DAY,
                    ROLE_ID,
                    GROUP_ID,
    ...
    def list_of_roles(self):
        list_of_roles = self.object_store.get(
            self.object_store.data_api.data_list_of_roles.read
        )
    ...
    def write(
        ...
        try:
            ...
            self.cursor.execute(
                ...
                % (VOLUNTEERS_IN_ROLES_TABLE, EVENT_ID, int(event_id))
            )

            for volunteer_in_role in list_of_volunteers_in_roles_at_event:
                self._write_row_of_volunteer_data_without_checks_or_commit(
                    event_id=event_id, volunteer_in_role=volunteer_in_role
                )
    ...
    def _write_row_of_volunteer_data_without_checks_or_commit(
        ...
        self.cursor.execute(
            insertion, (int(event_id), volunteer_id, day, group_id, role_id)
        )
    ...
    def create_table(self):
        ...
        index_creation_query = "CREATE UNIQUE INDEX %s ON %s (%s, %s, %s)" % (
            INDEX_VOLUNTEERS_IN_ROLES_TABLE,
            VOLUNTEERS_IN_ROLES_TABLE,
            EVENT_ID,
            VOLUNTEER_ID,
            DAY,
        ...
        try:
            ...
            raise Exception(
                "Error %s when creating volunteers in roles table" % str(e1)
            )
        finally:
            self.close()
