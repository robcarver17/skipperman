...
def get_summary_list_of_teams_and_groups_for_events(interface: abstractInterface, event: Event) -> PandasDFTable:
    return PandasDFTable(get_summary_list_of_teams_and_groups_for_events_as_pd_df(interface=interface, event=event))


def get_summary_list_of_teams_and_groups_for_events_as_pd_df(interface: abstractInterface, event: Event) -> pd.DataFrame:
    all_day_summaries= list_of_day_summaries_teams_and_groups_at_event(interface=interface, event=event)
    single_df = from_list_of_day_summaries_to_single_df(all_day_summaries=all_day_summaries, event=event)
...
def get_summary_of_teams_and_groups_for_events_on_day(
    ...
    for group_name in all_group_names:
        for team in all_teams:
            ...
            count = team_and_group_with_count(team_and_group,
                                                              list_of_teams_and_groups=list_of_teams_and_groups)
...
def team_and_group_with_count(team_and_group: TeamAndGroup, list_of_teams_and_groups: List[RoleAndGroup]) -> int:
    matching = [team_and_group_in_list for team_and_group_in_list in list_of_teams_and_groups if
                team_and_group_in_list == team_and_group]
