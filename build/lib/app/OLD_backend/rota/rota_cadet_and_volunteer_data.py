...
from app.objects.groups import sorted_locations, Group
...
def get_cadet_location_string(
    ...
    list_of_groups = list_of_cadet_groups_associated_with_volunteer(
        data_layer, volunteer_at_event=volunteer_at_event
    )
...
def list_of_cadet_groups_associated_with_volunteer(
    ...
    for cadet_id in list_of_cadet_ids:
        list_of_groups += groups_given_cadet_id(
            data_layer=data_layer, cadet_id=cadet_id, event=volunteer_at_event.event
        )

    return list(set(list_of_groups))


def groups_given_cadet_id(
    data_layer: DataLayer, event: Event, cadet_id: str
) -> List[Group]:
    group_data = GroupAllocationsData(data_layer)
    list_of_cadet_ids_with_groups = group_data.groups_given_cadet_id(
