...
class NoteWithVolunteer:
    ...
    @classmethod
    def from_new_quick_note_and_volunteer_author(cls, note: Note, author_volunteer: Volunteer):
    ...
    def update_attributes(self, text: str, priority: str, assigned_volunteer: Volunteer, completed: bool):
...
class ListOfNotesWithVolunteers(List[NoteWithVolunteer]):
    def __init__(self, raw_list: List[NoteWithVolunteer], list_of_notes: ListOfNotes, list_of_volunteers: ListOfVolunteers):
    ...
    def _generic_sort(self, sort_by_attribute:str, reverse: bool = False):
        raw_list = sorted(self, key=lambda note: getattr(note, sort_by_attribute), reverse=reverse)
        return ListOfNotesWithVolunteers(raw_list=raw_list, list_of_notes=self.list_of_notes, list_of_volunteers=self.list_of_volunteers)

    def completed_only(self) -> 'ListOfNotesWithVolunteers':
        new_list = [note for note in self if note.completed]
        return ListOfNotesWithVolunteers(new_list, list_of_notes=self.list_of_notes.completed_only(), list_of_volunteers=self.list_of_volunteers)

    def uncompleted_only(self) -> 'ListOfNotesWithVolunteers':
        new_list = [note for note in self if not note.completed]
        return ListOfNotesWithVolunteers(new_list, list_of_notes=self.list_of_notes.uncompleted_only(), list_of_volunteers=self.list_of_volunteers)


    def add_quick_note(self, text: str, author_volunteer: Volunteer):
        note = self.list_of_notes.add_quick_note(text=text, author_volunteer_id=author_volunteer.id)
        note_with_volunteer = NoteWithVolunteer.from_new_quick_note_and_volunteer_author(
            note=note,
            author_volunteer=author_volunteer
        )
    ...
    def update_attributes(self,
        ...
        existing_note.update_attributes(priority=priority, completed=completed,
    ...
    def note_with_id(self, note_id: str) -> NoteWithVolunteer:
        return get_unique_object_with_attr_in_list(self, attr_name='id', attr_value=note_id)
...
def compose_list_of_notes_with_volunteers_from_list_of_notes(list_of_notes: ListOfNotes, list_of_volunteers: ListOfVolunteers) -> ListOfNotesWithVolunteers:
    raw_list = [
        NoteWithVolunteer.from_note(note=note, list_of_volunteers=list_of_volunteers)
        for note in list_of_notes
    ]

    return ListOfNotesWithVolunteers(raw_list=raw_list, list_of_volunteers=list_of_volunteers, list_of_notes=list_of_notes)
