...
class DictOfCadetsWithFoodRequirementsAtEvent(Dict[Cadet, FoodRequirements]):
    ...
    def add_new_cadet_with_food_to_event(self,
            cadet: Cadet,
            food_requirements: FoodRequirements,
    ):
        self[cadet] = food_requirements
        self.list_of_cadets_with_ids_and_food_requirements.add_new_cadet_with_food_to_event(cadet_id=cadet.id,
                                                                                            food_requirements=food_requirements)

    def remove_food_requirements_for_cadet_at_event(self,
            cadet: Cadet):

        try:
            self.pop(cadet)
            self.list_of_cadets_with_ids_and_food_requirements.remove_food_requirements_for_cadet_at_event(cadet_id=cadet.id)
        except:
            pass

    def subset_matches_food_required_description(self, specific_food_requirements: FoodRequirements):
        raw_dict = dict([
            (cadet, food_requirements) for cadet, food_requirements in self if food_requirements==specific_food_requirements
        ])
    ...
    def update_cadet_food_data(
        ...
        self.list_of_cadets_with_ids_and_food_requirements.change_food_requirements_for_cadet(cadet_id=cadet.id, food_requirements=new_food_requirements)
    ...
    def _create_with_new_raw_dict(self, raw_dict: Dict[Cadet, FoodRequirements]):
        ...
        return DictOfCadetsWithFoodRequirementsAtEvent(
            raw_dict=raw_dict,
            list_of_cadets_with_ids_and_food_requirements=self.list_of_cadets_with_ids_and_food_requirements.filter_for_list_of_cadet_ids(filtered_list_of_cadet_ids)
        )
...
class DictOfVolunteersWithFoodRequirementsAtEvent(Dict[Volunteer, FoodRequirements]):
    ...
    def drop_volunteer(self, volunteer: Volunteer):
        self.pop(volunteer)
        self.list_of_volunteers_with_ids_and_food_requirements.drop_volunteer(volunteer_id=volunteer.id)
    ...
    def subset_matches_food_required_description(self, specific_food_requirements: FoodRequirements):
        raw_dict = dict([
            (volunteer, food_requirements) for volunteer, food_requirements in self if food_requirements==specific_food_requirements
        ])
    ...
    def update_volunteer_food_data(
        ...
        self.list_of_volunteers_with_ids_and_food_requirements.change_food_requirements_for_volunteer(volunteer_id=volunteer.id,
                                                                                                      food_requirements=new_food_requirements)

    def filter_for_list_of_volunteers(self, list_of_volunteers: ListOfVolunteers):
        raw_dict = dict([
            (volunteer, self.food_for_volunteer(volunteer)) for volunteer in list_of_volunteers
        ])
        return self._create_with_new_raw_dict(raw_dict)

    def _create_with_new_raw_dict(self, raw_dict: Dict[Volunteer, FoodRequirements]):
        filtered_list_of_volunteer_ids = ListOfVolunteers(list(raw_dict.keys())).list_of_ids
        return DictOfVolunteersWithFoodRequirementsAtEvent(
            raw_dict=raw_dict,
    ...
    def food_for_volunteer(self, volunteer: Volunteer, return_empty: bool= False) -> FoodRequirements:
        try:
