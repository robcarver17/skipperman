...
def get_dict_of_active_cadets_with_clothing_at_event(
    ...
    unfiltered_dict= get_unfiltered_dict_of_active_cadets_with_clothing_at_event(object_store=object_store,
                                                                                 event=event)
    cadets_currently_serving_on_committee = get_list_of_cadets_currently_serving(object_store)

    if only_committee:
        return unfiltered_dict.filter_for_list_of_cadets(cadets_currently_serving_on_committee)
    else:
...
def get_unfiltered_dict_of_active_cadets_with_clothing_at_event(
    ...
    list_of_cadets_with_clothing = get_dict_of_cadets_with_clothing_at_event(object_store=object_store, event=event)
    dict_of_registered_cadets = get_dict_of_cadets_with_registration_data(object_store=object_store, event=event)
