...
class DEPRECATE_VolunteerAtEventWithPatrolBoat(GenericSkipperManObject):
    ...
    @classmethod
    def from_volunteer_with_id_and_patrol_boat_id(
        cls,
        volunteer_with_id_at_event_with_patrol_boat_id: VolunteerWithIdAtEventWithPatrolBoatId,
        volunteer: Volunteer,
        patrol_boat: PatrolBoat,
    ):
        return cls(
            ...
            day=volunteer_with_id_at_event_with_patrol_boat_id.day,
        )
...
class DEPRECATE_ListOfVolunteersAtEventWithPatrolBoats(GenericListOfObjects):
    ...
    def list_of_volunteers(self) -> ListOfVolunteers:
        return ListOfVolunteers(
            [
                volunteer_at_event_with_boat.volunteer
                for volunteer_at_event_with_boat in self
            ]
        )
    ...
    def list_of_volunteers_and_boats_assigned_to_boat_and_day(
        ...
        return DEPRECATE_ListOfVolunteersAtEventWithPatrolBoats(
            [
                volunteer_at_event_with_boat
                for volunteer_at_event_with_boat in self
                if volunteer_at_event_with_boat.day == day
                and volunteer_at_event_with_boat.patrol_boat == patrol_boat
            ]
        )
...
class VolunteerAtEventWithSkillsAndRolesAndPatrolBoats:
    ...
    def assigned_to_boat_on_day(self, patrol_boat: PatrolBoat, day: Day) -> int:
        patrol_boat_on_day = self.patrol_boat_by_day.get(
            day, patrol_boat.create_empty()
        )
    ...
    @classmethod
    def from_volunteer_at_event_with_skills_and_roles(
        cls,
...
class ListOfVolunteersAtEventWithSkillsAndRolesAndPatrolBoats(
    ...
    def assigned_to_any_boat_on_any_day(
        self,
    ) -> "ListOfVolunteersAtEventWithSkillsAndRolesAndPatrolBoats":
        return ListOfVolunteersAtEventWithSkillsAndRolesAndPatrolBoats(
            [
                volunteer_at_event_with_skills_and_roles_and_boats
                for volunteer_at_event_with_skills_and_roles_and_boats in self
                if volunteer_at_event_with_skills_and_roles_and_boats.assigned_to_any_boat_on_any_day()
            ]
        )
    ...
    def assigned_to_boat_on_day(
        ...
        return ListOfVolunteersAtEventWithSkillsAndRolesAndPatrolBoats(
            [
                volunteer_at_event_with_skills_and_roles_and_boats
                for volunteer_at_event_with_skills_and_roles_and_boats in self
                if volunteer_at_event_with_skills_and_roles_and_boats.assigned_to_boat_on_day(
    ...
    def list_of_volunteer_ids(self) -> List[str]:
        return [
            volunteer_at_event_with_skills_and_roles_and_boats.volunteer.id
            for volunteer_at_event_with_skills_and_roles_and_boats in self
        ]
    ...
    def volunteers_with_pb2(self):
        return ListOfVolunteersAtEventWithSkillsAndRolesAndPatrolBoats(
            [
                volunteer_at_event_with_skills_and_roles_and_boats
                for volunteer_at_event_with_skills_and_roles_and_boats in self
                if volunteer_at_event_with_skills_and_roles_and_boats.has_pb2_qualification()
            ]
        )
    ...
    def has_volunteer_role(self, role: str, day: Day):
        return ListOfVolunteersAtEventWithSkillsAndRolesAndPatrolBoats(
            [
                volunteer_at_event_with_skills_and_roles_and_boats
                for volunteer_at_event_with_skills_and_roles_and_boats in self
                if volunteer_at_event_with_skills_and_roles_and_boats.has_role_on_day(
                    day=day, role=role
                )
            ]
        )
