...
def qr_code_for_report(interface: abstractInterface, report_generator: ReportGenerator):
    print_options = get_print_options_without_override(
    ...
    return generate_qr_code_for_file_in_public_path(
        filename_without_extension=path_and_filename.filename_without_extension
    )
...
def create_generic_report_and_return_filename(
    ...
    dict_of_df = report_generator.get_dict_of_df(
        interface, override_additional_options=override_additional_options
    )
...
def get_print_options_without_override(
