...
from app.backend.reporting.options_and_parameters.marked_up_list_from_df_parameters import MarkedUpListFromDfParametersWithActualGroupOrder
...
class ReportingOptions:
    ...
    def filter_arrangement_options_in_place_to_remove_non_existent_groups(self):
        group_order_from_df = get_group_order_from_dict_of_df_given_report_parameters(dict_of_df=self.dict_of_df,
                                                                                      specific_parameters_for_type_of_report=self.specific_parameters)

        self.arrange_options_and_group_order.subset_if_in_other_group_order(group_order_from_df)
