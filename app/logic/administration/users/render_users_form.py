...
def table_for_users(interface: abstractInterface,     existing_list_of_users: ListOfSkipperManUsers) -> Table:
    existing_entries = rows_for_existing_list_of_users(interface=interface, existing_list_of_users=existing_list_of_users)
...
def row_for_new_user(interface: abstractInterface) -> RowInTable:
    ...
    return RowInTable(
        [ "Add new user: ", text_box_for_username(user), text_box_for_password(user),text_box_for_password(user, True),
...
def rows_for_existing_list_of_users(interface: abstractInterface, existing_list_of_users: ListOfSkipperManUsers) -> List[RowInTable]:

    return [
        get_row_for_existing_user(interface=interface, existing_user=user) for user in existing_list_of_users
    ]


def get_row_for_existing_user(interface: abstractInterface, existing_user: SkipperManUser) -> RowInTable:
    return RowInTable(
        [ '',existing_user.username, text_box_for_password(existing_user),
...
def text_box_for_email(user: SkipperManUser) -> emailInput:
    return emailInput(value=user.username,
