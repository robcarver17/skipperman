...
class SkillsAndRolesConfiguration(List[str]):
    ...
    def get_from_filename(cls, filename: str):
        with open(roles_and_skills_filename) as file_to_parse:
            skills_and_roles_configuration = yaml.load(
                file_to_parse, Loader=yaml.FullLoader
            )

        return cls(
            volunteer_skills_from_config=skills_and_roles_configuration[
                "volunteer_skills"
            ],
            dict_of_volunteer_teams=skills_and_roles_configuration[
                "dict_of_volunteer_teams"
            ],
            role_and_skills_required=skills_and_roles_configuration[
                "role_and_skills_required"
            ],
            volunteers_requiring_boats=skills_and_roles_configuration[
                "volunteers_requiring_boats"
            ],
            volunteers_requiring_group=skills_and_roles_configuration[
...
skills_and_roles_configuration = SkillsAndRolesConfiguration.get_from_filename(
    roles_and_skills_filename
)


def save_skills_and_rolls_to_configuration_file(
    skills_and_rolls: SkillsAndRolesConfiguration,
):
...
dict_of_roles_and_skills_required = (
    skills_and_roles_configuration.role_and_skills_required
)
