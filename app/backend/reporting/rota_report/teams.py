...
def dataframe_for_team(data_for_df: DataForDfConstruction, team: Team) -> pd.DataFrame:
    ...
    leaders_df = dataframe_for_team_leaders(data_for_df=data_for_df,
                                            team=team)
    others_df = get_sorted_df_for_rest_of_team(data_for_df=data_for_df,
                                               team=team)

    combined_df = combine_and_drop_initial_duplicates(leaders_df=leaders_df, others_df=others_df)
...
def dataframe_for_team_leaders(data_for_df: DataForDfConstruction, team: Team):
    ...
    df_for_team_leaders = [df_row_for_volunteer_in_role_at_event(
        volunteer_in_role_at_event_with_team_name=volunteer,
...
def get_sorted_df_for_rest_of_team(data_for_df: DataForDfConstruction, team: Team)-> pd.DataFrame:
    ...
    list_of_rest_of_team = [df_row_for_volunteer_in_role_at_event(
        volunteer_in_role_at_event_with_team_name=volunteer,
        data_for_df=data_for_df
    ) for volunteer in team if not volunteer in volunteers_to_exclude]
...
def list_of_volunteers_in_leadership_position(team: Team) -> List[VolunteerInRoleAtEventWithTeamName]:
    team_name = get_team_name(team)
