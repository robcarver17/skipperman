...
create_report_button = Button(QUALIFICATION_LIST_BUTTON_LABEL, tile=True) ## tile
expected_report_button = Button(STATUS_REPORT_BUTTON_LABEL, tile=True) ## tile
...
def post_form_for_qualifications_report(
    ...
    elif last_button == QUALIFICATION_LIST_BUTTON_LABEL:
        filename = write_qualifications_to_temp_csv_file_and_return_filename(interface)
        return File(filename)
    elif last_button== STATUS_REPORT_BUTTON_LABEL:
        return interface.get_new_form_given_function(display_form_for_qualification_status_report)
