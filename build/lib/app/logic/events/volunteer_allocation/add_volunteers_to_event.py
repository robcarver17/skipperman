...
from app.backend.volunteers.volunteer_allocation import update_cadet_connections_when_volunteer_already_at_event, \
    ...
    get_list_of_relevant_information, relevant_information_requires_clarification_or_cadets_not_permanently_connected, \
    add_volunteer_at_event, \
    get_volunteer_at_event_from_list_of_relevant_information_with_no_conflicts, NO_ISSUES_WITH_VOLUNTEER, \
...
def process_identified_volunteer_at_event(interface: abstractInterface) -> Union[Form, NewForm]:
    ...
    if all_cancelled:
        ...
        return process_new_volunteer_at_event_with_active_cadets(interface=interface,
                                                                 event=event,
                                                                 volunteer_id=volunteer_id)

def process_new_volunteer_at_event_with_active_cadets(interface: abstractInterface, volunteer_id: str, event: Event) -> Union[Form, NewForm]:
    ...
    issues_with_volunteer = relevant_information_requires_clarification_or_cadets_not_permanently_connected(interface=interface, list_of_relevant_information=list_of_relevant_information,
                                                                                                            event=event,
                                                                                       volunteer_id=volunteer_id)

    if issues_with_volunteer == NO_ISSUES_WITH_VOLUNTEER:
        print("Volunteer %s has no issues, adding automatically" % get_volunteer_from_id(interface=interface, volunteer_id=volunteer_id))
...
def process_new_volunteer_at_event_with_active_cadets_and_where_no_manual_intervention_required(interface: abstractInterface,
    ...
    list_of_cadet_ids = get_list_of_active_associated_cadet_id_in_mapped_event_data_given_identified_volunteer_and_cadet(interface=interface,
                                                                                                                         volunteer_id=volunteer_id,
                                                                                                                                   event=event)
    volunteer_at_event = get_volunteer_at_event_from_list_of_relevant_information_with_no_conflicts(list_of_relevant_information=list_of_relevant_information, volunteer_id=volunteer_id,
