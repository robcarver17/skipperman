...
from app.backend.events.cadets_at_event import get_health_notes_for_list_of_cadets_at_event
from app.backend.groups.cadets_with_groups_at_event import get_joint_attendance_matrix_for_cadets_in_group_at_event
...
def get_labelled_ticksheet_df_for_group_at_event(
    ...
    if asterix_club_boats:
        list_of_club_boat_bool = \
            is_a_club_dinghy_allocated_for_list_of_cadets_on_any_day_at_event(object_store=object_store, event=event,
                list_of_cadets=labelled_ticksheet.list_of_cadets)
    ...
    if medical_notes:
        health_notes = (
            get_health_notes_for_list_of_cadets_at_event(
                object_store = object_store, event=event, list_of_cadets = labelled_ticksheet.list_of_cadets
            )
        )
...
def get_labelled_ticksheet_df_for_cadets_in_group_at_event_for_qualification(
    ...
    return labelled_tick_sheet_from_ticksheet_data(ticksheet_data=ticksheet_data, group=group)
