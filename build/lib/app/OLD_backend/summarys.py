...
def DEPRECATE_summarise_generic_counts_for_event(
    event: Event,
...
def DEPRECATE_get_row_for_group(
    group,
    event: Event,
    list_of_ids_with_groups: list,
    ...
    list_of_ids_for_group = get_id_function(
        group=group, list_of_ids_with_groups=list_of_ids_with_groups, event=event
    )

    counts = [
        DEPRECATE_get_count_on_day_for_group(
            list_of_ids_for_group=list_of_ids_for_group,
            availability_dict=availability_dict,
            day=day,
        )
...
def DEPRECATE_get_count_on_day_for_group(
    list_of_ids_for_group: list, availability_dict: Dict[str, DaySelector], day: Day
) -> int:
    present = [
        available_on_day(id, availability_dict=availability_dict, day=day)
        for id in list_of_ids_for_group
    ]
