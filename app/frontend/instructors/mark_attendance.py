from app.frontend.shared.cadet_state import  update_state_for_specific_cadet, clear_cadet_state
...
def get_save_buttons(interface: abstractInterface,
                     event: Event,
                     group: Group,
                     day: Day):
    ...
    if not_initial_registration:
        ...
        list_of_buttons= [initial_mark_all_as_attending_or_absence_button, mark_all_as_attending_button]

    return Line(list_of_buttons)

save_returned_ticks_button = Button("Save changes", shortcut=SAVE_KEYBOARD_SHORTCUT)
initial_mark_all_as_attending_or_absence_button = Button("Save changes - anyone not ticked will be marked as absent", shortcut=SAVE_KEYBOARD_SHORTCUT)
mark_all_as_attending_button = Button("Mark all registered for today as attending (unless late)")
...
def post_instructor_attendance(interface: abstractInterface):
    ...
    if initial_registration:
        return post_instructor_attendance_if_button_pressed_and_is_initial_registration(interface, last_button)
    else:
        return post_instructor_attendance_if_button_pressed_and_not_initial_registration(interface, last_button)


def post_instructor_attendance_if_button_pressed_and_is_initial_registration(interface: abstractInterface,
                                                                       last_button: str):
    ...
    if initial_mark_all_as_attending_or_absence_button.pressed(last_button):
        ...
        save_any_changes_to_ticks(interface=interface, force_mark_to_present=True, initial_stage=True)
    else:
...
def post_instructor_attendance_if_button_pressed_and_not_initial_registration(interface: abstractInterface,
                                                                       last_button: str):
...
def cadet_button_pressed(interface: abstractInterface, last_button: str):
    cadet = cadet_from_button_pressed(object_store=interface.object_store,
...
def save_ticks_for_cadet(interface: abstractInterface,
    ...
    if initial_stage:
        save_ticks_for_cadet_in_initial_stage(interface=interface,
                                              attendance_across_events=attendance_across_events,
                                              event=event,
                                              day=day,
                                              cadet=cadet,
        ...
        save_ticks_for_cadet_after_initial_stage(interface=interface,
                                              attendance_across_events=attendance_across_events,
                                              event=event,
                                              day=day,
                                              cadet=cadet)

def save_ticks_for_cadet_in_initial_stage(interface: abstractInterface,
                         attendance_across_events: DictOfAttendanceAcrossEvents,
                         event: Event, day: Day,
                         cadet: Cadet,
                         force_mark_to_present: bool = False):
...
def save_ticks_for_cadet_after_initial_stage(interface: abstractInterface,
                                          attendance_across_events: DictOfAttendanceAcrossEvents,
                                          event: Event, day: Day,
                                          cadet: Cadet):
...
def should_we_tick_cadet_as_present(interface: abstractInterface,
    ...
    if hastick_for_present:
        if hastick_for_late:
            interface.log_error("You ticked present AND late for %s - just tick one please. Assuming cadet will be late." % cadet.name)
            return False
