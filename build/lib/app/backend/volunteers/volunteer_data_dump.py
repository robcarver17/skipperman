...
import pandas as pd
from app.objects.volunteers_in_roles import RoleAndGroup

from app.objects.events import Event, ListOfEvents

from app.backend.volunteers.volunteers import get_dict_of_existing_skills, get_connected_cadets

from app.backend.volunteers.volunteer_rota_data import get_all_roles_across_recent_events_for_volunteer_as_dict
...
def get_volunteer_data_dump(data_layer: DataLayer) -> pd.DataFrame:
    ...
    list_of_all_row_data = ListOfVolunteerRowData([get_row_data_for_volunteer(data_layer=data_layer, volunteer=volunteer) for volunteer in list_of_all_volunteers])

    all_data_as_dict = {"Name": list_of_all_row_data.list_of_names(),
                        "Connected cadets": list_of_all_row_data.list_of_connected_cadets(),
                        "Skills": list_of_all_row_data.list_of_skills_str()}

    all_data_as_dict.update(list_of_all_row_data.dict_of_list_of_roles_and_groups_by_event_as_str())
...
class ListOfVolunteerRowData(List[VolunteerRowData]):
    ...
    def list_of_roles_and_groups_for_specific_event_as_str(self, event: Event) -> List[str]:
...
def get_row_data_for_volunteer(data_layer: DataLayer, volunteer: Volunteer) -> VolunteerRowData:
    name = volunteer.name
    ...
    return VolunteerRowData(name=name,
