...
class ListOfPatrolBoatLabelsAtEvents(GenericListOfObjects):
    ...
    def copy_patrol_boat_labels_across_event(
        self, event_id: str, days_in_event: List[Day], overwrite: bool = False
    ):
        list_of_boat_ids = self.unique_set_of_boat_ids_at_event(event_id)
        for boat_id in list_of_boat_ids:
            self.copy_patrol_boat_labels_across_event_for_boat(
                event_id=event_id,
                boat_id=boat_id,
    ...
    def copy_patrol_boat_labels_across_event_for_boat(
        self,
        event_id: str,
        boat_id: str,
        ...
        earliest_label = self.earliest_label_for_boat_at_event(
            event_id=event_id, days_in_event=days_in_event, boat_id=boat_id
        )
        if earliest_label is missing_data:
            return

        for day in days_in_event:
            existing_label = self.get_label(
                event_id=event_id, boat_id=boat_id, day=day, default=""
            )
            ...
            self.add_or_modify(
                event_id=event_id, day=day, boat_id=boat_id, label=earliest_label
            )

    def earliest_label_for_boat_at_event(
        self, event_id: str, boat_id: str, days_in_event: List[Day]
    ):
        for day in days_in_event:
            existing_label = self.get_label(
                event_id=event_id, boat_id=boat_id, day=day, default=""
    ...
    def add_or_modify(self, event_id, boat_id, day: Day, label: str):
        existing_label = self.get_patrol_boat_label(
            event_id=event_id, boat_id=boat_id, day=day, default=missing_data
        )
    ...
    def _add(self, event_id, boat_id, day: Day, label: str):
        self.append(
            PatrolBoatLabelAtEvent(
                event_id=event_id, boat_id=boat_id, label=label, day=day
            )
        )

    def get_label(self, event_id: str, boat_id: str, day: Day, default="") -> str:
        patrol_boat_label = self.get_patrol_boat_label(
            event_id=event_id, boat_id=boat_id, day=day, default=missing_data
        )
    ...
    def get_patrol_boat_label(
        self, event_id: str, boat_id: str, day: Day, default
    ) -> PatrolBoatLabelAtEvent:
        return get_unique_object_with_multiple_attr_in_list(
            self, dict(event_id=event_id, boat_id=boat_id, day=day), default=default
        )
