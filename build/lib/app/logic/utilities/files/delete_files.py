...
from app.logic.utilities.files.render_files import checkbox_name_for_filename, DELETE_IN_CHECKBOX, \
...
def delete_selected_files(interface: abstractInterface):
    delete_selected_files_in_directory(interface=interface, directory_name=public_reporting_directory)
    delete_selected_files_in_directory(interface=interface, directory_name=download_directory)
    delete_selected_files_in_directory(interface=interface, directory_name=upload_directory)

def delete_selected_files_in_directory(interface: abstractInterface, directory_name: str):
    all_files = get_files_in_directory(directory_name)
    for filename in all_files:
        check_if_file_selected_and_delete(interface=interface,
                                          directory_name=directory_name,
                                          filename=filename)

def check_if_file_selected_and_delete(interface: abstractInterface, directory_name: str, filename: str):
    checkbox_list=  interface.value_of_multiple_options_from_form(checkbox_name_for_filename(directory_name=directory_name, filename=filename),
                                                                      default=[]) ## if file recently created won't be in list
