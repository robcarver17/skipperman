...
MOST_COMMON_ROLE_FOR_VOLUNTEER_ACROSS_EVENTS_TABLE = (
    "most_common_role_for_volunteer_at_event"
)
INDEX_MOST_COMMON_ROLE_FOR_VOLUNTEER_ACROSS_EVENTS_TABLE = (
    "index_most_common_role_for_volunteer_at_event"
)


class SqlDataListOfLastRolesAcrossEventsForVolunteers(GenericSqlData):
    def delete_volunteer_from_data(self, volunteer_id: str):
        try:
    ...
    def get_most_common_role_and_group_or_none_for_volunteer_at_previous_events(
        ...
        raw_item = self.read_for_volunteer(
            volunteer_id=volunteer_id, default=missing_data
        )
        ...
        teams_and_index = (
            self.dict_of_teams_and_roles.list_of_teams_and_index_given_role(
                role_with_skills
            )
        )
        role_group_team = RoleAndGroupAndTeam(
            role=role_with_skills, group=group, list_of_team_and_index=teams_and_index
        )
    ...
    def get_dict_of_volunteers_with_last_role_and_group_across_events(
        ...
        for raw_item in raw_list:
            ...
            group = self.list_of_groups.group_with_id(raw_item.group_id)
            role_with_skills = self.list_of_roles_with_skills.role_with_id(
                raw_item.role_id
            )
            teams_and_index = (
                self.dict_of_teams_and_roles.list_of_teams_and_index_given_role(
    ...
    def write(self, list_of_roles: ListOfMostCommonRoleForVolunteersAcrossEventsWithId):
        try:
            ...
            self.cursor.execute(
                "DELETE FROM %s" % (MOST_COMMON_ROLE_FOR_VOLUNTEER_ACROSS_EVENTS_TABLE)
            )
    ...
    def _add_row_without_checks_or_commits(
        self, volunteer_with_role: MostCommonRoleForVolunteerAcrossEventsWithId
    ):
        ...
        self.cursor.execute(insertion, (volunteer_id, role_id, group_id))
