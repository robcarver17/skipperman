...
from app.backend.groups.cadet_event_history import get_df_of_history_for_active_cadets, temp_file_name
from app.backend.groups.previous_groups import get_dict_of_event_allocations_for_single_cadet_given_list_of_events
...
def write_new_sailors_recent_group_history_and_qualification_status_to_temp_csv_file_and_return_filename(
    object_store: ObjectStore,
        date_cutoff: datetime.date = arg_not_passed
) -> str:
    if date_cutoff is arg_not_passed:
        date_cutoff = datetime.date.today() - datetime.timedelta(days=366)

    list_of_cadets = get_list_of_cadets_whose_first_event_was_after_date(object_store=object_store,
                                                                         date_cutoff=date_cutoff)

    list_of_recent_events = get_list_of_events_after_cuttoff(object_store=object_store,
                                                             date_cutoff=date_cutoff)
...
def get_list_of_cadets_whose_first_event_was_after_date(object_store: ObjectStore, date_cutoff: datetime.date):
    list_of_cadets = get_list_of_cadets(object_store)
    list_of_events = get_list_of_events(object_store)

    return ListOfCadets([
        cadet for cadet in list_of_cadets
...
def was_cadet_first_event_after_date(object_store: ObjectStore, cadet: Cadet, date_cutoff: datetime.date, list_of_events: ListOfEvents):
...
def get_list_of_events_after_cuttoff(object_store: ObjectStore,
                                 date_cutoff: datetime.date):

    list_of_events = get_list_of_events(object_store)
    return ListOfEvents([event for event in list_of_events if event.start_date>date_cutoff])