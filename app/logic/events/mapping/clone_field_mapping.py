...
def post_form_for_clone_event_field_mapping(interface: abstractInterface):
    ...
    try:
        ...
        interface.log_error("Event %s has no mapping set up, or mapping file is corrupted - choose another event"
                            % event_name_selected)
        return initial_state_form

    write_field_mapping_for_event(event=current_event, new_mapping=mapping)

    return form_with_message_and_finished_button("Mapping copied from event %s to %s" % (event_name_selected, str(current_event)), interface=interface)