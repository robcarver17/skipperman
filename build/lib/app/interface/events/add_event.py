...
def display_view_for_add_event(state_data: StateDataForAction):
    ...
    if last_button_pressed == ADD_EVENT_BUTTON_LABEL:
        ...
        return get_add_event_form(state_data=state_data, first_time_displayed=True)
        ...
        return get_add_event_form(state_data=state_data, first_time_displayed=False)
...
def get_add_event_form(    state_data: StateDataForAction,
                           first_time_displayed: bool = True):
    if first_time_displayed:
        return get_add_event_form_with_information_passed(state_data)
    else:
...
def get_add_event_form_with_information_passed(
    ...
    html_inside_form = get_html_inside_event_form(
...
def get_html_inside_event_form(
...
def form_fields_for_add_event(
    ...
    html_inside_form_entries = html_joined_list_as_paragraphs(list_of_form_entries)

    return html_inside_form_entries
